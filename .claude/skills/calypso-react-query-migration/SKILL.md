...
Shared shape types that already exist in `data-stores` (e.g. `SiteSubscriptionDetails`, `EmailDeliveryFrequency`) keep their original names when copied into `api-core` — renaming them creates a confusing divergence with the still-shipping data-stores definitions used by mutations and type guards.
...
For **mutually exclusive arguments** (e.g. `blogId` OR `subscriptionId`), use a labeled object slot rather than positional empty-string fallbacks: `[ 'read', '{name}', { blogId, subscriptionId } ]`. This prevents id-collision concerns (a `blogId` and a `subscriptionId` with the same numeric value never share a cache entry) and satisfies the `@tanstack/query/exhaustive-deps` ESLint rule, which a conditional ternary on the queryKey does not.
...
Fetcher tests under `packages/api-core/src/{name}/__tests__/` run with `yarn test-packages packages/api-core/src/{name}`. If the fetcher uses raw `fetch` (e.g. the subkey path), include `import 'jest-fetch-mock';` at the top of the test — without it, jsdom errors with `ReferenceError: fetch is not defined`. Assert subkey-path requests by passing `reqheaders: { authorization: 'X-WPSUBKEY ...' }` to `nock( BASE, { reqheaders } )`.
...
| Conditional ternary `queryKey: a ? [..., 'a', a] : [..., 'b', b]` for either-or args | Use a labeled object slot `[..., { a, b } ]` — works with `exhaustive-deps` and avoids id collision |
| Raw `fetch` in fetcher test with `ReferenceError: fetch is not defined` | `import 'jest-fetch-mock'` at the top of the test file |
