		...
		steps {
			...
			bashNodeScript {
				...
				scriptContent = """
					...
					# Run suite.
					xvfb-run yarn jest --reporters=jest-teamcity --reporters=default --maxWorkers=40% --workerIdleMemoryLimit=1GB --group=$testGroup

					# Restore exit on error.
					set -o errexit

					# Retry failed tests only.
					RETRY_COUNT=1 xvfb-run yarn jest --reporters=jest-teamcity --reporters=default --maxWorkers=40% --workerIdleMemoryLimit=1GB --group=$testGroup --onlyFailures
				"""
				dockerImage = "%docker_image_e2e%"
