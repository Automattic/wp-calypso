...
If you add or modify text strings displayed to the users, consider if they would be available to users who don't use English. If the text strings would be displayed to international users, they should be enabled for translation. To make text translatable, you’d use the `translate()` function from [i18n-calypso](../packages/i18n-calypso/README.md). We extract the translatable texts wrapped inside the function and write a POT file that is imported to [translate.wordpress.com](https://translate.wordpress.com/).
