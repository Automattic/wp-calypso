...
`connect` accepts two arguments. Both pass props to the component which respectively provide data and set the component up to handle behavior:
...
`connect` is a [currying](https://www.sitepoint.com/currying-in-functional-javascript/) function: after we pass it the `mapStateToProps` and `mapDispatchToProps` functions we call it a second time, this time passing the `PostDeleteButton` visual component. `connect` does not modify the component class passed in this way, but returns a new component class which is connected to the store. The resulting component can have additional props which were not defined in the original component.
