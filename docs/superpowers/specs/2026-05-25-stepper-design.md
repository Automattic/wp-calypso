...
```ts
type RequireOneOf< T, Keys extends keyof T > =
...
.step[data-status='completed']::after { background: var(--wpds-color-stroke-success); }
.step[data-status='error']::after    { background: var(--wpds-color-stroke-error); }
