...
1. **Bridge release — PFX-signed, carrying the dual `publisherName` above.**
   It must be **PFX-signed** (the current CircleCI release flow already is): an Azure-signed build would already be rejected by the stranded users it needs to reach.
