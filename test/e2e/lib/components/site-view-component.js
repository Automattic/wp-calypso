...
export default class SiteViewComponent extends AsyncBaseContainer {
	...
	async isWebPreviewPresent() {
		...
		return await driverHelper.isElementPresent(this.driver, by.css('.main .web-preview__external'));
	}
	...
	async selectSearchAndSocialPreview() {
		...
		return await driverHelper.clickWhenClickable(this.driver, by.css('a[data-e2e-title="seo"]'));
	}
	...
	static async switchToIFrame(driver) {
		...
		await driverHelper.waitTillPresentAndDisplayed(driver, by.css('.web-preview__inner.is-loaded'));
