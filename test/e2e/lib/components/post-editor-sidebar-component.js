...
export default class PostEditorSidebarComponent extends AsyncBaseContainer {
	...
	async publicizeMessagePlaceholder() {
		return await this.driver.findElement(this.publicizeMessageSelector).getAttribute('placeholder');
	}
	...
	async setPublicizeMessage(message) {
		return await driverHelper.setWhenSettable(this.driver, this.publicizeMessageSelector, message);
	}
