...
export default class GutenbergEditorComponent extends AsyncBaseContainer {
	...
	async update({ visit = false } = {}) {
		...
		await driverHelper.clickWhenClickable(this.driver, By.css('button.editor-post-publish-button'));
	...
	async getTitle() {
		return await this.driver.findElement(By.css('.editor-post-title__input')).getAttribute('value');
	}
	...
	async openBlockInserterAndSearch(searchTerm) {
		await driverHelper.scrollIntoView(this.driver, By.css('.block-editor-writing-flow'), 'start');
	...
	async toggleSidebar(open = true) {
		...
		const sidebarOpen = await driverHelper.isElementPresent(this.driver, By.css(sidebarSelector));
		if (open && !sidebarOpen) {
			return await driverHelper.clickWhenClickable(
				this.driver,
	...
	async viewPublishedPostOrPage() {
		const viewPostLink = await this.driver.findElement(By.css('.components-snackbar__content a'));
	...
	async closeEditor() {
		...
		return await driverHelper.clickWhenClickable(
			this.driver,
			By.css('.edit-post-header .edit-post-fullscreen-mode-close, .edit-post-header-toolbar__back')
		);
	}
