...
export default class GutenbergEditorSidebarComponent extends AsyncBaseContainer {
	...
	async selectDocumentTab() {
		...
		return await driverHelper.waitTillPresentAndDisplayed(this.driver, By.css('.components-panel'));
	}
	...
	async addNewCategory(category) {
		const addNewCategoryButtonSelector = By.css('.editor-post-taxonomies__hierarchical-terms-add');
	...
	async scheduleFuturePost() {
		...
		await driverHelper.clickWhenClickable(this.driver, By.css('.edit-post-post-schedule__toggle'));
