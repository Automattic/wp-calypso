...
export default class ReaderPage extends AsyncBaseContainer {
	...
	async siteOfLatestPost() {
		const href = await this.driver.findElement(by.css('.reader-visit-link')).getAttribute('href');
	...
	async shareLatestPost() {
		...
		const clickAndOpenShareModal = async () => {
			...
			await driverHelper.waitTillPresentAndDisplayed(this.driver, by.css('.site-selector__sites'));
	...
	async commentOnLatestPost(comment) {
		...
		await driverHelper.setWhenSettable(this.driver, by.css('.comments__form textarea'), comment);
