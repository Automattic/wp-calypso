...
export default class PeoplePage extends AsyncBaseContainer {
	...
	async viewerDisplayed(username) {
		let retries = 0;
		if (await DriverHelper.isElementPresent(this.driver, By.css('.section-header__label .count'))) {
	...
	async removeUserByName(username) {
		...
		return await DriverHelper.clickWhenClickable(this.driver, By.css('.dialog button.is-primary'));
	}
	...
	async removeOnlyEmailFollowerDisplayed() {
		await DriverHelper.clickWhenClickable(this.driver, By.css('.people-list-item__remove-button'));
		return await DriverHelper.clickWhenClickable(this.driver, By.css('.dialog button.is-primary'));
	}
