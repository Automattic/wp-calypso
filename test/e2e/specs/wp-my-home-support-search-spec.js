...
describe( `[${ host }] My Home "Get help" support search card: (${ screenSize }) @parallel`, function () {
	...
	step( 'Verify "Get help" support card is displayed', async function () {
		...
		await driverHelper.waitUntilElementLocatedAndVisible( driver, helpCardSelector );
		...
		// Verify it is there.
		const isGetHelpCardPresent = await driverHelper.isElementLocated( driver, helpCardSelector );
		assert.equal(
