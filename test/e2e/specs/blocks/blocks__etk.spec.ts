...
test.describe( 'Blocks: ETK', { tag: [ tags.GUTENBERG ] }, () => {
	test( 'As a user, I can add and configure ETK blocks', async ( {
		browser,
		page,
		...
		await test.step( 'And I enter post title', async function () {
			await editorPage.enterTitle( `Blocks: ETK - ${ helperData.getDateString( 'ISO-8601' ) }` );
		} );
