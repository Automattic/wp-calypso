...
test.describe(
	...
	() => {
		...
		test.fixme( '"wpcom_block_editor_template_part_choose_existing" and "replace" events fire correctly', async ( {
			page,
		} ) => {
			const accountName = getTestAccountByFeature( { ...features, variant: 'siteEditor' } );
			let testAccount: TestAccount;
		...
		// TESTOPS-49.
		test.describe
			.fixme( '"wpcom_block_editor_convert_to_template_part" and "detach_blocks" events fire correctly', () => {
			let testAccount: TestAccount;
			let fullSiteEditorPage: FullSiteEditorPage;
			let editorTracksEventManager: EditorTracksEventManager;
			let templatePartName: string;
