...
test.describe(
	...
	() => {
		...
		test.fixme(
			'"wpcom_block_editor_global_styles_menu_selected" event fires for Typography and Blocks',
			async ( { page } ) => {
				const accountName = getTestAccountByFeature( { ...features, variant: 'siteEditor' } );
				let testAccount: TestAccount;
				let fullSiteEditorPage: FullSiteEditorPage;
		...
		test.fixme(
			'"wpcom_block_editor_global_styles_update" event fires for color and typography changes',
			async ( { page } ) => {
				const accountName = getTestAccountByFeature( { ...features, variant: 'siteEditor' } );
				let testAccount: TestAccount;
				let fullSiteEditorPage: FullSiteEditorPage;
		...
		// .fixme: depends on the same global-styles panel flow. See TESTOPS-49.
		test.describe.fixme(
			'"wpcom_block_editor_global_styles_save" event fires with correct style properties',
			() => {
