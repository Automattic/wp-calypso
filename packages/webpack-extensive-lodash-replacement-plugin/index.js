	...
	return new Promise((resolve, reject) => {
		moduleResolver.resolve(
			...
			(error, resource, resourceResolveData) => {
				if (error) {
					reject(
						createError(`Could not find module ${packageName} for import on ${rootPath}.`, error)
					);
				}
...
class ExtensiveLodashReplacementPlugin {
	...
	apply(compiler) {
		...
		compiler.hooks.normalModuleFactory.tap('LodashReplacementPlugin', (nmf) => {
			...
			nmf.hooks.beforeResolve.tapPromise('LodashReplacementPlugin', this.modifyResult.bind(this));
		});
	}
}
