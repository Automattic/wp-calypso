...
import {
	addReadShelfSourceMutation,
	createReadShelfMutation,
	deleteReadShelfMutation,
	deleteReadShelfSourceMutation,
...
import type { ReadShelf, ReadShelfDetails, SiteSubscriptionItem } from '@automattic/api-core';
...
describe( 'read shelves mutations', () => {
	afterEach( () => nock.cleanAll() );

	describe( 'queries', () => {
		it( 'persists and keeps previous data for the shelves list query', () => {
			const options = readShelvesQuery();

			expect( options.queryKey ).toEqual( [ 'read', 'shelves', 'list' ] );
			expect( options.meta ).toEqual( { persist: true } );
			expect( options.placeholderData ).toBe( keepPreviousData );
			expect( options.refetchOnMount ).toBe( 'always' );
		} );

		it( 'persists shelf detail queries without bleeding the previous shelf', () => {
			const options = readShelfQuery( '3' );

			expect( options.queryKey ).toEqual( [ 'read', 'shelves', 'detail', '3' ] );
			expect( options.meta ).toEqual( { persist: true } );
			// No keepPreviousData on the detail query: a shelfId change must not flash
			// the previous shelf's name/sources. Persisted cache + refetchOnMount keep
			// the same shelf visible during refetch.
			expect( options.placeholderData ).toBeUndefined();
			expect( options.refetchOnMount ).toBe( 'always' );
		} );

		it( 'does not retry a shelf detail 4xx but still retries other failures', () => {
			const { retry } = readShelfQuery( '3' );
		...
		it( 'keys the by-slug detail query on the slug and does not retry a 4xx', () => {
			const options = readShelfBySlugQuery( 'work' );

			expect( options.queryKey ).toEqual( [ 'read', 'shelves', 'detail-by-slug', 'work' ] );
			expect( options.meta ).toEqual( { persist: true } );
			expect( options.refetchOnMount ).toBe( 'always' );
		...
		it( 'keys the by-slug query canonically, so encoded and decoded slugs share a key', () => {
			...
			const encoded = readShelfBySlugQuery( '%d0%bf%d1%80%d0%b8%d0%b2%d0%b5%d1%82' ).queryKey;
			const decoded = readShelfBySlugQuery( 'привет' ).queryKey;

			expect( encoded ).toEqual( [ 'read', 'shelves', 'detail-by-slug', 'привет' ] );
			expect( encoded ).toEqual( decoded );
		} );
	} );

	describe( 'createReadShelfMutation', () => {
		it( 'appends the summary to the list and seeds the detail cache', async () => {
			...
			await runMutation( client, createReadShelfMutation( client ), { name: 'New' } );

			expect( client.getQueryData< ReadShelf[] >( readShelvesQuery().queryKey ) ).toEqual( [
			...
			expect( client.getQueryData< ReadShelfDetails >( readShelfQuery( '99' ).queryKey ) ).toEqual(
				createdDetail
			);
			// The by-slug detail cache is seeded too, so a slug-addressed view of the new
			// shelf paints from cache without a round-trip.
			expect(
				client.getQueryData< ReadShelfDetails >( readShelfBySlugQuery( 'new' ).queryKey )
			).toEqual( createdDetail );
			expect( invalidateQueries ).toHaveBeenCalledWith( {
				queryKey: readShelvesQuery().queryKey,
			} );
			expect( invalidateQueries ).toHaveBeenCalledWith( {
				queryKey: readShelfQuery( '99' ).queryKey,
			} );
		} );
	} );

	describe( 'updateReadShelfMutation', () => {
		it( 'refreshes the matching list summary and the detail cache', async () => {
			const client = newClient();
			const invalidateQueries = jest.spyOn( client, 'invalidateQueries' );
			client.setQueryData< ReadShelf[] >( readShelvesQuery().queryKey, [
				{ id: '3', slug: 'old', name: 'Old', layout: { color: 'blue', icon: 'inbox' } },
			] );
			nock( BASE )
				.put( '/wpcom/v2/reader/shelves/3' )
				.reply(
					200,
					detailResponse( {
						id: 3,
			...
			await runMutation( client, updateReadShelfMutation( client ), {
				shelfId: '3',
			...
			expect(
				client.getQueryData< ReadShelfDetails >( readShelfQuery( '3' ).queryKey )
			).toMatchObject( { id: '3', name: 'New name', tags: [ 'x' ] } );
			expect( invalidateQueries ).toHaveBeenCalledWith( {
				queryKey: readShelvesQuery().queryKey,
			} );
			expect( invalidateQueries ).toHaveBeenCalledWith( {
				queryKey: readShelfQuery( '3' ).queryKey,
			} );
		} );
		...
		it( 'on a rename seeds the new by-slug cache and drops the old one', async () => {
			...
			client.setQueryData(
				readShelfBySlugQuery( 'old' ).queryKey,
			...
			expect(
				client.getQueryData< ReadShelfDetails >( readShelfBySlugQuery( 'new-name' ).queryKey )
			).toMatchObject( { id: '3', slug: 'new-name' } );
			// ...and the stale old-slug entry is removed.
			expect(
				client.getQueryData< ReadShelfDetails >( readShelfBySlugQuery( 'old' ).queryKey )
			).toBeUndefined();
		} );
		...
		it( 'optimistically patches the list summary before the server responds', async () => {
			...
			await updateReadShelfMutation( client ).onMutate?.( {
				shelfId: '3',
		...
		it( 'rolls back the optimistic list patch when the update fails', async () => {
			...
			client.setQueryData< ReadShelf[] >( readShelvesQuery().queryKey, seeded );
			nock( BASE ).put( '/wpcom/v2/reader/shelves/3' ).reply( 500, { error: 'boom' } );
			...
			expect( client.getQueryData< ReadShelf[] >( readShelvesQuery().queryKey ) ).toEqual( seeded );
		} );

		it( "resets both of the shelf's streams so they reload after a tag/feed/language edit", async () => {
			const client = newClient();
			...
			// shelf's languages, so a language change must not leave it cached.
			expect( spy ).toHaveBeenCalledWith( {
				queryKey: getStreamInfiniteQueryKeyPrefix( 'shelf:3' ),
			} );
			expect( spy ).toHaveBeenCalledWith( {
				queryKey: getStreamInfiniteQueryKeyPrefix( 'shelf_discover:3' ),
			} );
		} );
	} );

	describe( 'deleteReadShelfMutation', () => {
		it( 'removes the deleted shelf from the list and discards its detail cache', async () => {
			const client = newClient();
			const invalidateQueries = jest.spyOn( client, 'invalidateQueries' );
			const keep: ReadShelf = {
				id: 'keep',
				slug: 'keep',
				name: 'Keep',
			...
			client.setQueryData( readShelfQuery( '3' ).queryKey, detailResponse() );
			client.setQueryData( readShelfBySlugQuery( 'work' ).queryKey, detailResponse() );
			nock( BASE ).delete( '/wpcom/v2/reader/shelves/3' ).reply( 200, { deleted: true, id: 3 } );

			await runMutation( client, deleteReadShelfMutation( client ), '3' );
			...
			expect(
				client.getQueryData< ReadShelfDetails >( readShelfBySlugQuery( 'work' ).queryKey )
			).toBeUndefined();
			expect( invalidateQueries ).toHaveBeenCalledWith( {
				queryKey: readShelvesQuery().queryKey,
			} );
	...
	describe( 'feed (source) mutations', () => {
		it( 'writes the returned detail to the cache after adding a feed', async () => {
			...
			const invalidateQueries = jest.spyOn( client, 'invalidateQueries' );
			nock( BASE )
				.post( '/wpcom/v2/reader/shelves/3/feeds' )
				.reply(
					200,
					detailResponse( {
						follows: [
			...
			await runMutation( client, addReadShelfSourceMutation( client ), {
				shelfId: '3',
				subscription: makeSubscription(),
			} );

			expect(
				client.getQueryData< ReadShelfDetails >( readShelfQuery( '3' ).queryKey )?.sources
			).toEqual( [
				{
					feedId: 456,
		...
		it( 'writes the returned detail to the cache after removing a feed', async () => {
			...
			await runMutation( client, deleteReadShelfSourceMutation( client ), {
				shelfId: '3',
				subscription: makeSubscription(),
			} );
		...
		it( 'leaves the detail cache untouched when the add request fails', async () => {
			...
			client.setQueryData( readShelfQuery( '3' ).queryKey, seeded );
			nock( BASE )
				.post( '/wpcom/v2/reader/shelves/3/feeds' )
				.reply( 409, {
					code: 'reader_shelves_duplicate_feed',
					message: '…',
					data: { status: 409 },
				} );
			...
			expect( client.getQueryData< ReadShelfDetails >( readShelfQuery( '3' ).queryKey ) ).toEqual(
				seeded
			);
		} );
	} );
