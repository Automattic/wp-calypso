...
describe('POT', () => {
	...
	test('should have all the default headers', () => {
		...
		expect(output).toEqual(expect.stringContaining('"Language-Team: LANGUAGE <LL@li.org>\\n"\n'));
	});
	...
	test('should create a context translation', () => {
		expect(output).toEqual(expect.stringContaining('msgctxt "verb"\nmsgid "post"\nmsgstr ""\n'));
	...
	test('should pass through an sprintf as a regular translation', () => {
		expect(output).toEqual(
			expect.stringContaining('msgid "Your city is %(city)s and your zip is %(zip)s."\nmsgstr ""\n')
		);
	});
	...
	test('should find translations from multiple files', () => {
		expect(output).toEqual(expect.stringContaining('msgid "My test has two files."\nmsgstr ""\n'));
	});
	...
	test('should handle template literals', () => {
		...
		expect(output).toEqual(expect.stringContaining('msgid "My hat\\nhas seventeen\\ncorners."'));
	});

	test('should properly handle unicode escapes', () => {
		expect(output).toEqual(expect.stringContaining('This is how the test performed\\\\u2026'));
		expect(output).toEqual(
...
describe('PHP', () => {
	...
	test('should create a plural _n() translation', () => {
		expect(output).toEqual(
			expect.stringContaining('_n( "My hat has four corners.", "My hats have five corners.", 1 ),')
		);
	});
	...
	test('should allow a variable placeholder for `count`', () => {
		expect(output).toEqual(expect.stringContaining('_n( "single test2", "plural test2", 1 ),'));
	});
