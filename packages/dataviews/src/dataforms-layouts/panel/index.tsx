...
function DropdownHeader( { title, onClose }: { title?: string; onClose: () => void } ) {
	return (
		<VStack className="dataforms-layouts-panel__dropdown-header" spacing={ 4 }>
			<HStack alignment="center">
				...
				<Spacer />
				{ onClose && (
					<Button label={ __( 'Close' ) } icon={ closeSmall } onClick={ onClose } size="small" />
				) }
			</HStack>
		</VStack>
	);
		...
		<Dropdown
			...
			renderContent={ ( { onClose } ) => (
				<>
					...
					<DataFormLayout data={ data } form={ form as Form } onChange={ onChange }>
...
export default function FormPanelField< Item >( {
	...
	if ( labelPosition === 'top' ) {
		return (
			<VStack className="dataforms-layouts-panel__field" spacing={ 0 }>
				<div className="dataforms-layouts-panel__field-label" style={ { paddingBottom: 0 } }>
					{ fieldLabel }
				</div>
	...
	return (
		<HStack ref={ setPopoverAnchor } className="dataforms-layouts-panel__field">
			<div className="dataforms-layouts-panel__field-label">{ fieldLabel }</div>
			<div className="dataforms-layouts-panel__field-control">
				<PanelDropdown
					field={ field }
					popoverAnchor={ popoverAnchor }
