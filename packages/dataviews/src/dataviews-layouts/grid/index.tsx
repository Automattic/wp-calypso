...
import type { Action, NormalizedField, ViewGrid as ViewGridType, ViewGridProps } from '../../types';
...
function GridItem< Item >( {
	...
	const renderedMediaField = mediaField?.render ? <mediaField.render item={ item } /> : null;
	...
	return (
		...
		>
			...
			<HStack justify="space-between" className="dataviews-view-grid__title-actions">
				<div { ...clickableTitleItemProps } { ...titleA11yProps }>
					{ renderedTitleField }
				</div>
				{ !! actions?.length && <ItemActions item={ item } actions={ actions } isCompact /> }
			</HStack>
			<VStack spacing={ 1 }>
				{ showDescription && descriptionField?.render && <descriptionField.render item={ item } /> }
				{ !! badgeFields?.length && (
					<HStack
					...
					>
						{ badgeFields.map( ( field ) => {
							return (
								<Badge key={ field.id } className="dataviews-view-grid__field-value">
