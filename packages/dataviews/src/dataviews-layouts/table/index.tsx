...
function TableColumnField< Item >( { item, fields, column }: TableColumnFieldProps< Item > ) {
	...
	return (
		...
		>
			...
			{ columns.map( ( column: string ) => {
				...
				const { width, maxWidth, minWidth } = view.layout?.styles?.[ column ] ?? {};

				return (
					<td key={ column } style={ { width, maxWidth, minWidth } }>
						<TableColumnField fields={ fields } item={ item } column={ column } />
					</td>
				);
			} ) }
			{ !! actions?.length && (
...
function ViewTable< Item >( {
	...
	const headerMenuRefs = useRef< Map< string, { node: HTMLButtonElement; fallback: string } > >(
		new Map()
	);
	const headerMenuToFocusRef = useRef< HTMLButtonElement >();
	const [ nextHeaderMenuToFocus, setNextHeaderMenuToFocus ] = useState< HTMLButtonElement >();
	...
	const hasPrimaryColumn =
		...
		( descriptionField && showDescription );
	const columns = view.fields ?? [];
	const headerMenuRef = ( column: string, index: number ) => ( node: HTMLButtonElement ) => {
		if ( node ) {
			...
			>
				<thead>
					<tr className="dataviews-view-table__row">
						{ hasBulkActions && (
							<th className="dataviews-view-table__checkbox-column" scope="col">
								<BulkSelectionCheckbox
									selection={ selection }
									onChangeSelection={ onChangeSelection }
									data={ data }
						...
						{ !! actions?.length && (
							<th className="dataviews-view-table__actions-column">
								<span className="dataviews-view-table-header">{ __( 'Actions' ) }</span>
							</th>
						) }
					</tr>
				</thead>
