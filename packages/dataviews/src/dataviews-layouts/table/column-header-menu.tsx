	...
	return (
		<Menu>
			...
			>
				{ header }
				{ view.sort && isSorted && (
					<span aria-hidden="true">{ sortArrows[ view.sort.direction ] }</span>
				) }
			</Menu.TriggerButton>
			<Menu.Popover style={ { minWidth: '240px' } }>
				<WithMenuSeparators>
					{ isSortable && (
						<Menu.Group>
							{ SORTING_DIRECTIONS.map( ( direction: SortDirection ) => {
								...
								return (
									...
									>
										<Menu.ItemLabel>{ sortLabels[ direction ] }</Menu.ItemLabel>
									</Menu.RadioItem>
								);
							} ) }
						</Menu.Group>
					...
					{ canAddFilter && (
						<Menu.Group>
							...
							>
								<Menu.ItemLabel>{ __( 'Add filter' ) }</Menu.ItemLabel>
							</Menu.Item>
						</Menu.Group>
					) }
					{ ( canMove || isHidable ) && field && (
						<Menu.Group>
							...
							{ isHidable && field && (
								...
								>
									<Menu.ItemLabel>{ __( 'Hide column' ) }</Menu.ItemLabel>
								</Menu.Item>
							) }
						</Menu.Group>
					) }
...
const ColumnHeaderMenu: < Item >(
	props: PropsWithoutRef< HeaderMenuProps< Item > > & RefAttributes< HTMLButtonElement >
