...
function generatePrimaryActionCompositeId( idPrefix: string, primaryActionId: string ) {
...
function ListItem< Item >( {
	...
	const renderedTitleField =
		showTitle && titleField?.render ? <titleField.render item={ item } /> : null;
		...
		>
			<HStack className="dataviews-view-list__item-wrapper" spacing={ 0 }>
				...
				<HStack spacing={ 3 } justify="start" alignment="flex-start">
					{ renderedMediaField }
					<VStack spacing={ 1 } className="dataviews-view-list__field-wrapper">
						<HStack spacing={ 0 }>
							<div className="dataviews-title-field" id={ labelId }>
								{ renderedTitleField }
							</div>
							{ usedActions }
						</HStack>
						...
						<div className="dataviews-view-list__fields" id={ descriptionId }>
							{ otherFields.map( ( field ) => (
								<div key={ field.id } className="dataviews-view-list__field">
									<VisuallyHidden as="span" className="dataviews-view-list__field-label">
										{ field.label }
									</VisuallyHidden>
...
export default function ViewList< Item >( props: ViewListProps< Item > ) {
	const { actions, data, fields, getItemId, isLoading, onChangeSelection, selection, view } = props;
	...
	const onSelect = ( item: Item ) => onChangeSelection( [ getItemId( item ) ] );
	...
	const [ activeCompositeId, setActiveCompositeId ] = useState< string | null | undefined >(
		undefined
	);
	...
	useEffect( () => {
		const wasActiveIdInList =
			previousActiveItemIndex !== undefined && previousActiveItemIndex !== -1;
	...
	if ( ! hasData ) {
		return (
			...
			>
				{ ! hasData && <p>{ isLoading ? <Spinner /> : __( 'No results' ) }</p> }
			</div>
		);
	}
