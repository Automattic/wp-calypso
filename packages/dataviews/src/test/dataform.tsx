...
describe( 'DataForm component', () => {
	describe( 'in regular mode', () => {
		it( 'should display fields', () => {
			render( <Dataform onChange={ noop } fields={ fields } form={ form } data={ data } /> );
		...
		it( 'should render combined fields correctly', async () => {
			...
			expect( screen.getByText( "Title and author's name" ) ).toBeInTheDocument();
		} );
	} );
	...
	describe( 'in panel mode', () => {
		...
		it( 'should display fields', async () => {
			render(
				<Dataform onChange={ noop } fields={ fields } form={ formPanelMode } data={ data } />
			);
