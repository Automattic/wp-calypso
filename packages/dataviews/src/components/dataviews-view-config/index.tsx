...
function ViewTypeMenu( { defaultLayouts = { list: {}, grid: {}, table: {} } }: ViewTypeMenuProps ) {
	...
	return (
		<Menu>
			<Menu.TriggerButton
				render={ <Button size="compact" icon={ activeView?.icon } label={ __( 'Layout' ) } /> }
			/>
			<Menu.Popover>
	...
	return (
		<ToggleGroupControl
			...
			onChange={ ( newItemsPerPage ) => {
				const newItemsPerPageNumber =
					typeof newItemsPerPage === 'number' || newItemsPerPage === undefined
						? newItemsPerPage
		...
		>
			{ PAGE_SIZE_VALUES.map( ( value ) => {
				return (
					<ToggleGroupControlOption key={ value } value={ value } label={ value.toString() } />
				);
			} ) }
		</ToggleGroupControl>
	);
...
function FieldItem( {
	...
	const [ isChangingPreviewOption, setIsChangingPreviewOption ] = useState< boolean >( false );
		...
		<Item>
			...
			>
				<span className="dataviews-field-control__icon">
					{ ! canMove && ! field.enableHiding && <Icon icon={ lock } /> }
				</span>
				<span className="dataviews-field-control__label-sub-label-container">
					<span className="dataviews-field-control__label">{ label || field.label }</span>
					{ description && (
						<span className="dataviews-field-control__sub-label">{ description }</span>
					) }
				</span>
				<HStack justify="flex-end" expanded={ false } className="dataviews-field-control__actions">
					{ isVisible && (
						<>
							<Button
...
function FieldControl() {
	...
	const hiddenFields = fields.filter(
		( f ) =>
			! visibleFieldIds.includes( f.id ) && ! togglableFields.includes( f.id ) && f.type !== 'media'
	);
	...
	return (
		<Grid columns={ 12 } className="dataviews-settings-section" gap={ 4 }>
			<div className="dataviews-settings-section__sidebar">
				<Heading level={ 2 } className="dataviews-settings-section__title">
					{ title }
				</Heading>
				{ description && (
					<Text variant="muted" className="dataviews-settings-section__description">
						{ description }
					</Text>
				) }
			</div>
			<Grid columns={ 8 } gap={ 4 } className="dataviews-settings-section__content">
				{ children }
			</Grid>
		</Grid>
	);
				...
				>
					<VStack className="dataviews-view-config" spacing={ 6 }>
						<SettingsSection title={ __( 'Appearance' ) }>
							...
							{ !! activeLayout?.viewConfigOptions && <activeLayout.viewConfigOptions /> }
							<ItemsPerPageControl />
						</SettingsSection>
