...
interface ActionWithModalProps< Item > {
	...
	items: Item[];
	ActionTriggerComponent: ( props: ActionTriggerProps< Item > ) => ReactElement;
}
...
export function useHasAPossibleBulkAction< Item >( actions: Action< Item >[], item: Item ) {
	return useMemo( () => {
...
function ActionTrigger< Item >( { action, onClick, isBusy, items }: ActionTriggerProps< Item > ) {
...
function renderFooterContent< Item >(
	...
	const message =
		selectedItems.length > 0
			? sprintf(
					...
					_n( '%d Item selected', '%d Items selected', selectedItems.length ),
	...
	return (
		<HStack expanded={ false } className="dataviews-bulk-actions-footer__container" spacing={ 3 }>
			<BulkSelectionCheckbox
				...
				getItemId={ getItemId }
			/>
			<span className="dataviews-bulk-actions-footer__item-count">{ message }</span>
			<HStack
