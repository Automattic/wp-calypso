/**
 * @file Tests the BrowserHelper functions using process.env and stubbed config file.
...
describe( `Test: getViewportName`, function () {
	...
	test( 'Returns default value found in config if environment variable not set', async function () {
...
describe( `Test: getViewportSize`, function () {
	...
	`(
		...
		async function ( { env_var, expected_width, expected_height } ) {
			process.env.VIEWPORT_NAME = env_var;
	...
	test.each`
		...
		${ 'desktop_variant' }
		${ 'm0bile' }
	`( 'Throws error when environment variable is set to $env_var', async function ( { env_var } ) {
		process.env.VIEWPORT_NAME = env_var;
		expect( () => getViewportSize() ).toThrow();
	} );

	test( 'Returns default value based on config if environment variable not set', async function () {
		delete process.env.VIEWPORT_NAME;
