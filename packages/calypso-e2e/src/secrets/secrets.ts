...
function initializeSecrets(): Secrets {
	...
	const parsedSecrets = parseSecretsFile( path.join( __dirname, SECRETS_FILE_NAME ) );
	const fakeReferenceSecrets = createFakeSecretsForValidation();
	validateSecrets( parsedSecrets, fakeReferenceSecrets );
 ...
 */
function parseSecretsFile( filePath: string ): any {
	try {
 ...
 */
function validateSecrets( parsedSecrets: any, fakeReferenceSecrets: Secrets ): void {
 ...
 */
function createFakeSecretsForValidation(): Secrets {
	const fakeAccount = {
		username: 'FAKE_VALUE',
		password: 'FAKE_VALUE',
	};
