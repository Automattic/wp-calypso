...
- **Checkpoint domains land with their abilities** — `utils/checkpoints.ts` snapshots and restores only the migrated domains (global styles, the site logo, the site title, pages, navigation and site metadata today; blocks arrive with `apply-block-edits`); a migrating ability that writes checkpoints brings its domain's snapshot/restore along, with scoped keys, and takes its checkpoint through `withCheckpoint()` (`utils/checkpoints.ts`), which keys it by tool call, keeps the first snapshot per call, drops it when the write throws, and skips it entirely when the write claims no keys. Every capture is key-gated, so a checkpoint holds only what the write declared, and a keyless one is skipped rather than offering an undo that does nothing. A domain captured the moment its key is claimed (global styles, the logo, the site title and metadata) also gates its restore on that key and throws when the snapshot is missing, since absence means the capture failed. The page and navigation domains are claimed up front but captured only if the write reaches them, so an empty snapshot is valid there and the restore gates on the snapshot instead. A write that applies a batch keeps its checkpoint when it partly succeeded, by reporting the failure instead of throwing — the writes that landed are still in place, and the snapshot is the only way back past them. A domain the write only discovers as it runs (which page was renamed, which menu followed it) is captured through the `CheckpointRecorder` `withCheckpoint()` hands to the write, rather than up front. A restore records a reciprocal so the change can be redone; a domain added to the engine has to be carried into that reciprocal too, or the redo claims it and puts nothing back. Until every writer migrates, provider-held checkpoints stay restorable through the `provider-checkpoints` bridge, which shrinks away per ability.
