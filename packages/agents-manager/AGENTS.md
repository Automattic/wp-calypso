...
- **Checkpoint domains land with their abilities** — `utils/checkpoints.ts` snapshots and restores only the migrated domains (global styles, the site logo, the site title, pages, navigation and site metadata today; blocks arrive with `apply-block-edits`); a migrating ability that writes checkpoints brings its domain's snapshot/restore along, with scoped keys, and takes its checkpoint through `withCheckpoint()` (`utils/checkpoints.ts`), which keys it by tool call, keeps the first snapshot per call, drops it when the write throws, and skips it entirely when the write claims no keys. Every snapshot and every restore is key-gated, so a checkpoint captures and puts back only what the write declared: a keyless one would offer an undo that does nothing, and a claimed domain whose snapshot is missing throws rather than skipping silently. A write that applies a batch keeps its checkpoint when it partly succeeded, by reporting the failure instead of throwing — the writes that landed are still in place, and the snapshot is the only way back past them. A domain the write only discovers as it runs (which page was renamed, which menu followed it) is captured through the `CheckpointRecorder` `withCheckpoint()` hands to the write, rather than up front. Until every writer migrates, provider-held checkpoints stay restorable through the `provider-checkpoints` bridge, which shrinks away per ability.
