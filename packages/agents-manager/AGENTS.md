...
- **Test both implementations with `?am_abilities=0`** — the switch flips execution, registration, and rendering of the migrated editor abilities to the provider copies in one move. Abilities with no provider copy stay on: the all-surface ones, and the AM-only editor abilities listed in `AM_ONLY_EDITOR_ABILITIES` (`abilities/editor-abilities.ts`). The switch is read once per page load, so put it in the URL you load; the site editor's router drops it from the address bar on the first navigation, but the decision holds for the session.
...
- **Validate the arguments in the callback** — `executeAbilityFromList` hands the raw wire arguments straight to `ability.callback( args )`, so the `input_schema` polices nothing at runtime: a stringified `"false"` reads as truthy, and a non-string value can reach `message`, which the output schema types as a string. Normalize what you use (`typeof summary === 'string' && summary.trim()`, strict `=== true` for booleans), as `editor-navigate`, `show-component` and `set-site-logo` do. An empty object the model sent arrives as `[]` after the backend's PHP round-trip, so treat an empty array where an object belongs as absent, as `apply-block-edits` does for `attributes`.
- **An ability that needs React context gets a bridge, not a hook** — callbacks are plain functions, so anything only reachable from a hook (the site editor's router history, for `editor-navigate` and `edit-entity-record`) is published into a module by a small component rendered on that surface, and read back by the callback. `components/editor-history-bridge` + `utils/editor-history.ts` are the pattern; keep the component lazy so its imports stay off the shared chat path. The reverse also holds: `stream-page-design` paints while a tool call streams, so its renderer is a hook mounted by `components/page-design-renderer` from the dock, outside the chat (which unmounts on close), fed by a transport (`abilities/stream-page-design/stream.ts`) that runs in the shared chat path and imports nothing from the editor. Sync views the shared path needs from the editor chunk (`getAmCheckpointContext`, `getAmPageContentMarkup`, `getAmPageStructure`, `getAmCheckpointActions`) go through the facade.
- **Whoever hands out block ids resolves them** — the agent addresses blocks by the ids the client context gave it, so AM builds `currentPageContent` and `selectedBlockClientId` itself (`utils/page-structure.ts`, in the shape the backend parses) under short ids it mints (`utils/block-ids.ts`). An ability that takes block ids resolves them with `resolveClientId()`, which passes a clientId through as it is: `get-block-tree`, WebMCP and the Jetpack AI sidebar hand those out, and the sidebar's own structure is left in place. The feed sits behind the same gate as the abilities, so `?am_abilities=0` hands ids and resolver back to the provider together.
- **Checkpoint domains land with their abilities** — `utils/checkpoints.ts` snapshots and restores only the migrated domains (global styles, custom CSS, the site logo, the site title, pages, navigation, site metadata and the page's blocks today). A migrating ability that writes checkpoints brings its domain's snapshot/restore along, with scoped keys, and takes its checkpoint through `withCheckpoint()` — or, where the write is not a callback, through `setCheckpoint()` at the last moment the state is still what the user had, guarded by `hasCheckpoint()` since `setCheckpoint()` replaces, as the page-design host does before its first frame:
  - Keyed by tool call. The first snapshot per call wins: a write that throws drops the checkpoint it created, a repeat keeps the first run's and snapshots afresh only a domain the first run dropped, and a write claiming no keys skips it — an undo that does nothing is never offered.
  - Every capture and restore is key-gated, so a checkpoint holds only what the write declared.
  - A domain captured when its key is claimed (global styles, custom CSS, the logo, the site title and metadata, the page's blocks) refuses the write when the snapshot cannot be taken (through `withCheckpoint()`; the page-design host resolves the root before it calls `setCheckpoint()`), and a restore when it is missing: absence means the capture failed. The blocks snapshot records its root by kind, the template parts beside it and the post it came from; a restore resolves the root again, refuses another page or view (recorded as `editor_scope_mismatch`) or a missing part, writes only the parts that differ, and leaves a part alone whose snapshot is empty while it has content (a part still loading reads as empty; recorded as `template_part_would_empty`). A saved menu's items live in its record, not the page's blocks, so an edit that reaches one captures the menu under the navigation key instead. Pages and navigation are claimed up front but captured only as the write reaches them, through the `CheckpointRecorder` it is handed, so an empty snapshot is valid there.
  - A domain a batch reaches part-way through (the site title and metadata, the page's blocks, custom CSS) is kept only when the write calls `recorder.markWritten()` for it: a batch that never changed it would otherwise offer an undo that does nothing.
  - A text edit alone can be sealed for the chat's inline Undo / Redo (`sealCheckpointForSwap()`): the swap is a tracked write, so the editor's own Undo sees it, and it refuses once the page has drifted from the state it left. The loader puts AM's store ahead of the providers' for the chat (`getAmCheckpointActions()`), never inside `mergeUseCheckpointHooks`, whose `setCheckpoint` drops metadata.
  - A batch write keeps its checkpoint after a partial success by returning the failure instead of throwing: the writes that landed are still in place, and the snapshot is the only way back past them.
  - A restore records a reciprocal so the change can be redone, and refuses to restore when it cannot. A domain added to the engine has to be carried into that reciprocal too, or the redo claims it and puts nothing back. A redo naming the undone change runs from the reciprocal, and the agent's list shows a reciprocal under the intent that created it, as the backend's redo rule expects.
  - Until every writer migrates, provider-held checkpoints stay restorable through the `provider-checkpoints` bridge, which shrinks away per ability.
