...
- **Checkpoint domains land with their abilities** — `utils/checkpoints.ts` snapshots and restores only the migrated domains (global styles, the site logo, the site title, pages, navigation and site metadata today; blocks arrive with `apply-block-edits`). A migrating ability that writes checkpoints brings its domain's snapshot/restore along, with scoped keys, and takes its checkpoint through `withCheckpoint()`:
  - Keyed by tool call. The first snapshot per call wins, a write that throws drops it, and a write claiming no keys skips it — an undo that does nothing is never offered.
  ...
  - A restore records a reciprocal so the change can be redone. A domain added to the engine has to be carried into that reciprocal too, or the redo claims it and puts nothing back.
