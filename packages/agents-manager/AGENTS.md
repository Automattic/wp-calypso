...
- **Checkpoint domains land with their abilities** — `utils/checkpoints.ts` snapshots and restores only the migrated domains (global styles, the site logo, the site title, pages, navigation, site metadata and the page's blocks today). A migrating ability that writes checkpoints brings its domain's snapshot/restore along, with scoped keys, and takes its checkpoint through `withCheckpoint()` — or, where the write is not a callback, through `setCheckpoint()` at the last moment the state is still what the user had, guarded by `hasCheckpoint()` since `setCheckpoint()` replaces, as the page-design host does before its first frame:
  ...
  - A domain captured when its key is claimed (global styles, the logo, the site title and metadata, the page's blocks) refuses the write when the snapshot cannot be taken (through `withCheckpoint()`; the page-design host resolves the root before it calls `setCheckpoint()`), and a restore when it is missing: absence means the capture failed. The blocks snapshot records its root by kind and the post it came from, and a restore resolves the root again and refuses another page or view. Pages and navigation are claimed up front but captured only as the write reaches them, through the `CheckpointRecorder` it is handed, so an empty snapshot is valid there.
