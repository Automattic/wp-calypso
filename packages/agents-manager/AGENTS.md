...
- **Migrating an ability = a folder under `src/abilities/` + an `EDITOR_ABILITIES` entry in `abilities/editor-abilities.ts`** — `amToolProvider` then executes it ahead of the provider's copy. Build results with `successResult()` / `errorResult()` from `abilities/ability-result.ts`; the backend acks echoing tools straight from that envelope, so its shape is a contract, not a local choice. If it renders a chat component, also add its type to `AM_COMPONENTS` in the converter, wrapped with the `lazyComponent()` helper (`utils/lazy-component.ts`).
