...
- **Checkpoint domains land with their abilities** — `utils/checkpoints.ts` snapshots and restores only the migrated domains (global styles and the site logo today); a migrating ability that writes checkpoints brings its domain's snapshot/restore along, with scoped keys, and takes its checkpoint through `withCheckpoint()` (`utils/checkpoints.ts`), which keys it by tool call, keeps the first snapshot per call, and drops it when the write throws. A restore records a reciprocal so the change can be redone; a redo naming the undone change runs from it, and the agent's list shows a reciprocal under the intent that created it, as the backend's redo rule expects. Until every writer migrates, provider-held checkpoints stay restorable through the `provider-checkpoints` bridge, which shrinks away per ability.
