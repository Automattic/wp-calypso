...
- **Checkpoint domains land with their abilities** — `utils/checkpoints.ts` snapshots and restores only the migrated domains (global styles, custom CSS, the site logo, the site title, pages, navigation, site metadata and the page's blocks today). A migrating ability that writes checkpoints brings its domain's snapshot/restore along, with scoped keys, and takes its checkpoint through `withCheckpoint()` — or, where the write is not a callback, through `setCheckpoint()` at the last moment the state is still what the user had, guarded by `hasCheckpoint()` since `setCheckpoint()` replaces, as the page-design host does before its first frame:
  ...
  - Every checkpoint an AM ability writes lives in AM's store, and the loader advertises them as `availableCheckpoints` from the same editor-page context that describes the page's blocks.
