...
- **Checkpoint domains land with their abilities** — `utils/checkpoints.ts` snapshots and restores only the migrated domains (global styles, the site logo, the site title, pages, navigation, site metadata and the post title and excerpt today; blocks arrive with `apply-block-edits`). A migrating ability that writes checkpoints brings its domain's snapshot/restore along, with scoped keys, and takes its checkpoint through `withCheckpoint()`:
