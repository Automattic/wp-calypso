...
describe( 'OrchestratorChat', () => {
	...
	it( 'keeps an unrelated current error visible beside a latched provider notice', () => {
		const notice = {
			message: 'You’re out of free credits.',
			status: 'error' as const,
			dismissible: false,
		...
		const useChatNotice = jest.fn( () => notice );
		mockUseAgentChat.mockReturnValue( agentChatReturn( { error: 'Network request failed' } ) );

		render( chat( { useChatNotice } ) );

		expect( mockAgentChat.mock.calls.at( -1 )![ 0 ] ).toEqual(
			expect.objectContaining( { notice, error: 'Network request failed' } )
		);
	} );

	it( 'does not expose the provider notice on Reader Chat', () => {
