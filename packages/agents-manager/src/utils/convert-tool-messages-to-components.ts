...
export default function convertToolMessagesToComponents( {
	...
	return messages.flatMap( ( message, index, array ) => {
		...
		if ( isShowComponentTool( textData.tool_id ) ) {
			...
			// TODO(am-migration): Remove once Big Sky drops its pattern-picker chat component; the
		...
		// TODO(am-migration): Double-check whether this branch is still needed when the
