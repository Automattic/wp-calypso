...
export function initializeAnalytics(currentUser: any | undefined, superProps: any): Promise<void> {
	// Update super props.
...
export function recordTracksPageView(urlPath: string, params: any) {
	...
	if (typeof window !== 'undefined' && window.location) {
		...
		const utmParamEntries =
			urlParams && Array.from(urlParams.entries()).filter(([key]) => key.startsWith('utm_'));
