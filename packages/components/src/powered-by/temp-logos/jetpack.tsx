...
type JetpackLogoProps = CommonLogoProps &
...
const LogoMarkTriangle = ( { fill }: Pick< JetpackLogoProps, 'fill' > ) => {
	return (
		<path
			fill={ fill }
			fillRule="evenodd"
...
const LogoMarkCircle = ( { fill, mask }: Pick< JetpackLogoProps, 'fill' | 'mask' > ) => {
	return <circle cx="50" cy="50" r="50" fill={ fill } mask={ mask } />;
};

const LogoMark = ( { fill }: Pick< JetpackLogoProps, 'fill' > ) => {
	const isMasked = fill === 'currentColor';
	return (
		<>
			{ /* In the monochrome version: the triangles are masked out. In the
			color-branded version, the triangles are hardcoded white. */ }
			<LogoMarkCircle fill={ fill } mask={ isMasked ? 'url(#cutout)' : undefined } />
			{ isMasked ? (
				<defs>
					<mask id="cutout">
...
const LogoText = ( { fill = 'currentColor' }: Pick< JetpackLogoProps, 'fill' > ) => {
	return (
		<path
			fill={ fill }
...
export const JetpackLogo = forwardRef< SVGSVGElement, JetpackLogoProps >( function JetpackLogo(
	{ variant = 'full', monochrome = false, theme = 'default', height = 32, className, ...props },
	ref
) {
	const titleId = useId();
	...
	return (
		<svg
			...
			viewBox={ `0 0 ${ variant === 'full' ? 368 : 100 } ${ 100 }` }
			height={ height }
			className={ clsx( 'a8c-components-jetpack-logo', className ) }
			aria-labelledby={ titleId }
			{ ...props }
			ref={ ref }
		>
			<title id={ titleId }>Jetpack</title>
			<LogoMark fill={ brandFill } />
			{ variant === 'full' ? <LogoText fill={ textFill } /> : null }
		</svg>
	);
} );
