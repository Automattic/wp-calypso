...
describe( 'useUndoSnapshots', () => {
	...
	it( 'reverts a retained snapshot even when none is required', () => {
		const { result } = renderHook( () => useUndoSnapshots< string >() );
		result.current.saveFromApplyResult( 'a', {
			success: true,
			clientId: 'b1',
		...
		expect( result.current.undo( 'a', false ) ).toBe( 'success' );
		expect( mockUndoBlockEdit ).toHaveBeenCalledWith( 'b1', 'x', 'y', undefined );
	} );
