...
- `isCompleteCallback?: () => boolean | Promise<boolean>`. Used to determine if a step is complete for purposes of validation. Default is a function returning false. If the validation requires an async processs, the callback may return a Promise that resolves with either true or false as its value.
