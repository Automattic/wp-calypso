...
describe( 'Client', () => {
	...
	describe( 'Running state tool execution', () => {
		...
		it( 'should not execute an ability from a running status echo', async () => {
			// A `running` status is an informational echo of a tool call the
			// server is working on, carrying the raw arguments. Abilities must
			// only run on the `input-required` handoff — dispatching them here
			// applies the call twice, the first time unvalidated, and throws
			// the result away so the agent never hears about a failure.
			const abilityCalls: any[] = [];
			const ability = {
			...
			const mockToolProvider: ToolProvider = {
				...
				async executeAbility() {
					throw new Error( 'Should not call executeAbility' );
				},
			};
			...
			const updates = [];
			for await ( const update of client.sendMessageStream( {
				message: createTextMessage( 'Make the heading bigger' ),
			} ) ) {
				updates.push( update );
			}
		...
		it( 'should execute only advertised tools when a running message also carries an ability call', async () => {
			// The Agents Manager merges several providers into one, so a single
			// provider can expose both advertised tools and abilities. A tool
			// match must not drag an ability in the same message along with it.
			const executedTools: Array< { toolId: string; args: any } > = [];
			const abilityCalls: any[] = [];
			const mockToolProvider: ToolProvider = {
				async getAvailableTools() {
			...
			expect( abilityCalls ).toHaveLength( 0 );
			expect( executedTools ).toHaveLength( 1 );
			expect( executedTools[ 0 ].toolId ).toBe( 'set_processing_state' );
			// Original running state plus the tool execution marker, and the
	...
	describe( 'Dispatchable (non-advertised) tools', () => {
		...
		it( 'retains server search results in the renderer continuation without replaying running echoes', async () => {
			const rendererId = 'wpcom__render_plugin_recommendations';
			const searchCall = {
				type: 'data',
				data: { toolCallId: 'search', toolId: 'wpcom__plugin_search', arguments: { query: 'SEO' } },
			};
			const searchResult = {
				type: 'data',
				data: {
					toolCallId: 'search',
					toolId: 'wpcom__plugin_search',
					result: {
						results: [
							{
								slug: 'seo-by-rank-math',
								url: 'https://wordpress.com/plugins/seo-by-rank-math/example.com?wp-agent-chat=session-dispatchable',
							},
						],
					},
				},
			};
			const renderEvent = JSON.parse( inputRequiredEventFor( rendererId ) );
			const runningEvent = ( part: unknown ) =>
				JSON.stringify( {
					...renderEvent,
					result: {
						...renderEvent.result,
			...
			const events = [
				runningEvent( searchCall ),
				runningEvent( searchResult ),
				runningEvent( searchResult ),
				runningEvent( renderEvent.result.status.message.parts[ 0 ] ),
				JSON.stringify( renderEvent ),
			];
			mockFetch.mockResolvedValueOnce( mockSSEResponse( events.join( '\n\ndata: ' ) ) );
			mockFetch.mockResolvedValueOnce( mockSSEResponse( completionEvent ) );
			const executeTool = vi
				.fn()
				.mockResolvedValue( { result: { success: true }, returnToAgent: true } );
			const client = createClient( {
				agentId: 'test-agent',
				toolProvider: {
					getDispatchableTools: async () => [ { ...dispatchableTool, id: rendererId } ],
					executeTool,
				},
			} );
			await sendMessageAndWait( client, { message: createTextMessage( 'I need a seo plugin' ) } );
			const { parts } = JSON.parse( mockFetch.mock.calls[ 1 ][ 1 ].body ).params.message;
			const interactions = parts.filter(
				( part: { data?: { toolCallId?: string } } ) => part.data?.toolCallId
			);
			expect( interactions ).toHaveLength( 4 );
			expect( interactions.slice( 0, 2 ) ).toEqual( [ searchCall, searchResult ] );
			expect( interactions[ 2 ] ).toEqual( renderEvent.result.status.message.parts[ 0 ] );
