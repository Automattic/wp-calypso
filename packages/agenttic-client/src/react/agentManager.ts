...
interface ManagedAgent {
	...
	/** User turn awaiting delivery resolution across tool follow-ups. */
	inFlightUserMessageId: string | null;
}
...
function createAgentManager(): AgentManager {
	...
	return {
		...
		async *sendMessageStream(
			...
			// mark a new pending delivery for real user turns that persist.
			if ( withHistory && ! isToolResultOnlyMessage( userMessage ) ) {
			...
			// exists. Internal only — do not notify consumers or write
			// sessionIdStorageKey; those APIs mean a real server id.
			// conversationStorageKey (if set) still owns the storage name.
			if ( withHistory && ! managedAgent.sessionId ) {
				managedAgent.sessionId = `local-${ generateMessageId() }`;
			}
