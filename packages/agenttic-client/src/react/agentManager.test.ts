...
import { getAgentManager, type AgentManager } from './agentManager';
...
describe('agentManager', () => {
	...
	describe('sendMessage', () => {
		...
		it('should handle withHistory=false option', async () => {
			await agentManager.sendMessage('test-key', 'Hello', { withHistory: false });
		...
		it('should update conversation history when withHistory=true', async () => {
			...
			const configWithSession = { ...testConfig, sessionId: 'test-session' };
		...
		it('should use custom message when provided', async () => {
			...
			await agentManager.sendMessage('test-key', 'Hello', { message: customMessage });
	...
	describe('sendMessageStream', () => {
		...
		it('should stream messages from agent', async () => {
			const mockUpdates: TaskUpdate[] = [
				{ id: 'task-123', status: { state: 'working', message: mockAgentMessage }, final: false, text: 'Hello back!' },
				{ id: 'task-123', status: { state: 'completed', message: mockAgentMessage }, final: true, text: 'Hello back!' },
			];
		...
		it('should handle tool calls during streaming', async () => {
			const toolCallUpdate: TaskUpdate = {
				id: 'task-123',
				status: { state: 'input-required', message: mockToolCallMessage },
				final: false,
				text: '',
			};
...
describe('agentManager helper functions', () => {
	...
	describe('conversationMessagesToDataParts logic', () => {
		it('should convert text messages to history data parts', () => {
			...
			expect(historyParts[0].data).toEqual({ role: 'user', text: 'Hello' });
			expect(historyParts[1].data).toEqual({ role: 'agent', text: 'Hi there' });
		});
