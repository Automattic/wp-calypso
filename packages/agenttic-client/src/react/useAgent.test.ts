...
import type { Message, DataPart, TextPart } from '../client/types/index';
	...
	describe('extractNewContentFromMessage logic', () => {
		...
		it('should preserve tool calls', () => {
			const toolParts = mockToolCallMessage.parts.filter((part) => {
				if (part.type === 'data') {
					return 'toolCallId' in part.data && 'arguments' in part.data;
				}
				return false;
			});
		...
		it('should preserve tool results', () => {
			const toolResultParts = mockToolResultMessage.parts.filter((part) => {
				if (part.type === 'data') {
					return 'toolCallId' in part.data && 'result' in part.data;
				}
				return false;
			});
