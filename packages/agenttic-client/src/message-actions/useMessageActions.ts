...
export function useMessageActions(): UseMessageActionsReturn & {
	...
	const [ registrations, setRegistrations ] = useState< MessageActionsRegistration[] >( [] );

	// Register a new set of message actions or update existing one
	const registerMessageActions = useCallback( ( registration: MessageActionsRegistration ) => {
		setRegistrations( ( prev ) => {
			// Check if registration with this ID already exists
			const existingIndex = prev.findIndex( ( r ) => r.id === registration.id );
