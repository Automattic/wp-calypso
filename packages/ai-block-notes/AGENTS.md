...
`@automattic/ai-block-notes` is an AI-powered block commenting system for WordPress. Users mention `@ai` in block notes to get contextual AI assistance. A headless wp-orchestrator agent receives block, post, and thread context, then responds by creating new notes via the WordPress Abilities API.

## Architecture

### State Management

Minimal state lives in the WordPress data store `ai-block-notes/store` (`src/store/index.ts`). This store tracks:
...
- **Minimal store**: The store is intentionally small. Cross-component coordination happens via the Redux store as the bridge between the separate AI Block Notes bundle and the block editor.

### Component Hierarchy

```
AiBlockNotesSubscriptions (src/components/subscriptions/)
  -> useAgentConfig (loads async agent config)
  -> AiBlockNotesSubscriptionsChat (main orchestration)
...
2. `AiBlockNotesSubscriptionsChat` detects the new note via `useSelect` on `@wordpress/core-data`
...
| File                                            | Purpose                                                                       |
| ----------------------------------------------- | ----------------------------------------------------------------------------- |
| `src/index.tsx`                                 | Entry point (`initAiBlockNotes` function)                                     |
...
| `src/utils/feature-flag.ts`                     | Feature flag check (`window.aiBlockNotesData`, with a legacy-reader fallback) |
...
- **Feature flag**: `window.aiBlockNotesData.enabled`, set by PHP enqueue. `window.blockNotesData` is a temporary compatibility reader for older Jetpack versions. Checked via `isAiBlockNotesEnabled()` in `src/utils/feature-flag.ts`.
...
4. Run `yarn workspace @automattic/ai-block-notes tsc --build --dry` to verify types compile
...
| ---------- | ----------------------------------------------------------------------------------- | -------------- |
| Unit tests | `yarn jest packages/ai-block-notes --config packages/ai-block-notes/jest.config.js` | No             |
| Type check | `yarn workspace @automattic/ai-block-notes tsc --build --dry`                       | No             |
...
AI Block Notes is bundled in `agents-manager` and served from `widgets.wp.com`. There is no local dev server -- **visual testing requires a sandbox**.
...
# Type check
yarn workspace @automattic/ai-block-notes tsc --build --dry

# Run tests (from repo root)
yarn jest packages/ai-block-notes --config packages/ai-block-notes/jest.config.js

# Deploy to sandbox (builds AI Block Notes as part of the Agents Manager bundle)
...
Deployed as part of the `agents-manager` bundle to `widgets.wp.com`. The entry point is `apps/agents-manager/ai-block-notes.js`, which is configured as a separate webpack entry in `apps/agents-manager/webpack.config.js`. PHP in Jetpack plugins enqueues the script on relevant admin screens.

`@wordpress/abilities` is bundled in for self-hosted compatibility.

The generated files intentionally remain `block-notes.min.js` and `block-notes.asset.json`
while Jetpack 16.1 is supported. Treat that basename, `big-sky/block-notes`,
`wp-block-notes`, `blockNotes` client context, `bigsky_ai_processed_date`, and the
existing Tracks event names as compatibility contracts rather than source nomenclature.
...
- **Cross-bundle communication**: AI Block Notes runs in a separate webpack bundle from the block editor. The `ai-block-notes/store` WordPress data store is the bridge. Do not assume direct component access to block editor internals.
