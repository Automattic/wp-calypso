/**
 * Smoke tests for AiBlockNotesSubscriptions component
 *
...
import AiBlockNotesSubscriptions from './index';
...
jest.mock( '@wordpress/data', () => ( {
	useSelect: jest.fn( ( callback: any ) => {
		const select = ( storeName: string ) => {
			...
			if ( storeName === 'ai-block-notes/store' ) {
				return {
...
// Mock AI Block Notes agent config.
jest.mock( '../../agent-config', () => ( {
	aiBlockNotesAgentConfig: {
...
describe( 'AiBlockNotesSubscriptions', () => {
	...
	describe( 'Component Initialization', () => {
		it( 'renders component', async () => {
			let rendered;
			await act( async () => {
				rendered = render( <AiBlockNotesSubscriptions /> );
			} );
	...
	describe( 'AI Mention Detection', () => {
		it( 'detects and processes notes with @ai mentions', async () => {
			...
			render( <AiBlockNotesSubscriptions /> );
