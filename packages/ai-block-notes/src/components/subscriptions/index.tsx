...
import { aiBlockNotesAgentConfig } from '../../agent-config';
...
const markNoteAsProcessed = async (
	...
	try {
		...
		window.console?.error( `AI Block Notes: Error marking note ${ noteId } as processed:`, error );
		...
		processStaleNote()
			...
			.catch( ( error ) => {
				window.console?.error(
					`AI Block Notes: Error processing stale note ID ${ note.note_ID }:`,
					error
				);
				removeInProcessBlockNote( note.note_ID, note.note_post_ID );
			} );
...
const isNoteStale = ( note: NoteEntity ): boolean => {
	...
	try {
		...
		if ( isNaN( noteDate.getTime() ) ) {
			window.console?.error( 'AI Block Notes: Invalid date_gmt format:', note.date_gmt );
			return false;
		}
		...
		window.console?.error( 'AI Block Notes: Error parsing note date_gmt:', error );
		return false;
	}
};
...
const processAiNote = async (
	...
	try {
		...
		window.console?.error(
			`AI Block Notes: Error submitting note id ${ note.note_ID } to agent with error ${ error }.`
		);
		trackBlockNoteAiResponseFailed( {
			noteId: note.note_ID,
		...
		window.console?.info( `AI Block Notes: Marking note ID ${ note.note_ID } as processed.` );
...
/**
 * AiBlockNotesSubscriptionsChat Component
 *
 ...
 */
function AiBlockNotesSubscriptionsChat( {
	agentConfigState,
}: {
	...
	const { currentUserId, blockNotes: blockNotes } = useSelect(
		( select ) => {
			...
			if ( ! currentPostId || ! userId || ! isAiBlockNotesEnabled() ) {
			...
			messagesAddressedToAi.forEach( async ( note: NoteEntity ) => {
				...
				processAiNote(
					...
					.catch( ( error ) => {
						// Clean up on error
						window.console?.error(
							`AI Block Notes: Error in async processing for note ID ${ transformedNote.note_ID }:`,
							error
						);
						removeInProcessBlockNote( transformedNote.note_ID, transformedNote.note_post_ID );
					} );
...
function AiBlockNotesSubscriptions() {
	const agentConfigState = useAgentConfig( aiBlockNotesAgentConfig );

	if ( ! agentConfigState ) {
		return null;
	}

	return <AiBlockNotesSubscriptionsChat agentConfigState={ agentConfigState } />;
}

export default AiBlockNotesSubscriptions;
