 ...
 * - AI monitoring: src/ai-block-notes/components/subscriptions/
 */
...
export const getBlockNotes = async (
	...
	try {
		...
		window.console?.error( 'AI Block Notes: API error:', {
...
export const replyToNote = async (
	...
	try {
		...
		window.console?.error( 'AI Block Notes: Error in replyToNote:', error );
		throw error;
	}
};
