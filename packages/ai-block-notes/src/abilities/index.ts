...
/**
 * AI Block Notes ability callback type.
 */
export type AiBlockNotesCallback = ( args: {
	operation: 'get' | 'reply';
	blockNoteId: number;
	notes?: string;
	summary?: string;
...
/**
 * Register the AI Block Notes ability with the WordPress Abilities API.
 */
export async function registerAiBlockNotesAbility(): Promise< void > {
	...
	try {
		...
		await registerAbility( {
			...
			callback: async ( input ) => {
				...
				try {
					postId = ( select( editorStore ).getCurrentPostId() as number | null ) ?? undefined;
					...
					window.console?.error( 'AI Block Notes: Ability execution failed', error );
					return {
						success: false,
