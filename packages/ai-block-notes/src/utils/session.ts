...
export async function getBlockNoteThreadSessionId(
	...
	if ( ! blogId || ! postId || ! noteId ) {
		window.console?.warn( 'AI Block Notes: Missing required parameters for session ID generation', {
			blogId: !! blogId,
			postId: !! postId,
			noteId: !! noteId,
		} );
	...
	try {
		...
		window.console?.error(
			'AI Block Notes: Failed to generate session ID - crypto operation failed',
			error
		);
		return undefined;
	}
