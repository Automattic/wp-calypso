...
- Resolve Node to the CommonJS build via a `node` condition. The ESM output uses extensionless specifiers and the package is not `type: module`, so native ESM could not load it; bundlers still take the ESM build and keep tree-shaking.
