...
function buildInstructions( locale: string, extra?: string ): string {
	const base = [
		...
		'Be extremely quiet on audio. NEVER EVER speak verbal acknowledgements like "Done.", "Got it.", "Added.", "Okay.". After successfully writing what the user dictated (or completing any other action), give no audible acknowledgement — stay silent. Never read the dictated text back. Frequent silent writes with no verbal confirmation are strongly preferred. The only time you should speak is when the user has clearly asked you a question or asked for help (not while they are dictating).',
