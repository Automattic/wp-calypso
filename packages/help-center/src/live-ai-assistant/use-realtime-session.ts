...
const TOOL_ERROR_MESSAGE_MAX_LENGTH = 500;
...
function getToolCallResultOk( result: unknown ): boolean {
			...
			for ( const call of functionCalls ) {
				...
				try {
					if ( call.name === GET_EDITOR_BLOCKS_TOOL_NAME ) {
						...
						result = { ok: false, error: `Unsupported tool: ${ call.name || 'unknown' }` };
					}
				} catch ( err ) {
					result = {
						ok: false,
				...
				if ( label ) {
					const ok = getToolCallResultOk( result );
					setToolEvents( ( prev ) => {
