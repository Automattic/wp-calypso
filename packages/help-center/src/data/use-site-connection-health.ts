 ...
 * can be unreachable from wpcom's perspective; for Simple sites (and when the
 * wpcom APIs aren't accessible, e.g. inside wp-admin of the affected site)
 * this reports the site as reachable.
 *
...
export function useSiteConnectionHealth( enabled = true ) {
	...
	const { data, isLoading } = useQuery< ConnectionHealth, Error >( {
		...
		enabled: enabled && canBeUnreachable && Boolean( siteId ) && canAccessWpcomApis(),
