...
describe( 'Data utilities', () => {
	...
	it( 'parses CSS selectors', () => {
		...
		expect(
			parseSimpleCssSelector( 'a[href^="https://syndication.optimizesrv.com/splash.php?"]' )
		).toEqual( [ 'a', { href: [ 'https://syndication.optimizesrv.com/splash.php?' ] } ] );
		expect( parseSimpleCssSelector( ' img[width="460"][height=60 i]' ) ).toEqual( [
			'img',
			{ width: [ '460' ], height: [ '60' ] },
		] );
		expect(
			parseSimpleCssSelector( 'a[href^="https://www.adultempire.com/"][href*="?partner_id="]' )
		).toEqual( [ 'a', { href: [ 'https://www.adultempire.com/', '?partner_id=' ] } ] );
		expect( parseSimpleCssSelector( '.mainmenu[style="padding:10px 0 0 0 !important;"]' ) ).toEqual(
			[ undefined, { class: [ 'mainmenu' ], style: [ 'padding:10px 0 0 0 !important;' ] } ]
		);
	} );
	...
	it( 'gets maximum in iterator', () => {
		...
		expect(
			maxInIterator( generator(), ( item ) => ( item.val % 2 === 0 ? item.val : item.val * 2 ) )
		).toEqual( { val: 7 } );
