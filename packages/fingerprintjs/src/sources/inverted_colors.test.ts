...
describe( 'Sources', () => {
	describe( 'invertedColors', () => {
		...
		it( 'handles various cases', async () => {
			...
			await withMockMatchMedia( { 'inverted-colors': [ 'none' ] }, true, () =>
				expect( areColorsInverted() ).toBeFalse()
			);
			await withMockMatchMedia( { 'inverted-colors': [ 'inverted' ] }, true, () =>
				expect( areColorsInverted() ).toBeTrue()
			);
		} );
	} );
} );
