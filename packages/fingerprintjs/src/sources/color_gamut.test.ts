...
describe( 'Sources', () => {
	describe( 'colorGamut', () => {
		...
		it( 'handles missing browser support', async () => {
			await withMockMatchMedia( { 'color-gamut': [ undefined ] }, true, () =>
				expect( getColorGamut() ).toBeUndefined()
			);
		} );

		it( 'handles various color gamuts', async () => {
			await withMockMatchMedia( { 'color-gamut': [ 'srgb' ] }, true, () =>
				expect( getColorGamut() ).toBe( 'srgb' )
			);
			await withMockMatchMedia( { 'color-gamut': [ 'srgb', 'p3' ] }, true, () =>
