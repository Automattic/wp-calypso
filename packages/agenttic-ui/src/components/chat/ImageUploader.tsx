...
const ImagePreviewItem = memo(
	( {
		...
		const fileType = image.mime_type ? image.mime_type.split( '/' )[ 1 ].toUpperCase() : '';
		const fileName = image.title || image.name || image.url.split( '/' ).pop() || 'image';
		...
		return (
			<div
				...
				aria-label={ uploading ? __( 'Uploading image', 'a8c-agenttic' ) : undefined }
			>
				<button
				...
				<div className={ styles.thumbnail }>
					...
					{ uploading && (
						<div className={ styles.uploadingOverlay }>
							{ uploadingIndicator || <div className={ styles.spinner } /> }
						</div>
					) }
				</div>
				{ showFileMetadata && (
					<div className={ styles.previewMeta }>
						<span
							...
							title={ fileName + ( image.alt ? ` - ${ image.alt }` : '' ) }
						>
							{ fileName }
						</span>
						<span className={ styles.previewType }>{ fileType }</span>
					</div>
				) }
			</div>
		);
...
export const ImageUploader = forwardRef< ImageUploaderHandle, ImageUploaderProps >(
	function ImageUploader(
		...
		useEffect( () => {
			...
			listenTarget.addEventListener( 'dragenter', handleDragEnter as EventListener );
			listenTarget.addEventListener( 'dragleave', handleDragLeave as EventListener );
			listenTarget.addEventListener( 'drop', handleDrop );

			return () => {
				listenTarget.removeEventListener( 'dragenter', handleDragEnter as EventListener );
				listenTarget.removeEventListener( 'dragleave', handleDragLeave as EventListener );
		...
		const handleFiles = useCallback(
			( files: FileList | File[] ) => {
				...
				const validFiles = fileArray.filter( ( file ) => acceptedFileTypes.includes( file.type ) );
		...
		useEffect( () => {
			const handlePaste = ( e: ClipboardEvent ) => {
				...
				const isPasteTargetTextarea =
					textareaRef.current &&
					e.target &&
					( e.target === textareaRef.current || textareaRef.current.contains( e.target as Node ) );
		...
		const isActive = hasImages || isUploading || isDraggingFile || showInvalidFileMessage;
		...
		const invalidFileMessage = sprintf(
			/* translators: %s: allowed file types (e.g., "JPEG or PNG") */
			__( 'Only %s image files are allowed.', 'a8c-agenttic' ),
			acceptedFileTypes.map( ( type ) => type.split( '/' )[ 1 ].toUpperCase() ).join( ' or ' )
		);
		...
		return (
			...
			>
				<div
					className={ `${ styles.uploader } ${ isUploading ? styles.uploading : '' } ${
						...
						{ ! showInvalidFileMessage && (
							<>
								...
								<div
									...
									aria-label={ __( 'Click to upload images or drag and drop', 'a8c-agenttic' ) }
								>
									{ showDropMessage && (
										<div className={ styles.draggingMessage }>
											<p>
												<strong>{ __( 'Drop files here to use', 'a8c-agenttic' ) }</strong>
												<br />
									...
									{ showPreview && (
										<div className={ styles.preview }>
											{ hasImages &&
												images.map( ( image ) => (
													<ImagePreviewItem
														key={ image.id }
														image={ image }
														allowDragToInsert={ allowDragToInsert }
											...
											{ isUploading &&
												uploadingImages.map( ( { id, url, file } ) =>
													url ? (
														<ImagePreviewItem
															key={ id }
															image={ {
														...
														<div
															key={ id }
															className={ cn( styles.previewItem, styles.uploadingItem ) }
															aria-label={ __( 'Uploading image', 'a8c-agenttic' ) }
														>
															{ uploadingIndicator || (
