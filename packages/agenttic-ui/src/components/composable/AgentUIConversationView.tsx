...
export const AgentUIConversationView = forwardRef< HTMLDivElement, AgentUIConversationViewProps >(
	function AgentUIConversationView(
		{ showHeader = true, children, className }: AgentUIConversationViewProps = {},
		ref
	) {
		...
		if ( children ) {
			...
			const hasHeader = React.Children.toArray( children ).some( ( child ) => {
				return (
					...
					( child.type === AgentUIHeader ||
						( typeof child.type === 'function' && child.type.name === 'AgentUIHeader' ) )
				);
			} );
			...
			return (
				<div
					...
					className={ `${ styles.container }${ hasHeader ? ` ${ styles.withHeader }` : '' }${
						className ? ` ${ className }` : ''
					}` }
				>
					{ children }
