...
const jedFiles = import.meta.glob( '../../languages/wpcom-agenttic-*.jed.json', {
	eager: true,
} );
...
Object.entries( jedFiles ).forEach( ( [ path, module ] ) => {
	...
	const locale = filename.replace( 'wpcom-agenttic-', '' ).replace( '.jed.json', '' );

	// Jed files contain translation objects directly
	bundledTranslations[ locale ] = ( module as any ).default;
} );

export function getBundledTranslation( locale: string ): TranslationData | null {
