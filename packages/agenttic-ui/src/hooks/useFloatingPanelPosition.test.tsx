...
describe( 'useFloatingPanelPosition', () => {
	...
	describe( 'snapToSide', () => {
		it( 'overrides an animation already in flight when it places the panel', async () => {
			harness = renderHook( { freeDrag: true } );
			await harness.render();
			const result = harness.captured.current!;

			// An out-of-bounds drop animates x back into the box. A command landing
			// mid-flight must win: `set` alone would be overwritten by that spring.
			await act( async () => {
				result.x.set( window.innerWidth * 2 );
				result.handleDragEnd( null, PAN_INFO );
			} );
			expect( result.x.isAnimating() ).toBe( true );

			await act( async () => {
				result.snapToSide( 'left' );
			} );
	...
	describe( 'window resize with an animation in flight', () => {
		it( 'keeps a snapped panel at the corner as the window shrinks', async () => {
			harness = renderHook( { freeDrag: true } );
			await harness.render();
			const result = harness.captured.current!;

			await act( async () => {
				result.snapToSide( 'right' );
			} );
		...
		it( 'stops a reposition animation the new viewport has invalidated', async () => {
			harness = renderHook( { freeDrag: true } );
			await harness.render();
			const result = harness.captured.current!;

			// A grow starts a spring toward the pinned-edge target for the CURRENT
			// viewport. Shrinking the window mid-flight must not let it settle
			// there — `set` alone cannot stop it.
			await act( async () => {
				result.x.set( 800 );
				result.repositionForResize( 200 );
			} );
			...
			// The stopped spring's remaining frames must not move it.
			await act( async () => {
				await new Promise( ( resolve ) => setTimeout( resolve, 150 ) );
			} );
			expect( result.x.get() ).toBe( cornerX );
		...
		it( 'leaves the minimize animation running — it targets state, not the viewport', async () => {
			harness = renderHook( { freeDrag: true, chatState: 'expanded' } );
			await harness.render();
			const result = harness.captured.current!;
			...
			// Minimizing stashes the dragged offset and animates y to the 0 dock.
			await harness.rerender( {
				freeDrag: true,
				chatState: 'minimized',
			} );
			expect( result.y.isAnimating() ).toBe( true );
			...
			// x is re-clamped for the new viewport; y keeps animating home.
			expect( result.x.isAnimating() ).toBe( false );
			expect( result.y.isAnimating() ).toBe( true );
		} );
	} );
