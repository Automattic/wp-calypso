...
export interface UseFloatingPanelPositionResult {
	...
	// Places the panel at the given side's corner — instantly, not animated —
	// persisting like a drag-end (`onChatPositionChange` on a side change,
	// `onFreeDragEnd` in free drag).
...
export function useFloatingPanelPosition( {
	...
	const snapToSide = useCallback(
		( side: ChatPosition ) => {
			...
			// (the panel leaving an embedded slot), where sliding across the
			// viewport reads as drift. It also keeps the reported position and the
			// rendered one in step.
			x.stop();
			x.set( target.x );
			y.stop();
			y.set( targetY );
	...
	useEffect( () => {
		const handleResize = () => {
			...
			// Stop before writing: `set` alone leaves an in-flight spring running,
			// and it would settle at a target computed for the old viewport —
			// off-screen when the window shrank. Only the axes written here are
			// stopped, so the minimize/restore animations (which target state, not
			// the viewport) still finish.
			x.stop();
			x.set( target.x );
