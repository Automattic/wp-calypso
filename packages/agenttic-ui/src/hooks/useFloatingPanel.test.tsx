...
describe( 'useFloatingPanel', () => {
	...
	it( 'keeps the commanded position when a controlled-size parent echoes the reset', async () => {
		// Controlled-size consumers echo `onResizeEnd` back through `size`;
		// the echo must not move the panel off the corner the command placed it at.
		const args: Args = {
			freeDrag: true,
			resizable: true,
		...
		// The command places x at the corner for the reset size, not the mid-flight
		// 600 width; the echo must leave it there.
		const cornerX = window.innerWidth - 372 - 16 - 16;
		expect( harness.captured.current?.x.get() ).toBe( cornerX );
	} );
} );
