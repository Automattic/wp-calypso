...
const useWpcomSimpleSiteCommands = ( { navigate, currentRoute }: useCommandsParams ) => {
	const { __, _x } = useI18n();
	...
	const clearCacheCommand = {
		...
		callback: commandNavigation( `/hosting-config` ),
		icon: cacheIcon,
	};

	return [ viewMySitesCommand, openDashboardCommand, clearCacheCommand ];
};

export default useWpcomSimpleSiteCommands;
