...
export interface ImageStudioState {
	...
	// When true, the "Generated by AI" label is hidden for this image (default: shown).
	aiDisclosureHidden: boolean;
...
type SetAiDisclosureHiddenAction = {
	type: 'SET_AI_DISCLOSURE_HIDDEN';
	payload: boolean;
};
...
/**
 * Key for the remembered "Generated by AI" disclosure preference.
 */
const AI_DISCLOSURE_HIDDEN_STORAGE_KEY = 'big-sky-image-studio-ai-disclosure-hidden';

/**
 * Get the remembered disclosure preference. Defaults to hidden when no
 * preference has been stored yet.
 */
const getAiDisclosureHiddenFromLocalStorage = (): boolean => {
	try {
		const stored = localStorage.getItem( AI_DISCLOSURE_HIDDEN_STORAGE_KEY );
...
const initialState: ImageStudioState = {
	...
	aiDisclosureHidden: getAiDisclosureHiddenFromLocalStorage(),
	canvasMetadata: null,
	...
	switch ( action.type ) {
		case 'OPEN_IMAGE_STUDIO':
			return {
				...
				isSidebarOpen: getSidebarIsOpenStateFromLocalStorage(),
				// Re-read the remembered disclosure preference on each open
				aiDisclosureHidden: getAiDisclosureHiddenFromLocalStorage(),
			};
		...
		case 'SET_AI_DISCLOSURE_HIDDEN':
			try {
				localStorage.setItem( AI_DISCLOSURE_HIDDEN_STORAGE_KEY, String( action.payload ) );
			} catch {
				// Ignore localStorage errors
			}
			return {
				...state,
				aiDisclosureHidden: action.payload,
			};
...
export interface ImageStudioActions {
	...
	setAiDisclosureHidden: ( hidden: boolean ) => Promise< SetAiDisclosureHiddenAction >;
...
const actions = {
	...
	setAiDisclosureHidden( hidden: boolean ): SetAiDisclosureHiddenAction {
		return {
			type: 'SET_AI_DISCLOSURE_HIDDEN',
			payload: hidden,
		};
...
export interface ImageStudioSelectors {
	...
	getAiDisclosureHidden: ( state: ImageStudioState ) => boolean;
...
const selectors = {
	...
	getAiDisclosureHidden( state: ImageStudioState ): boolean {
		return state.aiDisclosureHidden;
	},
