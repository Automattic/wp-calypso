...
export function ImageStudioAltTextSidebar( {
	...
	const { setHasUpdatedMetadata, setCanvasMetadata, setAiDisclosureHidden } = useDispatch(
		imageStudioStore
	) as any;
	...
	const aiDisclosureHidden = useSelect(
		( select ) => select( imageStudioStore ).getAiDisclosureHidden(),
		[]
	);

	const handleDisclosureToggle = ( showLabel: boolean ) => {
		setAiDisclosureHidden( ! showLabel );
	...
	return (
		<ImageStudioSidebar onClose={ onClose } title={ __( 'Image Info', __i18n_text_domain__ ) }>
			...
			<ToggleControl
				...
				checked={ ! aiDisclosureHidden }
				label={ __( 'Show "Generated by AI" label on this image', __i18n_text_domain__ ) }
