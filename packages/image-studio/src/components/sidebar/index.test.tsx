...
jest.mock( './editable-field', () => {
	// eslint-disable-next-line @typescript-eslint/no-require-imports -- hoisted jest.mock factory can't use a top-level import
...
jest.mock( '@wordpress/components', () => ( {
	...
	ToggleControl: ( { checked, label, onChange }: any ) => (
		<input
			type="checkbox"
			aria-label={ label }
			checked={ checked }
...
describe( 'ImageStudioAltTextSidebar', () => {
	...
	const mockSetAiDisclosureHidden = jest.fn();
	...
	beforeEach( () => {
		...
		mockUseDispatch.mockReturnValue( {
			...
			setAiDisclosureHidden: mockSetAiDisclosureHidden,
		} as any );
	...
	describe( 'AI disclosure toggle', () => {
		const TOGGLE_LABEL = 'Show "Generated by AI" label on this image';

		it( 'is checked when the store says the disclosure is shown', () => {
			render( <ImageStudioAltTextSidebar { ...defaultProps } /> );

			expect( screen.getByRole( 'checkbox', { name: TOGGLE_LABEL } ) ).toBeChecked();
		} );

		it( 'is off when the disclosure is hidden for the image', () => {
			mockUseSelect.mockImplementation( ( selector: any ) =>
				selector( () => ( {
					getImageStudioAttachmentId: () => 123,
					getCanvasMetadata: () => ( {} ),
					getAiDisclosureHidden: () => true,
				} ) )
			);

			render( <ImageStudioAltTextSidebar { ...defaultProps } /> );

			expect( screen.getByRole( 'checkbox', { name: TOGGLE_LABEL } ) ).not.toBeChecked();
		} );

		it( 'hides the disclosure and flags the metadata dirty when turned off', async () => {
			const user = userEvent.setup();
			render( <ImageStudioAltTextSidebar { ...defaultProps } /> );

			await user.click( screen.getByRole( 'checkbox', { name: TOGGLE_LABEL } ) );

			expect( mockSetAiDisclosureHidden ).toHaveBeenCalledWith( true );
			expect( mockSetHasUpdatedMetadata ).toHaveBeenCalledWith( true );
		} );

		it( 'shows the disclosure when turned on from a hidden image', async () => {
			const user = userEvent.setup();
			mockUseSelect.mockImplementation( ( selector: any ) =>
				selector( () => ( {
					getImageStudioAttachmentId: () => 123,
			...
			expect( mockSetAiDisclosureHidden ).toHaveBeenCalledWith( false );
			expect( mockSetHasUpdatedMetadata ).toHaveBeenCalledWith( true );
		} );
	} );
} );
