...
- In the editor, reuse `MediaUploadCheck` or the block editor's `getSettings().mediaUpload` for buttons, menu items, and suggestions. This follows the editor's behaviour, which may allow uploads while permission is still resolving. Outside the editor, use the host's upload check or require `canUser( 'create', 'media' ) === true` from the core data store.
- Add tests for available and unavailable uploads, missing settings, and permission changes where the UI subscribes to them.
