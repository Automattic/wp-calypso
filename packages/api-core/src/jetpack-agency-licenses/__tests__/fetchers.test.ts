...
import { fetchAgencyDevLicenses, fetchJetpackLicenses } from '..';
...
describe( 'fetchAgencyDevLicenses', () => {
	afterEach( () => nock.cleanAll() );
	...
	it( 'fetches the remaining free development licenses for the agency', async () => {
		...
		await expect( fetchAgencyDevLicenses( 123 ) ).resolves.toEqual( response );
		expect( scope.isDone() ).toBe( true );
	} );
} );
