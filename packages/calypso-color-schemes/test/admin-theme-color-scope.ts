...
// Built here rather than read from src/__wp-base-styles, which is generated and git-ignored: that file survives branch switches, so asserting against it fails for reasons unrelated to the code under test.
const schemes = (): Record< string, string > =>
	getAdminSchemes(
		fs.readFileSync( require.resolve( '@wordpress/base-styles/_mixins.scss' ), 'utf8' )
	);

// The generated file now emits a map and a mixin rather than rules, so these tests compile it with the real Sass rather than parsing the source string. That covers what a string check cannot: that the file is valid Sass, that `color.adjust` resolves, and that the selectors a caller ends up with are the ones intended.
...
describe( 'admin theme colour declarations', () => {
	...
	it( 'sets the admin theme token — not some other property — to each scheme’s own colour', () => {
		// Guards the whole point of the package: a block that named a different property, or reused one scheme’s hex for every block, would still satisfy every selector-shaped assertion.
		const blocks = parseBlocks( compile( '@include wp-admin-theme-colors(".stats-main");' ) );
	...
	it( 'derives both darker shades the way core does, and keeps them distinct', () => {
		// Core uses color.adjust($c, $lightness: -5%/-10%); matching that is what makes Calypso and wp-admin agree. Collapsing them to one value is the pre-existing bug this replaces, where hover and focus rendered identically.
...
describe( 'admin theme colour scoping', () => {
	...
	it( 'compounds the body qualifier rather than nesting a second body', () => {
		// `body.is-blue.is-section-stats :is(…)`, never `body.is-blue body.is-section-stats :is(…)`, which is what plain nesting would produce and which can never match.
	...
	it( 'emits nothing at all until a caller applies it', () => {
		// What keeps this out of calypso-color-schemes.scss safe to rely on: importing it costs nothing, so only a caller that names roots can put rules in anyone's bundle.
