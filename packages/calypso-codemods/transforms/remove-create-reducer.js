...
function getCases(j, handlerMap) {
	...
	const cases = handlerMap.properties.map((actionNode) => {
		...
		if (test.type === 'Identifier' && (test.name === 'SERIALIZE' || test.name === 'DESERIALIZE')) {
			hasPersistence = true;
		}

		if (test.type === 'Literal' && (test.value === 'SERIALIZE' || test.value === 'DESERIALIZE')) {
			hasPersistence = true;
		}
	...
	if (
		...
		const persistenceNode = j.expressionStatement(
			j.assignmentExpression(
				'=',
				j.memberExpression(j.identifier(varName), j.identifier('hasCustomPersistence'), false),
				j.literal(true)
			)
		);
