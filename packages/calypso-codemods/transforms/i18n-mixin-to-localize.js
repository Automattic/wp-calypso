...
export default function transformer(file, api) {
  ...
  function findDeclarationsToWrap(createClassInstance) {
    ...
    const exportDefaultConnectDeclarations = root.find(
  ...
  createClassesInstances.forEach(createClassInstance => {
    ...
    propertiesToModify.forEach(property => {
      const propertyInstances = j(createClassInstance).find(
        j.MemberExpression,
        {
  ...
  if (foundMixinUsage) {
    ...
    if (i18nCalypsoImports.size()) {
      ...
      if (!localizeImport.size()) {
        i18nCalypsoImport.value.specifiers.push(
          j.importSpecifier(j.identifier("localize"))
        );
      }
    } else {
      root
