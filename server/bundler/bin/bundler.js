 ...
 * Variables
 */
var start = new Date().getTime(), bundleEnv = config('env'), outputOptions;
...
function minify(files) {
    ...
    function minifyFile(file, callback) {
        ...
        child = cp.spawn(
            ...
            [file, '-m', '-c', '-o', file.replace('.js', '.m.js')],
...
webpack(webpackConfig, function(error, stats) {
    ...
    if (process.env.WEBPACK_OUTPUT_JSON) {
        fs.writeFile(path.join(process.cwd(), 'stats.json'), JSON.stringify(stats.toJson()));
    }
