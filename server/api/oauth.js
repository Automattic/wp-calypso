 ...
 */
var req = require('superagent'), bodyParser = require('body-parser');
...
function proxyOAuth(request, response) {
    ...
    req.post(config('desktop_oauth_token_endpoint')).type('form').send(data).end(
        ...
        if (typeof clientResponse === 'undefined') {
            return serverResponse
                ...
                .json({
                    ...
                    error_description: 'The request to ' +
                        error.host +
                        ' failed (code ' +
                        error.code +
