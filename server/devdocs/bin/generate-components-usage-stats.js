...
function main() {
    ...
    var fileList, outFilePath = 'server/devdocs/components-usage-stats.json';

    fileList = process.argv.splice(2, process.argv.length).map(function(fileWithPath) {
        return fileWithPath.replace(/^\.\//, '');
    });

    if (fileList.length === 0) {
        process.stderr.write(
            'You must pass a list of files to process (try "make server/devdocs/components-usage-stats.js"'
        );
        process.exit(1);
    }
