...
/**
 * Copied rather than extended to debug.
 */
class REST_Templates_Controller extends \WP_REST_Controller {
	/**
	 * Post type.
	 *
	 * @since 4.7.0
	...
	public function register_routes() {
		...
		if ( isset( $schema['properties']['password'] ) ) {
			$get_item_args['password'] = array(
				'description' => __( 'The password for the post if it is password protected.', 'full-site-editing' ),
				'type'        => 'string',
			);
		}
		register_rest_route(
			...
			array(
				'args'        => array(
					'id' => array(
						'description' => __( 'Unique identifier for the post.', 'full-site-editing' ),
						'type'        => 'integer',
					),
				),
				array(
				...
				array(
					...
					'args'                => array(
						'force' => array(
							'type'        => 'boolean',
							'default'     => false,
							'description' => __( 'Whether to bypass Trash and force deletion.', 'full-site-editing' ),
						),
					),
				),
				'allow_batch' => $this->allow_batch,
	...
	public function get_items_permissions_check( $request ) {
		...
		if ( 'edit' === $request['context'] && ! current_user_can( $post_type->cap->edit_posts ) ) {
			return new \WP_Error(
				...
				__( 'Sorry, you are not allowed to edit posts in this post type.', 'full-site-editing' ),
		...
		*/
		...
		if ( ! empty( $request['orderby'] ) && 'relevance' === $request['orderby'] && empty( $request['search'] ) ) {
			return new \WP_Error(
				...
				__( 'You need to define a search term to order by relevance.', 'full-site-editing' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! empty( $request['orderby'] ) && 'include' === $request['orderby'] && empty( $request['include'] ) ) {
			return new \WP_Error(
				...
				__( 'You need to define an include parameter to order by include.', 'full-site-editing' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( $page > $max_pages && $total_posts > 0 ) {
			return new \WP_Error(
				...
				__( 'The page number requested is larger than the number of pages available.', 'full-site-editing' ),
				array( 'status' => 400 )
			);
		}
	...
	protected function get_post( $id ) {
		$error = new \WP_Error(
			...
			__( 'Invalid post ID.', 'full-site-editing' ),
			array( 'status' => 404 )
		);
	...
	public function get_item_permissions_check( $request ) {
		...
		if ( 'edit' === $request['context'] && $post && ! $this->check_update_permission( $post ) ) {
			return new \WP_Error(
				'rest_forbidden_context',
				__( 'Sorry, you are not allowed to edit this post.', 'full-site-editing' ),
				array( 'status' => rest_authorization_required_code() )
			);
		}
		...
		if ( $post && ! empty( $request['password'] ) ) {
			...
			if ( ! hash_equals( $post->post_password, $request['password'] ) ) {
				return new \WP_Error(
					...
					__( 'Incorrect post password.', 'full-site-editing' ),
					array( 'status' => 403 )
				);
			}
		}
	...
	public function create_item_permissions_check( $request ) {
		if ( ! empty( $request['id'] ) ) {
			return new \WP_Error(
				'rest_post_exists',
				__( 'Cannot create existing post.', 'full-site-editing' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! empty( $request['author'] ) && get_current_user_id() !== $request['author'] && ! current_user_can( $post_type->cap->edit_others_posts ) ) {
			return new \WP_Error(
				...
				__( 'Sorry, you are not allowed to create posts as this user.', 'full-site-editing' ),
				array( 'status' => rest_authorization_required_code() )
			);
		}
		...
		if ( ! empty( $request['sticky'] ) && ! current_user_can( $post_type->cap->edit_others_posts ) && ! current_user_can( $post_type->cap->publish_posts ) ) {
			return new \WP_Error(
				...
				__( 'Sorry, you are not allowed to make posts sticky.', 'full-site-editing' ),
				array( 'status' => rest_authorization_required_code() )
			);
		}
		...
		if ( ! $this->check_assign_terms_permission( $request ) ) {
			return new \WP_Error(
				...
				__( 'Sorry, you are not allowed to assign the provided terms.', 'full-site-editing' ),
				array( 'status' => rest_authorization_required_code() )
			);
		}
	...
	public function update_item_permissions_check( $request ) {
		...
		if ( ! empty( $request['author'] ) && get_current_user_id() !== $request['author'] && ! current_user_can( $post_type->cap->edit_others_posts ) ) {
			return new \WP_Error(
				'rest_cannot_edit_others',
				__( 'Sorry, you are not allowed to update posts as this user.', 'full-site-editing' ),
				array( 'status' => rest_authorization_required_code() )
			);
		}
	...
	public function delete_item_permissions_check( $request ) {
		...
		if ( $post && ! $this->check_delete_permission( $post ) ) {
			return new \WP_Error(
				'rest_cannot_delete',
				__( 'Sorry, you are not allowed to delete this post.', 'full-site-editing' ),
				array( 'status' => rest_authorization_required_code() )
			);
		}
	...
	public function delete_item( $request ) {
		...
		if ( $force ) {
			...
			if ( ! $supports_trash ) {
				return new \WP_Error(
					...
					sprintf( __( "The post does not support trashing. Set '%s' to delete.", 'full-site-editing' ), 'force=true' ),
					array( 'status' => 501 )
				);
			}
			...
			if ( 'trash' === $post->post_status ) {
				return new \WP_Error(
					...
					__( 'The post has already been deleted.', 'full-site-editing' ),
					array( 'status' => 410 )
				);
			}
		...
		if ( ! $result ) {
			return new \WP_Error(
				'rest_cannot_delete',
				__( 'The post cannot be deleted.', 'full-site-editing' ),
				array( 'status' => 500 )
			);
		}
	...
	protected function prepare_item_for_database( $request ) {
		...
		if ( ! empty( $schema['properties']['author'] ) && ! empty( $request['author'] ) ) {
			...
			if ( get_current_user_id() !== $post_author ) {
				...
				if ( ! $user_obj ) {
					return new \WP_Error(
						'rest_invalid_author',
						__( 'Invalid author ID.', 'full-site-editing' ),
						array( 'status' => 400 )
					);
				}
			}
		...
		if ( ! empty( $schema['properties']['password'] ) && isset( $request['password'] ) ) {
			...
			if ( '' !== $request['password'] ) {
				if ( ! empty( $schema['properties']['sticky'] ) && ! empty( $request['sticky'] ) ) {
					return new \WP_Error(
						'rest_invalid_field',
						__( 'A post can not be sticky and have a password.', 'full-site-editing' ),
						array( 'status' => 400 )
					);
				}
				...
				if ( ! empty( $prepared_post->ID ) && is_sticky( $prepared_post->ID ) ) {
					return new \WP_Error(
						'rest_invalid_field',
						__( 'A sticky post can not be password protected.', 'full-site-editing' ),
						array( 'status' => 400 )
					);
				}
			}
		...
		if ( ! empty( $schema['properties']['sticky'] ) && ! empty( $request['sticky'] ) ) {
			if ( ! empty( $prepared_post->ID ) && post_password_required( $prepared_post->ID ) ) {
				return new \WP_Error(
					'rest_invalid_field',
					__( 'A password protected post can not be set to sticky.', 'full-site-editing' ),
					array( 'status' => 400 )
				);
			}
		}
		...
		if ( ! empty( $schema['properties']['parent'] ) && isset( $request['parent'] ) ) {
			if ( 0 === (int) $request['parent'] ) {
				...
				if ( empty( $parent ) ) {
					return new \WP_Error(
						'rest_post_invalid_id',
						__( 'Invalid post parent ID.', 'full-site-editing' ),
						array( 'status' => 400 )
					);
				}
		...
		switch ( $post_status ) {
			...
			case 'private':
				if ( ! current_user_can( $post_type->cap->publish_posts ) ) {
					return new \WP_Error(
						'rest_cannot_publish',
						__( 'Sorry, you are not allowed to create private posts in this post type.', 'full-site-editing' ),
						array( 'status' => rest_authorization_required_code() )
					);
				}
				break;
			...
			case 'future':
				if ( ! current_user_can( $post_type->cap->publish_posts ) ) {
					return new \WP_Error(
						'rest_cannot_publish',
						__( 'Sorry, you are not allowed to publish posts in this post type.', 'full-site-editing' ),
						array( 'status' => rest_authorization_required_code() )
					);
				}
				break;
		...
		if ( $featured_media ) {
			...
			if ( $result ) {
				...
				return new \WP_Error(
					...
					__( 'Invalid featured media ID.', 'full-site-editing' ),
					array( 'status' => 400 )
				);
			}
		} else {
	...
	public function check_template( $template, $request ) {
		...
		return new \WP_Error(
			...
			sprintf( __( '%1$s is not one of %2$s.', 'full-site-editing' ), 'template', implode( ', ', array_keys( $allowed_templates ) ) )
		);
	}
	...
	protected function check_assign_terms_permission( $request ) {
		...
		foreach ( $taxonomies as $taxonomy ) {
			...
			foreach ( (array) $request[ $base ] as $term_id ) {
				...
				if ( ! current_user_can( 'assign_term', get_term( $term_id, $taxonomy->name ) ) ) { // WPCOM Hack: pass the WP_Term to assign_term capability check. See: https://wp.me/p7H4VZ-2Vd
					return false;
				}
			}
		}
	...
	public function get_item_schema() {
		if ( $this->schema ) {
			// return $this->add_additional_fields_schema( $this->schema ); // WPCOM: do not cache the schema. See https://systemsrequests.wordpress.com/2019/11/04/show_avatars-option-disabled-on-simple-sites/
		}
		...
		$schema = array(
			...
			'properties' => array(
				'date'         => array(
					'description' => __( "The date the post was published, in the site's timezone.", 'full-site-editing' ),
					'type'        => array( 'string', 'null' ),
					'format'      => 'date-time',
					'context'     => array( 'view', 'edit', 'embed' ),
				),
				'date_gmt'     => array(
					'description' => __( 'The date the post was published, as GMT.', 'full-site-editing' ),
					'type'        => array( 'string', 'null' ),
					'format'      => 'date-time',
					'context'     => array( 'view', 'edit' ),
				),
				'guid'         => array(
					'description' => __( 'The globally unique identifier for the post.', 'full-site-editing' ),
					'type'        => 'object',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
					'properties'  => array(
						'raw'      => array(
							'description' => __( 'GUID for the post, as it exists in the database.', 'full-site-editing' ),
							'type'        => 'string',
							'context'     => array( 'edit' ),
							'readonly'    => true,
						),
						'rendered' => array(
							'description' => __( 'GUID for the post, transformed for display.', 'full-site-editing' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
						),
				...
				'link'         => array(
					'description' => __( 'URL to the post.', 'full-site-editing' ),
					'type'        => 'string',
					'format'      => 'uri',
					'context'     => array( 'view', 'edit', 'embed' ),
					'readonly'    => true,
				...
				'modified'     => array(
					'description' => __( "The date the post was last modified, in the site's timezone.", 'full-site-editing' ),
					'type'        => 'string',
					'format'      => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				...
				'modified_gmt' => array(
					'description' => __( 'The date the post was last modified, as GMT.', 'full-site-editing' ),
					'type'        => 'string',
					'format'      => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				...
				'slug'         => array(
					'description' => __( 'An alphanumeric identifier for the post unique to its type.', 'full-site-editing' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit', 'embed' ),
					'arg_options' => array(
				...
				'status'       => array(
					'description' => __( 'A named status for the post.', 'full-site-editing' ),
					'type'        => 'string',
				...
				'type'         => array(
					'description' => __( 'Type of post.', 'full-site-editing' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit', 'embed' ),
					'readonly'    => true,
				),
				'password'     => array(
					'description' => __( 'A password to protect access to the content and excerpt.', 'full-site-editing' ),
					'type'        => 'string',
					'context'     => array( 'edit' ),
				),
			),
		...
		if ( is_post_type_viewable( $post_type_obj ) && $post_type_obj->public ) {
			$schema['properties']['permalink_template'] = array(
				'description' => __( 'Permalink template for the post.', 'full-site-editing' ),
				'type'        => 'string',
				'context'     => array( 'edit' ),
				'readonly'    => true,
			);
			...
			$schema['properties']['generated_slug'] = array(
				'description' => __( 'Slug automatically generated from the post title.', 'full-site-editing' ),
				'type'        => 'string',
				'context'     => array( 'edit' ),
				'readonly'    => true,
			);
		...
		if ( $post_type_obj->hierarchical ) {
			$schema['properties']['parent'] = array(
				'description' => __( 'The ID for the parent of the post.', 'full-site-editing' ),
				'type'        => 'integer',
				'context'     => array( 'view', 'edit' ),
			);
		}
			...
			switch ( $attribute ) {
				...
				case 'title':
					$schema['properties']['title'] = array(
						'description' => __( 'The title for the post.', 'full-site-editing' ),
						'type'        => 'object',
						'context'     => array( 'view', 'edit', 'embed' ),
						'arg_options' => array(
						...
						'properties'  => array(
							'raw'      => array(
								'description' => __( 'Title for the post, as it exists in the database.', 'full-site-editing' ),
								'type'        => 'string',
								'context'     => array( 'edit' ),
							),
							'rendered' => array(
								'description' => __( 'HTML title for the post, transformed for display.', 'full-site-editing' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit', 'embed' ),
								'readonly'    => true,
							),
				...
				case 'editor':
					$schema['properties']['content'] = array(
						'description' => __( 'The content for the post.', 'full-site-editing' ),
						'type'        => 'object',
						'context'     => array( 'view', 'edit' ),
						'arg_options' => array(
							'sanitize_callback' => null, // Note: sanitization implemented in self::prepare_item_for_database().
						...
						'properties'  => array(
							'raw'           => array(
								'description' => __( 'Content for the post, as it exists in the database.', 'full-site-editing' ),
								'type'        => 'string',
								'context'     => array( 'edit' ),
							),
							'rendered'      => array(
								'description' => __( 'HTML content for the post, transformed for display.', 'full-site-editing' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
							'block_version' => array(
								'description' => __( 'Version of the content block format used by the post.', 'full-site-editing' ),
								'type'        => 'integer',
								'context'     => array( 'edit' ),
								'readonly'    => true,
							),
							'protected'     => array(
								'description' => __( 'Whether the content is protected with a password.', 'full-site-editing' ),
								'type'        => 'boolean',
								'context'     => array( 'view', 'edit', 'embed' ),
								'readonly'    => true,
							),
				...
				case 'author':
					$schema['properties']['author'] = array(
						'description' => __( 'The ID for the author of the post.', 'full-site-editing' ),
						'type'        => 'integer',
						'context'     => array( 'view', 'edit', 'embed' ),
					);
					break;
				...
				case 'excerpt':
					$schema['properties']['excerpt'] = array(
						'description' => __( 'The excerpt for the post.', 'full-site-editing' ),
						'type'        => 'object',
						'context'     => array( 'view', 'edit', 'embed' ),
						'arg_options' => array(
							'sanitize_callback' => null, // Note: sanitization implemented in self::prepare_item_for_database().
						...
						'properties'  => array(
							'raw'       => array(
								'description' => __( 'Excerpt for the post, as it exists in the database.', 'full-site-editing' ),
								'type'        => 'string',
								'context'     => array( 'edit' ),
							),
							'rendered'  => array(
								'description' => __( 'HTML excerpt for the post, transformed for display.', 'full-site-editing' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit', 'embed' ),
								'readonly'    => true,
							),
							'protected' => array(
								'description' => __( 'Whether the excerpt is protected with a password.', 'full-site-editing' ),
								'type'        => 'boolean',
								'context'     => array( 'view', 'edit', 'embed' ),
								'readonly'    => true,
							),
				...
				case 'thumbnail':
					$schema['properties']['featured_media'] = array(
						'description' => __( 'The ID of the featured media for the post.', 'full-site-editing' ),
						'type'        => 'integer',
						'context'     => array( 'view', 'edit', 'embed' ),
					);
					break;
				...
				case 'comments':
					$schema['properties']['comment_status'] = array(
						'description' => __( 'Whether or not comments are open on the post.', 'full-site-editing' ),
						'type'        => 'string',
						'enum'        => array( 'open', 'closed' ),
						'context'     => array( 'view', 'edit' ),
					);
					$schema['properties']['ping_status']    = array(
						'description' => __( 'Whether or not the post can be pinged.', 'full-site-editing' ),
						'type'        => 'string',
						'enum'        => array( 'open', 'closed' ),
						'context'     => array( 'view', 'edit' ),
					);
				...
				case 'page-attributes':
					$schema['properties']['menu_order'] = array(
						'description' => __( 'The order of the post in relation to other posts.', 'full-site-editing' ),
						'type'        => 'integer',
						'context'     => array( 'view', 'edit' ),
					);
					break;
				...
				case 'post-formats':
					...
					$schema['properties']['format'] = array(
						'description' => __( 'The format for the post.', 'full-site-editing' ),
						'type'        => 'string',
						'enum'        => $formats,
						'context'     => array( 'view', 'edit' ),
					);
		...
		if ( 'post' === $this->post_type ) {
			$schema['properties']['sticky'] = array(
				'description' => __( 'Whether or not the post should be treated as sticky.', 'full-site-editing' ),
				'type'        => 'boolean',
				'context'     => array( 'view', 'edit' ),
			);
		}
		...
		$schema['properties']['template'] = array(
			'description' => __( 'The theme file to use to display the post.', 'full-site-editing' ),
			'type'        => 'string',
			'context'     => array( 'view', 'edit' ),
			'arg_options' => array(
		...
		foreach ( $taxonomies as $taxonomy ) {
			...
			if ( array_key_exists( $base, $schema['properties'] ) ) {
				...
				_doing_it_wrong(
					...
					sprintf(
						...
						__( 'The "%1$s" taxonomy "%2$s" property (%3$s) conflicts with an existing property on the REST API Posts Controller. Specify a custom "rest_base" when registering the taxonomy to avoid this error.', 'full-site-editing' ),
						$taxonomy->name,
			...
			$schema['properties'][ $base ] = array(
				...
				'description' => sprintf( __( 'The terms assigned to the post in the %s taxonomy.', 'full-site-editing' ), $taxonomy->name ),
				'type'        => 'array',
				'items'       => array(
					'type' => 'integer',
				),
		...
		if ( count( $new_fields ) > 0 ) {
			_doing_it_wrong(
				...
				sprintf(
					...
					__( 'Please use %s to add new schema properties.', 'full-site-editing' ),
					'register_rest_field'
				),
				'5.4.0'
			);
	...
	protected function get_schema_links() {
		...
		if ( 'attachment' !== $this->post_type ) {
			$links[] = array(
				...
				'title'        => __( 'The current user can publish this post.', 'full-site-editing' ),
				'href'         => $href,
				'targetSchema' => array(
					'type'       => 'object',
					'properties' => array(
		...
		$links[] = array(
			...
			'title'        => __( 'The current user can post unfiltered HTML markup and JavaScript.', 'full-site-editing' ),
			'href'         => $href,
			'targetSchema' => array(
				'type'       => 'object',
				'properties' => array(
		...
		if ( 'post' === $this->post_type ) {
			$links[] = array(
				...
				'title'        => __( 'The current user can sticky this post.', 'full-site-editing' ),
				'href'         => $href,
				'targetSchema' => array(
					'type'       => 'object',
					'properties' => array(
		...
		if ( post_type_supports( $this->post_type, 'author' ) ) {
			$links[] = array(
				...
				'title'        => __( 'The current user can change the author on this post.', 'full-site-editing' ),
				'href'         => $href,
				'targetSchema' => array(
					'type'       => 'object',
					'properties' => array(
		...
		foreach ( $taxonomies as $tax ) {
			...
			/* translators: %s: Taxonomy name. */
			$assign_title = sprintf( __( 'The current user can assign terms in the %s taxonomy.', 'full-site-editing' ), $tax->name );
			/* translators: %s: Taxonomy name. */
			$create_title = sprintf( __( 'The current user can create terms in the %s taxonomy.', 'full-site-editing' ), $tax->name );
	...
	public function get_collection_params() {
		...
		$query_params['after'] = array(
			'description' => __( 'Limit response to posts published after a given ISO8601 compliant date.', 'full-site-editing' ),
			'type'        => 'string',
			'format'      => 'date-time',
		);

		$query_params['modified_after'] = array(
			'description' => __( 'Limit response to posts modified after a given ISO8601 compliant date.', 'full-site-editing' ),
			'type'        => 'string',
			'format'      => 'date-time',
		);
		...
		if ( post_type_supports( $this->post_type, 'author' ) ) {
			$query_params['author']         = array(
				'description' => __( 'Limit result set to posts assigned to specific authors.', 'full-site-editing' ),
				'type'        => 'array',
				'items'       => array(
					'type' => 'integer',
				),
			...
			$query_params['author_exclude'] = array(
				'description' => __( 'Ensure result set excludes posts assigned to specific authors.', 'full-site-editing' ),
				'type'        => 'array',
				'items'       => array(
					'type' => 'integer',
				),
		...
		$query_params['before'] = array(
			'description' => __( 'Limit response to posts published before a given ISO8601 compliant date.', 'full-site-editing' ),
			'type'        => 'string',
			'format'      => 'date-time',
		);

		$query_params['modified_before'] = array(
			'description' => __( 'Limit response to posts modified before a given ISO8601 compliant date.', 'full-site-editing' ),
			'type'        => 'string',
			'format'      => 'date-time',
		);

		$query_params['exclude'] = array(
			'description' => __( 'Ensure result set excludes specific IDs.', 'full-site-editing' ),
			'type'        => 'array',
			'items'       => array(
				'type' => 'integer',
			),
		...
		$query_params['include'] = array(
			'description' => __( 'Limit result set to specific IDs.', 'full-site-editing' ),
			'type'        => 'array',
			'items'       => array(
				'type' => 'integer',
			),
		...
		if ( 'page' === $this->post_type || post_type_supports( $this->post_type, 'page-attributes' ) ) {
			$query_params['menu_order'] = array(
				'description' => __( 'Limit result set to posts with a specific menu_order value.', 'full-site-editing' ),
				'type'        => 'integer',
			);
		}

		$query_params['offset'] = array(
			'description' => __( 'Offset the result set by a specific number of items.', 'full-site-editing' ),
			'type'        => 'integer',
		);

		$query_params['order'] = array(
			'description' => __( 'Order sort attribute ascending or descending.', 'full-site-editing' ),
			'type'        => 'string',
			'default'     => 'desc',
			'enum'        => array( 'asc', 'desc' ),
		);
		...
		$query_params['orderby'] = array(
			'description' => __( 'Sort collection by post attribute.', 'full-site-editing' ),
			'type'        => 'string',
			'default'     => 'date',
			'enum'        => array(
				'author',
		...
		if ( $post_type->hierarchical || 'attachment' === $this->post_type ) {
			$query_params['parent']         = array(
				'description' => __( 'Limit result set to items with particular parent IDs.', 'full-site-editing' ),
				'type'        => 'array',
				'items'       => array(
					'type' => 'integer',
				),
			...
			$query_params['parent_exclude'] = array(
				'description' => __( 'Limit result set to all items except those of a particular parent ID.', 'full-site-editing' ),
				'type'        => 'array',
				'items'       => array(
					'type' => 'integer',
				),
		...
		$query_params['slug'] = array(
			'description'       => __( 'Limit result set to posts with one or more specific slugs.', 'full-site-editing' ),
			'type'              => 'array',
			'items'             => array(
				'type' => 'string',
			),
		...
		$query_params['status'] = array(
			'default'           => 'publish',
			'description'       => __( 'Limit result set to posts assigned one or more statuses.', 'full-site-editing' ),
			'type'              => 'array',
			'items'             => array(
		...
		if ( 'post' === $this->post_type ) {
			$query_params['sticky'] = array(
				'description' => __( 'Limit result set to items that are sticky.', 'full-site-editing' ),
				'type'        => 'boolean',
			);
		}
		...
		foreach ( $statuses as $status ) {
			...
			if ( current_user_can( $post_type_obj->cap->edit_posts ) || 'private' === $status && current_user_can( $post_type_obj->cap->read_private_posts ) ) {
				...
				return new \WP_Error(
					...
					__( 'Status is forbidden.', 'full-site-editing' ),
					array( 'status' => rest_authorization_required_code() )
				);
			}
		}
	...
	private function prepare_taxonomy_limit_schema( array $query_params ) {
		...
		$query_params['tax_relation'] = array(
			'description' => __( 'Limit result set based on relationship between multiple taxonomies.', 'full-site-editing' ),
			'type'        => 'string',
			'enum'        => array( 'AND', 'OR' ),
		);
		...
		$limit_schema = array(
			...
			'oneOf' => array(
				array(
					'title'       => __( 'Term ID List', 'full-site-editing' ),
					'description' => __( 'Match terms with the listed IDs.', 'full-site-editing' ),
					'type'        => 'array',
					'items'       => array(
						'type' => 'integer',
					),
				...
				array(
					'title'                => __( 'Term ID Taxonomy Query', 'full-site-editing' ),
					'description'          => __( 'Perform an advanced term query.', 'full-site-editing' ),
					'type'                 => 'object',
					'properties'           => array(
						'terms'            => array(
							'description' => __( 'Term IDs.', 'full-site-editing' ),
							'type'        => 'array',
							'items'       => array(
								'type' => 'integer',
							),
						...
						'include_children' => array(
							'description' => __( 'Whether to include child terms in the terms limiting the result set.', 'full-site-editing' ),
							'type'        => 'boolean',
							'default'     => false,
						),
					),
		...
		$include_schema = array_merge(
			array(
				/* translators: %s: Taxonomy name. */
				'description' => __( 'Limit result set to items with specific terms assigned in the %s taxonomy.', 'full-site-editing' ),
			),
			$limit_schema
		);
		// 'operator' is supported only for 'include' queries.
		$include_schema['oneOf'][1]['properties']['operator'] = array(
			'description' => __( 'Whether items must be assigned all or any of the specified terms.', 'full-site-editing' ),
			'type'        => 'string',
			'enum'        => array( 'AND', 'OR' ),
			'default'     => 'OR',
		);
		...
		$exclude_schema = array_merge(
			array(
				/* translators: %s: Taxonomy name. */
				'description' => __( 'Limit result set to items except those with specific terms assigned in the %s taxonomy.', 'full-site-editing' ),
			),
			$limit_schema
		);
