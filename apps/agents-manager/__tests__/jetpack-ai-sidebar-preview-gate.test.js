...
const AI_BLOCK_NOTES_PROVIDER = 'ai-block-notes/headless-agent-provider';
...
describe( 'shouldSuppressJetpackAiSidebarPreview', () => {
	...
	it( 'does not suppress non-Jetpack providers like AI Block Notes', () => {
		window._currentSiteType = 'atomic';
		setAgentsManagerData( {
			sectionName: 'gutenberg',
			agentProviders: [ AI_BLOCK_NOTES_PROVIDER ],
		} );

		expect( shouldSuppressJetpackAiSidebarPreview() ).toBe( false );
		expect( globalThis.agentsManagerData.agentProviders ).toEqual( [ AI_BLOCK_NOTES_PROVIDER ] );
	} );
