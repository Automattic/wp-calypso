...
This app bundles the package into 8 webpack entry points deployed to `widgets.wp.com/agents-manager/`. They are separate compilations sharing one `dist/`, so each entry's chunk filenames and chunk-loading global must stay entry-unique. Jetpack enqueues these on Simple and Atomic sites.
