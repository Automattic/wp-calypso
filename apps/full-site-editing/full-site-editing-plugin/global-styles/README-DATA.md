...
Global Styles adds a new REST API endpoint. It is a thin endpoint that outsources the data processing to the `Global_Styles_Data_Set` class, which implements a declarative approach to data retrieval and persistance.
...
The `\A8C\FSE\Global_Styles_Data_Set` takes as input a declarative data format and knows how to process three kinds of data:

```php
// Initialize
$data_set = new \A8C\FSE\Global_Styles_Data_Set( [
	'font_list' => [
		'type' => 'literal',
		'default' => [
			'Arvo',
...
**The `Global_Styles_Data_Set` doesn't do any validation or sanitization**, it's the consumer responsibility to do so. There are two filters that can be used to prepare, filter, validate and sanitize your data:
