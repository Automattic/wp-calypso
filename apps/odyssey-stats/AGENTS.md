...
`overlayClassName` lands on the exact same element that carries `.components-modal__screen-overlay`, never on an ancestor — Modal always portals straight to `document.body`. That's fine for the vendor CSS's descendant selectors (`.components-modal__frame` etc.: the overlay div is a real ancestor of those), but the vendor CSS's own bare `.components-modal__screen-overlay` rule (its base position/backdrop styling) targets that element directly, so the default ancestor-based prefixing can never match it — nothing is ever an ancestor of an element carrying its own marker class. `vendorTransform` in `webpack-css-scope.js` special-cases exactly that selector, compounding `.is-odyssey-stats` onto it instead of prefixing it as an ancestor requirement. `bin/verify-css-scope.js`'s `findVendorOverlaySelfNestingFailures` check guards against this regressing (e.g. a future `@wordpress/components` upgrade changing that selector's text so the transform's regex stops matching).
