...
export function RedirectToHomeLaunchpadAfterPublishingPost() {
	const currentPostType = useSelect(
		( localSelect ) => localSelect( 'core/editor' ).getCurrentPostType(),
		[]
	);

	const hasPublishFirstPostTaskQueryArg =
		getQueryArg( window.location.search, 'task' ) === 'publish_first_post';
	...
	const unsubscribe = subscribe( () => {
		...
		if ( ! isSavingPost && ( isCurrentPostPublished || isCurrentPostScheduled ) ) {
			unsubscribe();

			dispatch( 'core/edit-post' ).closePublishSidebar();

			// Show success notice with Next steps link
			dispatch( 'core/notices' ).createSuccessNotice( 'Well done publishing your first post!', {
				actions: [
					{
						label: 'Next steps',
