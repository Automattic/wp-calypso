...
describe('QuickSaveButtons', () => {
	describe('#isSaveAvailableFn()', () => {
		...
		test('should return false if post does not exist', () => {
			expect(isSaveAvailableFn({ isSaving: false, hasContent: true, isDirty: true })).to.be.false;
		});

		test('should return true if dirty and post has content and post is not published', () => {
			expect(isSaveAvailableFn({ isSaving: false, post: {}, hasContent: true, isDirty: true })).to
				.be.true;
		});
		...
		test('should return false if dirty and post is published', () => {
			expect(isSaveAvailableFn({ isSaving: false, post: { status: 'publish' }, isDirty: true })).to
				.be.false;
		});
	});
});
