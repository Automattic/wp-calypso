...
class VerifyEmailDialog extends React.Component {
    ...
    sendVerification(e) {
        ...
        this.props.user.sendVerificationEmail(
    ...
    getDialogButtons() {
        return [
            <FormButton key="close" isPrimary={true} onClick={this.props.onClose}>
                {i18n.translate('OK')}
            </FormButton>,
            <FormButton
                key="resend"
    ...
    render() {
        const strings = {
            ...
            confirmExplanation: i18n.translate(
                'We sent you an email when you first signed up. Please open the message and click the blue button.'
            ),

            confirmReasoning: i18n.translate(
                'Email confirmation allows us to assist when recovering your account in the event you forget your password.'
            ),
            ...
            confirmEmail: i18n.translate(
                '{{wrapper}}%(email)s{{/wrapper}} {{emailPreferences}}change{{/emailPreferences}}',
                {
                    components: {
                        wrapper: (
                            <span className="post-editor__confirmation-dialog-email-wrapper" />
                        ),
        ...
        return (
            ...
            >
                <h1 className="post-editor__confirmation-dialog-heading is-variable-height">
                    {strings.confirmHeading}
                </h1>
                <p className="post-editor__confirmation-dialog-email">{strings.confirmEmail}</p>
                <p className="post-editor__confirmation-dialog-explanation">
                    {strings.confirmExplanation}
                </p>
                <p className="post-editor__confirmation-dialog-reasoning">
