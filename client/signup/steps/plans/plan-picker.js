...
const PlanPicker = ( { isShowing, closePlanPicker } ) => {
	const defaultSelectedFeature = [];
	const [ selectedFeatures, setSelectedFeatures ] = useState( defaultSelectedFeature );
	...
	const getRecommendedPlanSlug = () => {
		const plansOrder = [ 'Free', 'Personal', 'Premium', 'Business', 'eCommerce' ];
		if ( ! selectedFeatures.length ) return undefined;

		return selectedFeatures.reduce( ( currentMinSupportedPlan, featureId ) => {
			const featureMinSupportedPlan = featureData[ featureId ].minSupportedPlan;
		...
		}, featureData[ selectedFeatures[ 0 ] ].minSupportedPlan );
	};

	useEffect( () => {
		setRecommendedPlan( getRecommendedPlanSlug );
	}, [ selectedFeatures ] );

	return (
		<Dialog
			isVisible={ isShowing }
			buttons={ [ <Button onClick={ closePlanPicker }>Close</Button> ] }
			onCancel={ closePlanPicker }
			onClose={ closePlanPicker }
		>
			<Container>
				<Header>
					<Title>Which features will you need?</Title>
					<SubTitle>
						Choose the features that matter to you and we'll suggest a plan to suit your needs.
						Don't worry, you won't be charged and can change it at any time.
					</SubTitle>
				</Header>

				<Items>
					{ Object.entries( featureData ).map( ( [ id, feature ] ) => (
						<Button
