...
class ImportURLStepComponent extends Component {
	...
	renderContent = () => {
		const { translate, signupDependencies: { importSiteUrl } = {} } = this.props;
		...
		return (
			<div className="confirm-import__wrapper">
				...
				<SiteMockup className={ siteMockupClasses } size="desktop">
					{ previewImageSrc && (
						<img
							src={ previewImageSrc }
							alt={ `Preview Screenshot of site: ${ withoutHttp( importSiteUrl ) }` }
						/>
					) }
				</SiteMockup>
			</div>
	...
	render() {
		const {
			flowName,
			positionInFlow,
			signupDependencies: { importSiteUrl } = {},
			signupProgress,
			stepName,
			translate,
		} = this.props;
		...
		const subHeaderText = translate(
			"Here's what we found at {{strong}}%(importSiteUrl)s{{/strong}}, please take a moment to confirm that this is your site.",
			{
				args: { importSiteUrl: withoutHttp( importSiteUrl ) },
