...
class DomainsStepPaidMedia extends Component {
	static propTypes = {
	...
	constructor( props ) {
		...
		const domain = props?.[ 'queryObject.new' ] ?? false;
		const search = ( props?.[ 'queryObject.search' ] ?? false ) === 'yes';
		const suggestedDomain = props?.[ 'signupDependencies.suggestedDomain' ];
	...
	shouldIncludeDotBlogSubdomain() {
		...
		if (
			...
			'blog' === signupDependencies?.siteType ||
			'blog' === siteType
		) {
			return true;
		}

		const lastQuery = this.props?.step?.[ 'domainForm.lastQuery' ];
	...
	domainForm = () => {
		...
		const initialQuery =
			( this.props?.queryObject?.new ?? '' ) || this.props?.signupDependencies?.suggestedDomain;

		// Search using the initial query but do not show the query on the search input field.
		const hideInitialQuery = ( this.props?.queryObject?.hide_initial_query ?? false ) === 'yes';
	...
	useYourDomainForm = () => {
		const queryObject = Object.fromEntries( new URLSearchParams( window.location?.search ) );
		const initialQuery = this.props?.step?.domainForm?.lastQuery || queryObject.initialQuery;
	...
	getPreviousStepUrl() {
		...
		const { step, ...queryValues } = Object.fromEntries(
