...
const ConfirmDialog = ( {
	...
	<div className="activity-log-item activity-log-item__restore-confirm">
		...
		<Card className="activity-log-item__card">
			<h5 className="confirm-dialog__title">{ title }</h5>

			<div className="confirm-dialog__highlight">{ children }</div>

			<div className="confirm-dialog__partial-restore-settings">
				<p>
					{ notice
			...
			{ notice && (
				<div className="confirm-dialog__notice">
					...
					<span className="confirm-dialog__notice-content">{ notice }</span>
				</div>
			) }

			<div className="confirm-dialog__button-wrap">
				<div className="confirm-dialog__primary-actions">
					<Button onClick={ onClose }>{ translate( 'Cancel' ) }</Button>
					<Button primary disabled={ disableButton } onClick={ onConfirm }>
						{ confirmTitle }
					</Button>
				</div>
				<div className="confirm-dialog__secondary-actions">
					<ExternalLink icon href={ supportLink } onClick={ () => {} }>
						{ translate( 'More info' ) }
					</ExternalLink>
					<HappychatButton className="confirm-dialog__more-info-link" onClick={ happychatEvent }>
						<Gridicon icon="chat" />
...
export default connect( null, mapDispatchToProps )( localize( ConfirmDialog ) );
