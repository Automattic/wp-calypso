...
function getRedirectToAfterLoginUrl( { oauth2Signup, initialContext } ) {
	if (
		oauth2Signup &&
	...
	if ( initialContext.query.redirect_to ) {
...
export class UserStep extends Component {
	...
	getSubHeaderText() {
		const { flowName, oauth2Client, translate, wccomFrom, isOnboardingAffiliateFlow, isWCCOM } =
			this.props;
		...
		if ( [ 'wpcc', 'crowdsignal' ].includes( flowName ) && oauth2Client ) {
			...
			} else if ( isGravatarOAuth2Client( this.props.oauth2Client ) ) {
				subHeaderText = translate(
	...
	getHeaderText() {
		...
		if ( isGravatarOAuth2Client( this.props.oauth2Client ) ) {
			return translate( 'Welcome to Gravatar' );
		}
	...
	render() {
		...
		const getGoBack = () => {
			const backToParam = this.props.initialContext?.query?.back_to?.toString();

			return backToParam?.startsWith( '/' ) ? backToParam : undefined;
		};

		return this.props.children( {
			goBack: getGoBack(),
			loginLink: this.getLoginUrl(),
			children: this.renderSignupForm(),
