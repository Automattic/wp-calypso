...
class AuthorizeFormLoggedInForm extends Component {
	...
	renderFormHeader( isConnected ) {
		...
		const headerText = ( isConnected )
			? this.props.translate( 'You are connected!' )
			: this.props.translate( 'Completing connection' );
		const subHeaderText = ( isConnected )
			? this.props.translate( 'Thank you for flying with Jetpack' )
			: this.props.translate( 'Jetpack is finishing up the connection process' );
	...
	renderXmlrpcFeedback() {
		const xmlrpcErrorText = this.props.translate( 'We had trouble connecting.' );
		return (
			<div>
	...
	getButtonText() {
		...
		if ( ! this.props.isAlreadyOnSitesList &&
			! this.props.isFetchingSites &&
			queryObject.already_authorized ) {
			return this.props.translate( 'Go back to your site' );
		}

		if ( authorizeError && ! this.retryingAuth ) {
			return this.props.translate( 'Try again' );
		}

		if ( this.props.isFetchingSites ) {
			return this.props.translate( 'Preparing authorization' );
		}

		if ( authorizeSuccess && isRedirectingToWpAdmin ) {
			return this.props.translate( 'Returning to your site' );
		}

		if ( authorizeSuccess ) {
			return this.props.translate( 'Finishing up!', {
		...
		if ( isAuthorizing || this.retryingAuth ) {
			return this.props.translate( 'Authorizing your connection' );
		}

		if ( this.props.isAlreadyOnSitesList ) {
			return this.props.translate( 'Return to your site' );
		}

		if ( ! this.retryingAuth ) {
			return this.props.translate( 'Approve' );
		}
	}
	...
	getUserText() {
		...
		let text = this.props.translate( 'Connecting as {{strong}}%(user)s{{/strong}}', {
			args: { user: this.props.user.display_name },
			components: { strong: <strong /> }
		} );

		if ( authorizeSuccess || this.props.isAlreadyOnSitesList ) {
			text = this.props.translate( 'Connected as {{strong}}%(user)s{{/strong}}', {
				args: { user: this.props.user.display_name },
				components: { strong: <strong /> }
			} );
		}
			...
			return (
				<LoggedOutFormLinks>
					...
					<LoggedOutFormLinkItem href={ this.getRedirectionTarget() }>
						{ this.props.translate( 'I\'m not interested in upgrades' ) }
					</LoggedOutFormLinkItem>
				</LoggedOutFormLinks>
			);
		}
