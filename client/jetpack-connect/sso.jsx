...
class JetpackSsoForm extends Component {
	...
	onApproveSSO = event => {
		...
		analytics.tracks.recordEvent( 'jetpack_sso_log_in_button_click' );
	...
	onCancelClick = event => {
		...
		analytics.tracks.recordEvent( 'jetpack_sso_return_to_site_link_click' );
		this.returnToSiteFallback( event );
	};

	onTryAgainClick = event => {
		debug( 'Clicked try again link' );
		analytics.tracks.recordEvent( 'jetpack_sso_try_again_link_click' );
		this.returnToSiteFallback( event );
	};

	onClickSignInDifferentUser = () => {
		analytics.tracks.recordEvent( 'jetpack_sso_sign_in_different_user_link_click' );
	};

	onClickSharedDetailsModal = event => {
		event.preventDefault();
		analytics.tracks.recordEvent( 'jetpack_sso_shared_details_link_click' );
		this.setState( {
	...
	returnToSiteFallback = event => {
		...
		if ( ! get( this.props, 'blogDetails.admin_url' ) ) {
			analytics.tracks.recordEvent( 'jetpack_sso_admin_url_fallback_redirect' );
			event.preventDefault();
			window.history.back();
		}
	};
