...
export class JetpackAuthorize extends Component {
	...
	isWooRedirect(props = this.props) {
		const { from } = props.authQuery;
		return includes(
			['woocommerce-services-auto-authorize', 'woocommerce-setup-wizard', 'woocommerce-onboarding'],
			from
		);
	}
	...
	renderNotices() {
		...
		if (this.retryingAuth) {
			return (
				<JetpackConnectNotices noticeType={RETRYING_AUTH} onTerminalError={redirectToMobileApp} />
			);
		}
	...
	getRedirectionTarget() {
		...
		return addQueryArgs({ redirect: redirectAfterAuth }, `${JPC_PATH_PLANS}/${urlToSlug(homeUrl)}`);
	}
