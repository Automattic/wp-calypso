...
console.log('hey', this.getStatus());
    const product = window.location.href.split( '/' )[ 5 ];
    const is_search = startsWith( product, 'jetpack_search' );

    console.log('hey qq', product, is_search );
		...
		if (
			...
			return this.goToRemoteAuth( this.props.siteHomeUrl, is_search);
  	}
		if ( this.getStatus() === ALREADY_OWNED && ! this.state.redirecting ) {
			...
			return is_search ?
        page( `/checkout/${ urlToSlug( this.state.currentUrl ) }/jetpack_search` ):
	...
	goToRemoteAuth = this.makeSafeRedirectionFunction( ( url, is_search ) => {
		this.props.recordTracksEvent( 'calypso_jpc_success_redirect', {
			url: url,
			type: 'remote_auth',
		} );
console.log('hey yy', is_search, addQueryArgs(
  { product: 'jetpack_search' }, url + REMOTE_PATH_AUTH ));
