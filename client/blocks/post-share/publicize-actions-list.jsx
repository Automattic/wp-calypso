...
class PublicizeActionsList extends PureComponent {
	...
	renderScheduledMenu(publicizeActionId, message, service) {
		...
		if (isEnabled('publicize-preview')) {
			actions.push(
				<PopoverMenuItem onClick={this.togglePreviewModal(message, service)} key="1" icon="visible">
					{translate('Preview')}
				</PopoverMenuItem>
			);
		}
		...
		actions.push(
			<PopoverMenuItem onClick={this.deleteScheduledAction(publicizeActionId)} key="2" icon="trash">
				{translate('Trash')}
			</PopoverMenuItem>
		);
