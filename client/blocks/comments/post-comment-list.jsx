...
class PostCommentList extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (
            nextSiteId &&
            nextPostId &&
            (this.props.post.site_ID !== nextSiteId || this.props.post.ID !== nextPostId)
        ) {
    ...
    getCommentsCount(commentIds) {
        ...
        return commentIds.reduce(
            (prevSum, commentId) =>
                prevSum +
                this.getCommentsCount(this.props.commentsTree.getIn([commentId, 'children'])) +
                1,
            0
        );
    }
    ...
    render() {
        ...
        const showViewEarlier = this.props.commentsTree.get('children').size >
            this.state.amountOfCommentsToTake || this.props.haveMoreCommentsToFetch;
                    ...
                    <div className="comments__info-bar">
                        ...
                        {showViewEarlier
                            ? <span
                              ...
                              >
                                  {translate(
                                      'View earlier comments (Showing %(shown)d of %(total)d)',
                                      {
                                          args: {
...
export default connect(
    (state, ownProps) => ({
        ...
        totalCommentsCount: getPostTotalCommentsCount(
            state,
