...
describe('ImageEditorToolbar', () => {
	...
	test('should not add `is-disabled` class to aspect ratio toolbar button by default', () => {
		expect(wrapper.find('.image-editor__toolbar-button').at(1).hasClass('is-disabled')).to.be.false;
	});
	...
	test(
		...
		() => {
			...
			expect(wrapper.find('.image-editor__toolbar-button').at(1).hasClass('is-disabled')).to.be
				.true;
		}
	);
