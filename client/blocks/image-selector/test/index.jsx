...
describe('ImageSelector', () => {
	...
	describe('rendering', () => {
		test('should show the add image uploader label when no images are passed', () => {
			...
			expect(wrapper.find('.image-selector__uploader-label').contains('Add Image')).to.equal(true);
		});
		...
		test('should show an uploader when image exists and set to allow multiple images', () => {
			...
			expect(wrapper.find('.image-selector__uploader-wrapper').hostNodes()).to.have.lengthOf(1);
		});
		...
		test('should not show an uploader when an image exists and multiple images not allowed', () => {
			...
			expect(wrapper.find('.image-selector__uploader-wrapper').hostNodes()).to.have.lengthOf(0);
		});
	...
	describe('events', () => {
		...
		test('should pass back image for removal when remove button is clicked', () => {
			...
			expect(mockOnRemoveImage).to.have.been.calledWith(require('./fixtures').DUMMY_MEDIA[100]);
		});
		...
		test('should pass back image when file is dropped', () => {
			...
			expect(mockOnAddImage).to.have.been.calledWith(require('./fixtures').DUMMY_MEDIA[100]);
		});
