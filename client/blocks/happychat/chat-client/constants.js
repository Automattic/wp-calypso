// Auth strategies
export const AUTH_TYPE_WPCOM_OAUTH_BY_TOKEN = 'wpcom-oauth-by-token';
export const AUTH_TYPE_WPCOM_PROXY_IFRAME = 'wpcom-proxy-iframe';

// Layouts
export const LAYOUT_MAX_PARENT_SIZE = 'max-parent-size';
export const LAYOUT_MAX_WIDTH_FIXED_HEIGHT = 'max-width-fixed-height';
export const LAYOUT_PANEL_FIXED_SIZE = 'panel-fixed-size';
export const LAYOUT_PANEL_MAX_PARENT_SIZE = 'panel-max-parent-size';

// Entry
export const ENTRY_CHAT = 'chat';
export const ENTRY_FORM = 'form';

// Skills
export const HAPPYCHAT_SKILL_LANGUAGE = 'language';
export const HAPPYCHAT_SKILL_PRODUCT = 'product';

// Groups
export const HAPPYCHAT_GROUP_JPOP = 'jpop';
export const HAPPYCHAT_GROUP_JPPHP = 'jpphp';
export const HAPPYCHAT_GROUP_WOO = 'woo';
export const HAPPYCHAT_GROUP_WPCOM = 'WP.com';

// Events
export const HAPPYCHAT_EVENT_CHAT_AVAILABILITY = 'chatAvailability';
export const HAPPYCHAT_EVENT_CHAT_LAST_ACTIVITY = 'chatLastActivity';
export const HAPPYCHAT_EVENT_CHAT_PANEL_OPEN = 'chatPanelOpen';
export const HAPPYCHAT_EVENT_CHAT_STATUS = 'chatStatus';
export const HAPPYCHAT_EVENT_CONNECTION_STATUS = 'connectionStatus';
