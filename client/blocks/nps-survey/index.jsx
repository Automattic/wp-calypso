	...
	renderPromotion() {
		...
		return (
			<div className="nps-survey__promotion">
						...
						<p>
							{translate(
								'{{booking}}Reserve a 1:1 Quick Start Session{{/booking}} now or connect with a Happiness Engineer {{contact}}over live chat or email{{/contact}}.',
								{
									components: {
										booking: (
											<a href="/me/concierge" onClick={this.handleLinkClick} data-type="booking" />
										),
										contact: (
											<a
					...
					<p>
						{translate(
							'If you would like help with your site, our WordPress.com Happiness Engineers are ready {{contact}}over live chat or email{{/contact}} now.',
							{
								components: {
									contact: (
										<a href={CALYPSO_CONTACT} onClick={this.handleLinkClick} data-type="contact" />
									),
								},
							}
						)}
				...
				<div className="nps-survey__buttons">
					<Button primary className="nps-survey__finish-button" onClick={this.handlePromotionClose}>
						{translate('Close')}
					</Button>
				</div>
			</div>
