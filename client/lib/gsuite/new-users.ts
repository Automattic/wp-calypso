...
const validateOverallEmailAgainstExistingEmails = (
	...
	value: mailBox,
	error:
		!mailBoxError && includes(mapValues(existingGSuiteUsers, 'email'), `${mailBox}@${domain}`)
...
const validateUser = (user: GSuiteNewUser): GSuiteNewUser => {
	...
	const { domain, mailBox, firstName, lastName } = mapValues(user, (field) => requiredField(field));
