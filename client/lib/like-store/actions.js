...
export function fetchLikes( siteId, postId ) {
	...
	const callback = requestTracker( requestKey, ( error, data ) =>
...
export function likePost( siteId, postId ) {
	...
	wpcom
		...
		.post( postId )
		.like()
		.add( getQuery(), ( error, data ) => receiveLikeResponse( error, siteId, postId, data ) );
}
...
export function unlikePost( siteId, postId ) {
	...
	wpcom
		...
		.post( postId )
		.like()
		.del( getQuery(), ( error, data ) => receiveUnlikeResponse( error, siteId, postId, data ) );
}
