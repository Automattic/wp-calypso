...
const olark = {
    ...
    configureOlark: function(wpcomOlarkConfig = {}, dispatch) {
        ...
        if (config.isEnabled('help')) {
            ...
            olarkApi(
                ...
                details => details.isConversing ? this.showChatBox() : olarkActions.hideBox()
            );
        } else {
            this.showChatBox();
        }
    ...
    setOlarkOptions(userData, wpcomOlarkConfig = {}) {
        ...
        const visitorNickname = wpcomOlarkConfig.nickname ||
            userData.username + ' | ' + this.userType;
    ...
    updateOlarkFormatting(username, avatarURL) {
        ...
        for (nameNodeIndex = 0; nameNodeIndex < allNameNodes.length; nameNodeIndex++) {
            ...
            personClassName = nameNode.className.replace(
                /.*(habla_conversation_person\d+).*/,
                '$1'
            );
            ...
            if (!nameNode.querySelector('.gravatar')) {
                ...
                if (isUserResponse) {
                    ...
                    gravatar.setAttribute(
                        'src',
                        olarkAvatarMap[nameNodeContent] || defaultAvatarURL
                    );
                }
    ...
    sendVisitorDataToOperator(userData, siteUrl) {
        const visitorData = [
            ...
            'User ID: ' + userData.ID,
            'Site: ' + siteUrl,
            'Network Admin: https://wordpress.com/wp-admin/network/users.php?submit=Search+Users&s=' +
                userData.username,
            'Upgrades and Transactions : https://wordpress.com/wp-admin/network/wpcom-paid-upgrades.php?action=search&username=' +
                userData.username,
        ];
    ...
    updateLivechatActiveCookie() {
        ...
        olarkApi('api.chat.onReady', () => {
            //This function will always run when called if the olark API is ready (It's not a single fire event).
