...
describe('MenuData', function() {
    ...
    before(function() {
        (MenuData = require('../menu-data').default), (wp = require('lib/wp')), (fixtures = require('./fixtures'));
    });
        ...
        describe('interceptLoadForHomepageLink', function() {
            ...
            it('should convert the home link to page item', function() {
                var pageItemOriginal = this.menuData.generateHomePageMenuItem(),
                    interceptedItem = this.menuData.interceptLoadForHomepageLink(this.menu).items[
                        0
                    ];
    ...
    describe('singleton', function() {
        it('should return the same valid MenuData instance everytime', function() {
            var i1 = require('lib/menu-data'), i2 = require('lib/menu-data');
    ...
    describe('fetch', function() {
        ...
        it('should decode HTML entities', function() {
            this.menuData.fetch();
            assert(
                this.menuData.data.menus[1].name === 'Menu 2 &c.',
                'Ampersand in menu name not decoded'
            );
            assert(
                this.menuData.data.menus[2].description === 'The third menu is dull & boring',
                'Ampersand in menu description not decoded'
            );
            assert(
                this.menuData.data.menus[1].items[0].name === 'Item 21 & counting',
                'Ampersand in menu item name not decoded'
            );

            assert(
                this.menuData.data.locations[2].name === 'social&lite',
                'Ampersand in location name not decoded'
            );
            assert(
                this.menuData.data.locations[0].description === 'Top & Primary Menu',
                'Ampersand in location description not decoded'
            );
        });
    });
    ...
    describe('updateMenuItem', function() {
        beforeEach(function() {
            ...
            // Update an item
            var menu = this.menuData.getMenu('primary'), item = menu.items[0];
            this.itemID = item.id;
    ...
    describe('moveItem', function() {
        ...
        it('should move an item to a different parent', function() {
            ...
            expect(menu.items[2].items[0].items[0]).to.have.property(
                'name',
                'Space invader designs'
            );
        });
