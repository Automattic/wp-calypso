...
describe('getMonthlyPlanByYearly', () => {
	test('should return a proper plan', () => {
		expect(getMonthlyPlanByYearly(PLAN_JETPACK_PERSONAL)).to.equal(PLAN_JETPACK_PERSONAL_MONTHLY);
		expect(getMonthlyPlanByYearly(PLAN_JETPACK_PREMIUM)).to.equal(PLAN_JETPACK_PREMIUM_MONTHLY);
		expect(getMonthlyPlanByYearly(PLAN_JETPACK_BUSINESS)).to.equal(PLAN_JETPACK_BUSINESS_MONTHLY);
...
describe('getYearlyPlanByMonthly', () => {
	test('should return a proper plan', () => {
		expect(getYearlyPlanByMonthly(PLAN_JETPACK_PERSONAL_MONTHLY)).to.equal(PLAN_JETPACK_PERSONAL);
		expect(getYearlyPlanByMonthly(PLAN_JETPACK_PREMIUM_MONTHLY)).to.equal(PLAN_JETPACK_PREMIUM);
		expect(getYearlyPlanByMonthly(PLAN_JETPACK_BUSINESS_MONTHLY)).to.equal(PLAN_JETPACK_BUSINESS);
		expect(getYearlyPlanByMonthly('unknown_plan')).to.equal('');
	});
});
...
describe('findSimilarPlansKeys', () => {
	test('should return a proper similar plan - by term', () => {
		...
		expect(findSimilarPlansKeys(PLAN_JETPACK_PERSONAL, { term: TERM_BIENNIALLY })).to.deep.equal(
			[]
		);
		...
		expect(findSimilarPlansKeys(PLAN_JETPACK_BUSINESS, { term: TERM_BIENNIALLY })).to.deep.equal(
			[]
		);
...
describe('findPlansKeys', () => {
	...
	test('all matching plans keys - by type', () => {
		expect(findPlansKeys({ type: TYPE_FREE })).to.deep.equal([PLAN_FREE, PLAN_JETPACK_FREE]);
...
describe('planMatches - personal', () => {
	test('should return true for matching queries', () => {
		...
		expect(planMatches(PLAN_JETPACK_PERSONAL_MONTHLY, { type: TYPE_PERSONAL })).to.equal(true);
		...
		expect(planMatches(PLAN_JETPACK_PERSONAL_MONTHLY, { group: GROUP_JETPACK })).to.equal(true);
	...
	test('should return false for non-matching queries', () => {
		...
		expect(planMatches(PLAN_JETPACK_PERSONAL_MONTHLY, { type: TYPE_BUSINESS })).to.equal(false);
		...
		expect(planMatches(PLAN_JETPACK_PERSONAL_MONTHLY, { group: GROUP_WPCOM })).to.equal(false);
		...
		expect(planMatches(PLAN_JETPACK_PERSONAL_MONTHLY, { term: TERM_ANNUALLY })).to.equal(false);
	});
});
...
describe('planMatches - business', () => {
	test('should return true for matching queries', () => {
		...
		expect(planMatches(PLAN_JETPACK_BUSINESS_MONTHLY, { type: TYPE_BUSINESS })).to.equal(true);
		...
		expect(planMatches(PLAN_JETPACK_BUSINESS_MONTHLY, { group: GROUP_JETPACK })).to.equal(true);
		...
		expect(planMatches(PLAN_BUSINESS, { type: TYPE_BUSINESS, group: GROUP_WPCOM })).to.equal(true);
		expect(planMatches(PLAN_BUSINESS, { type: TYPE_BUSINESS, term: TERM_ANNUALLY })).to.equal(true);
		expect(
	...
	test('should return false for non-matching queries', () => {
		...
		expect(planMatches(PLAN_JETPACK_BUSINESS_MONTHLY, { type: TYPE_PERSONAL })).to.equal(false);
		...
		expect(planMatches(PLAN_JETPACK_BUSINESS_MONTHLY, { group: GROUP_WPCOM })).to.equal(false);
		...
		expect(planMatches(PLAN_JETPACK_BUSINESS_MONTHLY, { term: TERM_ANNUALLY })).to.equal(false);
	});
});
...
describe('planHasSuperiorFeature', () => {
	test('should return true when a yearly plan has a superior feature', () => {
		expect(planHasSuperiorFeature(PLAN_JETPACK_BUSINESS, FEATURE_JETPACK_BACKUP_DAILY)).to.be.true;
	});

	test('should return true when a monthly plan has a superior feature', () => {
		expect(planHasSuperiorFeature(PLAN_JETPACK_BUSINESS_MONTHLY, FEATURE_JETPACK_BACKUP_DAILY)).to
			.be.true;
	});
	...
	test('should return false when a monthly plan does not have a superior feature', () => {
		expect(planHasSuperiorFeature(PLAN_JETPACK_BUSINESS_MONTHLY, FEATURE_JETPACK_BACKUP_REALTIME))
			.to.be.false;
	});
});
