...
describe('plansLink', () => {
	...
	test('should append intervalType to URL when forceInterval is true', () => {
		expect(plansLink('/plans', 'example.com', 'monthly', true)).toBe('/plans/monthly/example.com');
		expect(plansLink('/plans', 'example.com', 'yearly', true)).toBe('/plans/yearly/example.com');
	});
