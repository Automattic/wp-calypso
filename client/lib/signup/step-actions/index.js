...
export function createSiteIfNeeded( callback, dependencies, stepData, reduxStore ) {
	const { flowName, lastKnownFlow } = stepData;
	const { siteUrl, siteId, siteSlug, cartItem, domainItem } = dependencies;
	const isPurchasingItem = ! isEmpty( cartItem ) || ! isEmpty( domainItem );

	if ( isPurchasingItem || ( siteId && siteSlug !== 'no-site' ) ) {
...
export function createSiteWithCart( callback, dependencies, stepData, reduxStore ) {
	...
	if ( isSiteless ) {
		...
		return defer( () =>
			// TODO: save newSiteParams to the shopping cart
			processItemCart(
