...
import { isServerGeneratedBlogNameFlow } from '@automattic/onboarding';
...
export const getNewSiteParams = ( {
	...
	const selectedDesign = signupDependencies?.selectedDesign ?? false;
	...
	const newSiteParams = {
		...
		options: {
			...
			theme,
			use_theme_annotation: signupDependencies?.useThemeHeadstart ?? false,
	...
	if ( useAutoGeneratedBlogName ) {
		newSiteParams.blog_name = isServerGeneratedBlogNameFlow( flowToCheck )
			? siteTitle
			: siteTitle || getCurrentUserName( state ) || signupDependencies?.username || '';
...
export function maybeRemoveStepForUserlessCheckout( stepName, defaultDependencies, nextProps ) {
	...
	const cartItem = nextProps?.signupDependencies?.cartItem ?? false;
	const domainItem = nextProps?.signupDependencies?.domainItem ?? false;
