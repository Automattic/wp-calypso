...
describe('getUrlType', () => {
	test('should format absolute URLs as absolute URLs', () => {
		...
		expect(format('http://example.com/foo?b=a#z', 'ABSOLUTE')).toBe('http://example.com/foo?b=a#z');
		...
		expect(format('https://example.org/foo bar', 'ABSOLUTE')).toBe('https://example.org/foo%20bar');
	...
	test('should format scheme-relative URLs as scheme-relative URLs', () => {
		...
		expect(format('//example.com/foo?b=a#z', 'SCHEME_RELATIVE')).toBe('//example.com/foo?b=a#z');
		expect(format('//example.com/foo/bar/../baz', 'SCHEME_RELATIVE')).toBe('//example.com/foo/baz');
	});
