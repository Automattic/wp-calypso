...
const analytics = {
            ...
            if (process.env.NODE_ENV !== 'production') {
                for (const key in eventProperties) {
                    if (isObjectLike(eventProperties[key]) && typeof console !== 'undefined') {
                        console.error(
                            `Unable to record event "${eventName}" because nested properties are not supported by Tracks. Check '${key}' on`,
                            eventProperties
    ...
    statsd: {
        ...
        recordTiming: function(pageUrl, eventType, duration, triggerName) {
            ...
            if (config('boom_analytics_enabled')) {
                ...
                new Image().src = 'https://pixel.wp.com/boom.gif?v=calypso&u=' +
    ...
    ga: {
        ...
        recordEvent: function(category, action, label, value) {
            ...
            let debugText = 'Recording Event ~ [Category: ' +
                category +
                '] [Action: ' +
                action +
                ']';
