describe('safeImageUrl()', () => {
	...
	function commonTests() {
		...
		test('should make a non-wpcom http url safe', () => {
			expect(safeImageUrl('http://example.com/foo')).toEqual('https://i1.wp.com/example.com/foo');
		});
		...
		test('should promote an http wpcom url to https', () => {
			expect(safeImageUrl('http://files.wordpress.com/')).toEqual('https://files.wordpress.com/');
			expect(safeImageUrl('http://wordpress.com/')).toEqual('https://wordpress.com/');
		});

		test('should leave https wpcom url alone', () => {
			expect(safeImageUrl('https://files.wordpress.com/')).toEqual('https://files.wordpress.com/');
		...
		test('should promote an http gravatar url to https', () => {
			expect(safeImageUrl('http://files.gravatar.com/')).toEqual('https://files.gravatar.com/');
			expect(safeImageUrl('http://gravatar.com/')).toEqual('https://gravatar.com/');
		});

		test('should leave https gravatar url alone', () => {
			expect(safeImageUrl('https://files.gravatar.com/')).toEqual('https://files.gravatar.com/');
