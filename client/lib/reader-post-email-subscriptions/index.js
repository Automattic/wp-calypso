...
const PostEmailSubscriptionStore = {
    ...
    receiveUpdateDeliveryFrequencyResponse: function(action) {
        ...
        if (originalDeliveryFrequency) {
            if (
                updateSubscription(action.blog_id, {
                    delivery_frequency: originalDeliveryFrequency,
                })
            ) {
                PostEmailSubscriptionStore.emit('change');
            }
    ...
    receiveFeedSubscriptions: function(action) {
        let addedSubscriptionCount = 0, postEmailSubscription;

        if (!action.data || (action.data && (action.data.errors || !action.data.subscriptions))) {
            return;
        }
