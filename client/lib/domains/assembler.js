...
export function createDomainObjects( dataTransferObject ) {
	...
	domains = dataTransferObject.map( domain => {
		return {
			...
			name: domain.domain,
			owner: domain.owner,
			privacyProductAvailable: domain.private_registration_product_purchase_allowed,
			privacyIncludedByDefault: domain.whois_privacy_included,
			privacyEnabled: domain.whois_privacy_enabled,
