...
test( 'demonstrate poor handling of no-protocol', () => {
	expect( getSiteConnectInfo( '//example.com' )[ 0 ] ).toMatchSnapshot();
	expect( getSiteConnectInfo( 'example.com' )[ 0 ] ).toMatchSnapshot();
} );
...
test( 'received url should not overwrite filters with its own query 😱', () => {
	expect( getSiteConnectInfo( 'http://example.com/?filters=no-way' ) ).toEqual( [
		`${ apiPath }/http/example.com`,
		expectedParams,
	] );
} );
