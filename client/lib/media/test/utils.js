...
describe('MediaUtils', () => {
	...
	describe('#isExceedingSiteMaxUploadSize()', () => {
		...
		test('should return null if a video is being uploaded for a Jetpack site with VideoPress enabled', () => {
			expect(
				MediaUtils.isExceedingSiteMaxUploadSize({ size: 1024, mime_type: 'video/mp4' }, jetpackSite)
			).to.be.null;
		});
	...
	describe('#mediaURLToProxyConfig()', () => {
		...
		test('should detect domain mismatch', () => {
			expect(MediaUtils.mediaURLToProxyConfig('https://test.com/media.jpg', 'test2.com')).to.eql({
				query: '',
				filePath: '/media.jpg',
		...
		test('should not recognize non-photon wp.com URLs as ones relative to site URL', () => {
			expect(
				MediaUtils.mediaURLToProxyConfig('https://bad.wp.com/test.com/media.jpg?w=100', 'test.com')
			).to.eql({
				query: '?w=100',
		...
		test('should recognize domain mismatch in photon URL', () => {
			expect(
				MediaUtils.mediaURLToProxyConfig('https://i0.wp.com/test.com/media.jpg?w=100', 'test2.com')
			).to.eql({
				query: '?w=100',
				filePath: '/media.jpg',
				isRelativeToSiteRoot: false,
