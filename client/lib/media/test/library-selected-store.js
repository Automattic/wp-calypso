...
describe('MediaLibrarySelectedStore', () => {
	...
	describe('#get()', () => {
		test('should return a single value', () => {
			...
			expect(MediaLibrarySelectedStore.get(DUMMY_SITE_ID, DUMMY_TRANSIENT_MEDIA_OBJECT.ID)).to.eql(
				DUMMY_TRANSIENT_MEDIA_OBJECT
			);
		});

		test('should return undefined for an item that does not exist', () => {
			expect(MediaLibrarySelectedStore.get(DUMMY_SITE_ID, DUMMY_TRANSIENT_MEDIA_OBJECT.ID + 1)).to
				.be.undefined;
		});
	});
