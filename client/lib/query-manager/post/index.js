...
export default class PostQueryManager extends PaginatedQueryManager {
        ...
        return every(queryWithDefaults, (value, key) => {
            switch (key) {
                case 'search':
                    ...
                    return (post.title && includes(post.title.toLowerCase(), search)) ||
                ...
                case 'category': {
                    ...
                    return some(post[field], ({ name, slug }) => {
                        return (name && name.toLowerCase() === valueLowercase) ||
    ...
    compare(query, postA, postB) {
        ...
        switch (query.order_by) {
            ...
            case 'comment_count':
                order = get(postA.discussion, 'comment_count', 0) -
                    get(postB.discussion, 'comment_count', 0);
                break;
