...
describe('PaginatedQueryManager', () => {
	...
	describe('#receive()', () => {
		...
		test('should preserve existing pages when receiving items queried with different number', () => {
			...
			expect(manager.getItemsIgnoringPage({})).toEqual([{ ID: 144, changed: true }, { ID: 152 }]);
			expect(manager.getItems({ number: 1, page: 1 })).toEqual([{ ID: 144, changed: true }]);
			expect(manager.getItems({ number: 1, page: 2 })).toEqual([{ ID: 152 }]);
		});
		...
		test('should adjust for the difference in found after an item is removed', () => {
			...
			expect(manager.getItems({ search: 'title', number: 2, page: 3 })).toEqual([{ ID: 184 }]);
		});
		...
		test('should adjust for the difference in found after an item is added', () => {
			...
			expect(manager.getItems({ search: 'title', number: 2, page: 4 })).toEqual([{ ID: 184 }]);
		});
		...
		test('handles items missing from the last page', () => {
			...
			expect(manager.getItemsIgnoringPage({ search: 'title' })).toEqual([{ ID: 144 }, { ID: 152 }]);
		...
		test('handles items missing from the first page', () => {
			...
			expect(manager.getItemsIgnoringPage({ search: 'title' })).toEqual([{ ID: 1 }, { ID: 3 }]);
