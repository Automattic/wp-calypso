...
export class SyncHandler {
    ...
    syncHandlerWrapper(handler) {
        return (params, callback) => {
            ...
            const localResponseHandler = localRecord => {
                ...
                if (localRecord && localRecord.body) {
                    ...
                    try {
                        ...
                        debug(
                            'Callback failed with localRecord (%o), deleting record.',
                            localRecord,
                            error
                        );
                    }
            ...
            const networkFetch = localRecord => {
                return new Promise((resolve, reject) => {
                    handler(reqParams, (err, res) => {
                        ...
                        debug(
                            'server callback run => %o, params (%o), response (%o)',
                            path,
                            reqParams,
                            res
                        );
            ...
            request.then(adjustPagination, networkErrorHandler).then(cacheResponse); // => combinedResponse { localResponse, serverResponse } // => combinedResponse { localResponse, serverResponse }
        };
    }
