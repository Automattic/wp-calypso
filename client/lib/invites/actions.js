...
export function fetchInvite( siteId, inviteKey ) {
	...
	wpcom.undocumented().getInvite( siteId, inviteKey, ( error, data ) => {
		...
		if ( error ) {
			analytics.tracks.recordEvent( 'invite_validation_failure', {
				error: error.error,
			} );
		}
	} );
			...
			.usersNew(
				...
				( error, response ) => {
					...
					if ( error ) {
						...
						analytics.tracks.recordEvent( 'invite_account_creation_failed', {
							error: error.error,
						} );
					} else {
						analytics.tracks.recordEvent( 'invite_account_created' );
					}
	...
	return dispatch => {
		...
		wpcom.undocumented().acceptInvite( invite, ( error, data ) => {
			...
			if ( error ) {
				...
				analytics.tracks.recordEvent( 'invite_accept_failed', {
					error: error.error,
				} );
			} else {
				...
				analytics.tracks.recordEvent( 'invite_accepted' );
			}
	...
	return dispatch => {
		...
		wpcom.undocumented().sendInvites( siteId, usernamesOrEmails, role, message, ( error, data ) => {
			...
			if ( isErrored ) {
				...
				analytics.tracks.recordEvent( 'invite_send_failed' );
			} else {
				dispatch(
					successNotice(
				...
				analytics.tracks.recordEvent( 'invite_send_success' );
			}
		} );
	};
}
...
export function createInviteValidation( siteId, usernamesOrEmails, role ) {
	...
	wpcom.undocumented().createInviteValidation( siteId, usernamesOrEmails, role, ( error, data ) => {
		...
		if ( error ) {
			analytics.tracks.recordEvent( 'invite_create_validation_failed' );
		} else {
			analytics.tracks.recordEvent( 'invite_create_validation_success' );
		}
	} );
}
