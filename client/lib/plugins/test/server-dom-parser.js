...
import {
	...
	getSanitizeSectionRoot,
} from '../sanitize-section-content';
import { getServerRoot } from '../server-dom-parser';

const root = getServerRoot();
overrideSanitizeSectionRoot( root );
/**
...
test( 'should change the sanitization root object', () => {
	expect( root ).toBe( getSanitizeSectionRoot() );
} );
...
test( 'should prevent known XSS attacks', () => {
	...
	expect( clean( '<IMG """><SCRIPT>alert("XSS")</SCRIPT>">' ) ).toBe( 'alert("XSS")<img>">' );
