...
export function FollowingView( { connectionId, actor }: Props ) {
	...
	const {
		data: followsData,
		isPending: followsIsPending,
		isError: followsIsError,
		error: followsError,
		hasNextPage: followsHasNextPage,
		isFetchingNextPage: followsIsFetchingNextPage,
	...
	const query: SocialAccountListProps< AtmosphereScopedProfileSummary >[ 'query' ] = useMemo(
		() => ( {
			...
			isPending: followsIsPending,
			isError: followsIsError,
			error: projectAtmosphereError( followsError ),
			hasNextPage: followsHasNextPage,
			isFetchingNextPage: followsIsFetchingNextPage,
			fetchNextPage: () => {
