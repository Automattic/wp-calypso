...
export function FollowingView( { connectionId, actor }: Props ) {
	...
	const query: SocialAccountListProps< AtmosphereScopedProfileSummary >[ 'query' ] = useMemo(
		() => ( {
			...
			isPending: followsQuery.isPending,
			isError: followsQuery.isError,
			error: projectAtmosphereError( followsQuery.error ),
			hasNextPage: followsQuery.hasNextPage,
			isFetchingNextPage: followsQuery.isFetchingNextPage,
			...
			// type.
			fetchNextPage:
				followsQuery.fetchNextPage as unknown as SocialAccountListProps< AtmosphereScopedProfileSummary >[ 'query' ][ 'fetchNextPage' ],
			refetch:
				followsQuery.refetch as unknown as SocialAccountListProps< AtmosphereScopedProfileSummary >[ 'query' ][ 'refetch' ],
		} ),
		[
			followsQuery.data,
	...
	return (
		<ReaderMain className="atmosphere-view">
			<DocumentHead
				title={ String(
					translate( 'Following · @%(actor)s ‹ ATmosphere ‹ Reader', {
						args: { actor },
					} )
				) }
			/>
