...
- `sanitizePostHtml` — DOMPurify wrapper with allow-list (`<p> <br> <a>` / `href, rel, target, data-id`, `ALLOW_DATA_ATTR: false`) and a global `afterSanitizeAttributes` hook that forces `target="_blank"` and `rel="nofollow noopener noreferrer"` on every surviving anchor (post body and bio alike — `SocialProfileCard` reuses the hook via the exported `ensureSanitizerHook`). The `data-id` attribute is carried by @-mention anchors so `<PostCardBody>` can route mentions in-app via `getProfileUrl` without parsing the href.
...
- A single global `afterSanitizeAttributes` hook (registered by `sanitize-post-html.ts` and reused by `profile-card.tsx` via the exported `ensureSanitizerHook`) forces `target="_blank"` and `rel="nofollow noopener noreferrer"` on every surviving `<a>`. Belt-and-suspenders against window-opener leaks and against upstream HTML that omits `target`. Pre-existing rel tokens (e.g. `rel="me"` for IndieAuth-style verification) are preserved.
