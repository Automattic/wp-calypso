...
Follows are fully on React Query:
...
- Calypso consumers should use `calypso/reader/data/follows` for hooks such as `useFollows()`, follow/unfollow mutations, delivery-setting mutations, followed-site selectors, and organization feed info.
- Do not reintroduce `state.reader.follows`, `calypso/state/reader/follows`, `SyncReaderFollows`, or `/read/following/mine*` data-layer handlers. For cross-feature refreshes, invalidate `getFollowsQueryKey()` on the active `QueryClient`.
