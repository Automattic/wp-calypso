import { getFollowsFromData } from '@automattic/api-queries';
...
const SubscribeModal: React.FC< SubscribeModalProps > = ( { isOpen, onClose } ) => {
	...
	const handleFollowToggle = useCallback(
		async ( site: CardData, following: boolean ) => {
			const isFollowingSite = ( site: CardData, followItems: FollowItem[] ) =>
				followItems.some(
			...
			if ( following === isFollowingSite( site, follows ) ) {
				return;
			}
			...
			for ( let attempt = 0; attempt < MAX_RETRIES; attempt++ ) {
				...
				const result = await refetchFollows();

				// Delay the next attempt.
				await new Promise( ( resolve ) => setTimeout( resolve, 300 ) );

				if ( following === isFollowingSite( site, getFollowsFromData( result.data ) ) ) {
					return;
				}
			}
		},
		[ follows, refetchFollows ]
	);
