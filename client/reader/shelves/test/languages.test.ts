import {
	SHELF_LANGUAGE_OPTIONS,
	SHELF_LANGUAGE_SUGGESTIONS,
...
describe( 'shelves languages helpers', () => {
	...
	describe( 'the base-language list', () => {
		it( 'collapses regional variants onto one base option with the base name', () => {
			const pt = SHELF_LANGUAGE_OPTIONS.filter( ( option ) => option.code === 'pt' );
		...
		it( 'has no duplicate codes', () => {
			const codes = SHELF_LANGUAGE_OPTIONS.map( ( option ) => option.code );
			expect( new Set( codes ).size ).toBe( codes.length );
		} );

		it( 'is sorted by display name', () => {
			const names = SHELF_LANGUAGE_OPTIONS.map( ( option ) => option.name );
			expect( names ).toEqual( [ ...names ].sort( ( a, b ) => a.localeCompare( b ) ) );
		} );

		it( 'exposes the names as suggestions', () => {
			expect( SHELF_LANGUAGE_SUGGESTIONS ).toContain( 'English' );
			expect( SHELF_LANGUAGE_SUGGESTIONS ).toContain( 'Português' );
		} );
	} );
