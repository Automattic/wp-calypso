...
import { ShelfColorPicker } from '../color-picker';

describe( 'ShelfColorPicker', () => {
	it( 'renders a swatch for every color and checks the selected one', () => {
		renderWithProvider( <ShelfColorPicker value="green" onChange={ jest.fn() } /> );
	...
	it( 'reports the chosen color', async () => {
		...
		renderWithProvider( <ShelfColorPicker value="blue" onChange={ onChange } /> );
	...
	it( 'adds a "None" swatch and checks it when selected', () => {
		renderWithProvider( <ShelfColorPicker value="none" onChange={ jest.fn() } allowNone /> );
	...
	it( 'reports "none" when the None swatch is chosen', async () => {
		const user = userEvent.setup();
		const onChange = jest.fn();
		renderWithProvider( <ShelfColorPicker value="blue" onChange={ onChange } allowNone /> );
