...
import { ShelvesOnboardingModal } from '../index';
...
function setup() {
	...
	renderWithProvider( <ShelvesOnboardingModal onProceed={ onProceed } onClose={ onClose } /> );
	return { onProceed, onClose, user };
}

describe( 'ShelvesOnboardingModal', () => {
	beforeEach( () => {
	...
	it( 'opens on the welcome step with Skip and no Back', () => {
		...
		expect( screen.getByRole( 'heading', { name: 'Meet Shelves' } ) ).toBeVisible();
	...
	it( 'uses a single dialog label id while steps are visible', async () => {
		...
		expect( screen.getByRole( 'dialog', { name: 'Set up Shelves' } ) ).toBeVisible();
		expect( document.querySelectorAll( '#reader-shelves-onboarding__heading' ) ).toHaveLength( 1 );
	...
	it( 'records a step_viewed event for each step reached', async () => {
		...
		expect( mockRecordReaderTracksEvent ).toHaveBeenCalledWith(
			'calypso_reader_shelves_onboarding_step_viewed',
	...
	it( 'records a skipped event with the step it was dismissed on', async () => {
		...
		expect( mockRecordReaderTracksEvent ).toHaveBeenCalledWith(
			'calypso_reader_shelves_onboarding_skipped',
			{ step: 1, step_name: 'explain' }
		);
	} );
