 ...
 */
import { readShelfQuery, readShelvesQuery } from '@automattic/api-queries';
import { QueryClient } from '@tanstack/react-query';
import { screen, waitFor, within } from '@testing-library/react';
import userEvent from '@testing-library/user-event';
import nock from 'nock';
import { renderWithProvider } from 'calypso/test-helpers/testing-library';
import { CreateShelfModal } from '../index';
...
// adapted shelf carries it through to the cache and callbacks.
...
function setup( {
	existing = [] as ReadShelf[],
	onCreated = jest.fn(),
	localeSlug,
}: { existing?: ReadShelf[]; onCreated?: jest.Mock; localeSlug?: string } = {} ) {
	const queryClient = new QueryClient();
	queryClient.setQueryData( readShelvesQuery().queryKey, existing );
	const onClose = jest.fn();
	const user = userEvent.setup();
	renderWithProvider( <CreateShelfModal isOpen onClose={ onClose } onCreated={ onCreated } />, {
		queryClient,
...
describe( 'CreateShelfModal', () => {
	...
	it( 'renders nothing when closed', () => {
		renderWithProvider( <CreateShelfModal isOpen={ false } onClose={ jest.fn() } /> );
	...
	it( 'steps through the identity, layout, feeds and topics sections', async () => {
		...
		const dialog = screen.getByRole( 'dialog', { name: 'Create a new shelf' } );
		...
		expect(
			within( dialog ).getByText(
				'Pick the subscriptions whose posts make up this shelf’s main feed.'
			)
		).toBeVisible();
	...
	it( 'creates a shelf with identity and layout settings, updates the caches, and closes', async () => {
		mockSubscriptions = [ mockSubscription ];
		...
		const shelves = queryClient.getQueryData< ReadShelf[] >( readShelvesQuery().queryKey );
		...
		expect( shelves?.[ 0 ] ).not.toHaveProperty( 'sources' );
		expect( shelves?.[ 0 ] ).not.toHaveProperty( 'tags' );
		const detail = queryClient.getQueryData< ReadShelfDetails >( readShelfQuery( '7' ).queryKey );
		expect( detail?.layout.view ).toBe( 'legacy' );
		expect( onClose ).toHaveBeenCalled();
		expect( mockRecordReaderTracksEvent ).toHaveBeenCalledWith(
			'calypso_reader_shelves_shelf_created',
			{
				tag_count: 0,
				language_count: 0,
				source_count: 1,
	...
	it( 'notifies the parent with the created shelf', async () => {
	...
	it( 'closes without creating when Cancel is clicked', async () => {
		...
		expect( onClose ).toHaveBeenCalled();
		expect( queryClient.getQueryData< ReadShelf[] >( readShelvesQuery().queryKey ) ).toEqual( [] );
	} );
} );
