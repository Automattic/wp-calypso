...
function GalleryCard( {
	...
	return (
		<VStack
			className="shelf-feed-gallery__card"
			spacing={ 1.5 }
			alignment="stretch"
			data-selected={ isSelected || undefined }
		>
			<a
				className="shelf-feed-gallery__thumb"
			...
			>
				{ fields.imageUrl ? (
					<img
						className="shelf-feed-gallery__image"
						src={ fields.imageUrl }
						alt=""
						loading="lazy"
					/>
			...
			<VStack spacing={ 4 }>
				<VStack spacing={ 1 }>
					<HStack
						className="shelf-feed-gallery__meta"
						spacing={ 2 }
						alignment="center"
						justify="flex-start"
					>
						...
						{ showTimestamp && fields.publishedDate && (
							<>
								...
								<span className="shelf-feed-gallery__time"></span>
							</>
						) }
					</HStack>
					<h3 className="shelf-feed-gallery__title">
						<a
							className="shelf-feed-gallery__title-link"
							href={ fields.postHref }
							onClick={ onOpen }
						>
							{ fields.title }
...
function GallerySkeletonCard() {
	return (
		<VStack className="shelf-feed-gallery__card" spacing={ 1.5 } alignment="stretch">
			<span className="shelf-feed-gallery__thumb">
				<Shimmer className="shelf-feed-gallery__image" />
			</span>
			<VStack spacing={ 1 }>
				<Shimmer className="shelf-feed-gallery__skeleton-line is-meta" />
				<Shimmer className="shelf-feed-gallery__skeleton-line" />
				<Shimmer className="shelf-feed-gallery__skeleton-line is-short" />
			</VStack>
		</VStack>
	);
}
...
export function GalleryLayout( {
	...
	return (
		<div { ...getListProps( { className: 'shelf-feed-gallery' } ) }>
			{ items.map( ( virtualRow ) => (
				<div
					key={ virtualRow.key }
					data-index={ virtualRow.index }
					ref={ measureElement }
					className="shelf-feed-gallery__item"
					style={ { transform: `translateY(${ virtualRow.start - scrollMargin }px)` } }
				>
					<div
						className="shelf-feed-gallery__row"
...
export function GallerySkeleton( { count }: ShelfFeedSkeletonProps ) {
	const columns = useGalleryColumns();
	return (
		<div
			className="shelf-feed-gallery__skeleton"
			style={ { gridTemplateColumns: `repeat(${ columns }, 1fr)` } }
			aria-hidden="true"
		>
