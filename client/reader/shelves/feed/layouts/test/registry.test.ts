...
import { DEFAULT_SHELF_FEED_LAYOUT, getLayout } from '../registry';
...
describe( 'getLayout', () => {
	...
	it( 'falls back to the standard list when the layout is undefined', () => {
		...
		expect( DEFAULT_SHELF_FEED_LAYOUT ).toBe( 'standard-list' );
	} );

	it( 'falls back to the standard list for an unknown layout value', () => {
		// A shelf whose `feed` names a layout that isn't built yet still renders.
