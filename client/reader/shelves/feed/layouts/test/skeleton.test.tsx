...
describe( 'layout skeletons', () => {
	it( 'renders the requested number of placeholder rows', () => {
		...
		expect( container.querySelectorAll( '.shelf-feed-standard-list__skeleton-row' ) ).toHaveLength(
			4
		);
	} );
	...
	it( 'renders the requested number of placeholder cards', () => {
		...
		expect( container.querySelectorAll( '.shelf-feed-board__card' ) ).toHaveLength( 5 );
	} );
} );
