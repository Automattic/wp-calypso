...
/**
 * The contract every shelf-feed layout implements. The shell owns the data and
 ...
 */
export interface ShelfFeedLayoutProps {
	...
	loadMore: () => void;
	/** Stable key (`${shelfId}:${layout}`) for saving/restoring scroll on Back. */
	restoreKey: string;
 ...
 */
export interface ShelfFeedSkeletonProps {
	count: number;
}
