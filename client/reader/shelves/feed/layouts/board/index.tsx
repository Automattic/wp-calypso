...
function BoardSkeletonCard() {
	return (
		<div className="shelf-feed-board__card">
			<Shimmer className="shelf-feed-board__skeleton-hero" />
			<div className="shelf-feed-board__body">
				<Shimmer className="shelf-feed-board__skeleton-line is-title" />
				<Shimmer className="shelf-feed-board__skeleton-line" />
				<Shimmer className="shelf-feed-board__skeleton-line is-short" />
			</div>
		</div>
	);
}
...
function BoardCard( {
	...
	return (
		<div className="shelf-feed-board__card">
			<div className="shelf-feed-board__hero">
				{ fields.imageUrl ? (
					<img className="shelf-feed-board__image" src={ fields.imageUrl } alt="" loading="lazy" />
				) : (
					<SiteIcon iconUrl={ fields.siteIconUrl } size={ 40 } />
				) }
			</div>
			<div className="shelf-feed-board__body">
				<h3 className="shelf-feed-board__title">
					<a className="shelf-feed-board__title-link" href={ fields.postHref } onClick={ onOpen }>
						{ fields.title }
					</a>
				</h3>
				{ fields.excerptHtml && (
				...
				<div className="shelf-feed-board__footer">
					<span className="shelf-feed-board__source-group">
						...
						<span className="shelf-feed-board__source">{ fields.sourceName }</span>
					</span>
					{ showTimestamp && fields.publishedDate && (
						<span className="shelf-feed-board__time">
							<ShelfFeedTimeSince date={ fields.publishedDate } />
						</span>
					) }
				</div>
				<div className="shelf-feed-board__actions">
					<ReaderPostActions
						post={ post }
						onCommentClick={ () => {
							onOpen();
...
export function BoardLayout( {
	...
	return (
		<div { ...getListProps( { className: 'shelf-feed-board' } ) }>
			{ items.map( ( virtualItem ) => (
				<div
					...
					ref={ measureElement }
					className="shelf-feed-board__item"
					style={ {
...
export function BoardSkeleton( { count }: ShelfFeedSkeletonProps ) {
	return (
		<div className="shelf-feed-board__skeleton" aria-hidden="true">
