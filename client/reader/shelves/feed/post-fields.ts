 ...
 */
export type ShelfFeedDayGroup = 'today' | 'yesterday' | 'earlier' | 'older' | '';
 ...
 */
export interface ShelfFeedPostFields {
	title: string;
...
function dayGroupOf( dateIso?: string ): ShelfFeedDayGroup {
	if ( ! dateIso ) {
		return '';
	}
...
export function getPostDayGroup( post: ReadStreamPost ): ShelfFeedDayGroup {
...
export function getPostFields( post: NormalizedPost ): ShelfFeedPostFields {
	return {
