...
import { ShelfFeedSourceNotice } from './components/source-notice';
import {
	ShelfFeedEmpty,
	ShelfFeedError,
	ShelfFeedLoading,
	ShelfFeedLoadingMore,
} from './components/states';
import {
	DEFAULT_SHELF_FEED_LAYOUT,
	getLayout,
	getLayoutInlineLoadMore,
	getLayoutPageSize,
	getLayoutSkeleton,
} from './layouts/registry';
import type { ReadShelf, ReadStreamPost } from '@automattic/api-core';

import './style.scss';

interface Props {
	// The shelf, already resolved by the view (list summary or by-slug detail). Only
	// its `id` (keys the streams) and `layout` (selects the layout) are read here, so
	// the summary shape is enough — no re-fetch by id.
	shelf: ReadShelf;
	// Retry the shelf detail; backs the stream error state's retry alongside the
	// stream's own refetch. Owned by the view (the by-slug query's `refetch`).
	onRetryShelf?: () => void;
	// Which per-shelf stream to render: the posts feed (`shelf:<id>`, followed
	// feeds + tags) or Discover (`shelf_discover:<id>`, recommended on-topic posts
...
/**
 * A shelf's stream surface, shared by both tabs. The `variant` selects the
 * per-shelf stream — the posts feed (`/reader/shelves/<id>/posts`, keyed
 * `shelf:<id>`, built server-side from the shelf's followed feeds and tags) or
 * Discover (`/reader/shelves/<id>/discover`, keyed `shelf_discover:<id>`,
 * recommended on-topic posts the user doesn't follow). The shelf detail is resolved
 * once by the view (from the URL slug) and passed in: its numeric `id` keys the
 * stream (`shelf:<id>`) and `layout.view` selects the layout. Both variants share
 * the same layouts.
 */
export function ShelfFeed( { shelf, onRetryShelf, variant = 'feed', onAddSources }: Props ) {
	const shelfId = shelf.id;
	const layout = shelf.layout.view ?? DEFAULT_SHELF_FEED_LAYOUT;

	// The Shelf's own stream. Keyed by the route's `shelfId` (not gated on the
	...
	const streamKey = isDiscover ? `shelf_discover:${ shelfId }` : `shelf:${ shelfId }`;
		...
		( post: ReadStreamPost ) => {
			...
			dispatch(
				recordReaderTracksEvent(
					...
					{ shelf_id: shelfId, layout, variant },
					{ post }
				)
			);
		},
		[ dispatch, selectPostKey, stream.items, shelfId, layout, variant ]
	);
	...
	const renderBody = () => {
		...
		if ( isLegacy ) {
			return (
				<Layout
					...
					isLoadingMore={ false }
					loadMore={ () => {} }
					restoreKey={ `${ shelfId }:${ variant }:${ layout }` }
					isPostSelected={ isPostSelected }
					selectPost={ selectPost }
					showTimestamp={ ! isDiscover }
					emptyContent={ <ShelfFeedEmpty variant={ variant } onAddSources={ onAddSources } /> }
				/>
			);
		}
		// The feed is driven by the posts stream, which loads in parallel with the
		// shelf detail — it never waits on the detail to show posts.
		if ( stream.isLoading ) {
			if ( ! Skeleton ) {
				return <ShelfFeedLoading />;
			}
			return (
		...
		if ( posts.length === 0 ) {
			return <ShelfFeedEmpty variant={ variant } onAddSources={ onAddSources } />;
		}
		return (
			<Layout
				posts={ posts }
	...
	return (
		<div
			className={ clsx(
				'shelf-feed',
				shelf.layout.color !== 'none' && `shelf-feed--${ shelf.layout.color }`
			) }
		>
			...
			{ ! isDiscover && <ShelfFeedSourceNotice failedCount={ 0 } /> }
			<div className="shelf-feed__viewport" ref={ setViewport }>
				{ renderBody() }
				{ showLoadingMore &&
					( Skeleton ? (
