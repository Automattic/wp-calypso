...
function makeShelf( id: string, name: string, view: ShelfFeedLayout ): ReadShelfDetails {
	return {
		id,
...
const WORK = makeShelf( 'work-id', 'Work', 'standard-list' );

// ShelfFeed receives the already-resolved shelf detail as a prop (the view resolves
// it once by slug), so tests just pass the shelf — no query cache to seed.
function render( shelf: ReadShelfDetails ) {
	return renderWithProvider( <ShelfFeed shelf={ shelf } />, {
...
describe( 'ShelfFeed', () => {
	beforeEach( () => {
		window.history.replaceState( {}, '', '/reader/shelves/work-id' );
	...
	it( 'shows an error with a retry that refetches the stream and the shelf', async () => {
		const user = userEvent.setup();
		const refetch = jest.fn();
		const onRetryShelf = jest.fn();
		mockUseInfiniteStream.mockReturnValue(
			streamResult( { error: new Error( 'boom' ), refetch } )
		);
		renderWithProvider( <ShelfFeed shelf={ WORK } onRetryShelf={ onRetryShelf } />, {
			queryClient: new QueryClient( { defaultOptions: { queries: { retry: false } } } ),
			initialState: { currentUser: { id: 1 } },
		} );
		...
		expect( onRetryShelf ).toHaveBeenCalled();
	} );

	it( 'enables the posts stream immediately from the passed shelf', () => {
		render( WORK );

		expect( mockUseInfiniteStream ).toHaveBeenCalledWith(
			expect.objectContaining( {
				streamKey: `shelf:${ WORK.id }`,
	...
	it( 'shows an actionable empty state with an Add feeds CTA when the feed has no posts', async () => {
		...
		renderWithProvider( <ShelfFeed shelf={ WORK } onAddSources={ onAddSources } />, {
			queryClient,
			initialState: { currentUser: { id: 1 } },
		} );
	...
	it( 'shows the Add feeds CTA in the legacy layout empty state', async () => {
		const user = userEvent.setup();
		const onAddSources = jest.fn();
		const legacy = makeShelf( 'work-id', 'Work', 'legacy' );
		const queryClient = new QueryClient( { defaultOptions: { queries: { retry: false } } } );

		renderWithProvider( <ShelfFeed shelf={ legacy } onAddSources={ onAddSources } />, {
			queryClient,
			initialState: { currentUser: { id: 1 } },
		} );
	...
	it( 'shows the Discover empty state without an Add feeds CTA', () => {
		...
		renderWithProvider(
			<ShelfFeed shelf={ WORK } variant="discover" onAddSources={ jest.fn() } />,
	...
	it( 'requests the shelf posts stream keyed by the shelf id', () => {
		render( WORK );

		expect( mockUseInfiniteStream ).toHaveBeenCalledWith(
			expect.objectContaining( { streamKey: `shelf:${ WORK.id }` } )
		);
	} );
	...
	it( 'passes the non-default locale to the stream request', () => {
		...
		renderWithProvider( <ShelfFeed shelf={ WORK } />, {
			queryClient,
			initialState: { currentUser: { id: 1 } },
		} );
	...
	it( 'stores the full stream item when selecting a post', async () => {
		...
		expect(
			queryClient.getQueryData( [ 'read', 'stream', 'selected', `shelf:${ WORK.id }`, null ] )
		).toEqual( streamItem );
	} );
	...
	it( 'records a tracks event when a post is opened', async () => {
		...
		expect( mockRecordReaderTracksEvent ).toHaveBeenCalledWith(
			'calypso_reader_shelves_post_opened',
			{ shelf_id: WORK.id, layout: 'standard-list', variant: 'feed' },
			{ post }
		);
	} );

	it( 'requests the discover stream keyed by the shelf id for the discover variant', () => {
		const queryClient = new QueryClient( { defaultOptions: { queries: { retry: false } } } );

		renderWithProvider( <ShelfFeed shelf={ WORK } variant="discover" />, {
			queryClient,
			initialState: { currentUser: { id: 1 } },
		} );

		expect( mockUseInfiniteStream ).toHaveBeenCalledWith(
			expect.objectContaining( { streamKey: `shelf_discover:${ WORK.id }` } )
		);
	} );
	...
	it( 'renders the layout selected by the shelf layout view', () => {
		mockUseInfiniteStream.mockReturnValue(
			streamResult( { pages: [ { posts: [ makePost() ] } as unknown as ReadStreamResponse ] } )
		);
		const { container } = render( makeShelf( 'work-id', 'Work', 'gallery' ) );

		expect( container.querySelector( '.shelf-feed-gallery' ) ).toBeInTheDocument();
		expect( container.querySelector( '.shelf-feed-standard-list' ) ).not.toBeInTheDocument();
	} );

	it( 'falls back to the default layout when the shelf has no view', () => {
		mockUseInfiniteStream.mockReturnValue(
			streamResult( { pages: [ { posts: [ makePost() ] } as unknown as ReadStreamResponse ] } )
		);
		const { layout, ...shelfWithoutView } = WORK;
		// No `layout.view` on the shelf → the default (standard-list) layout renders.
		const { container } = render( {
			...shelfWithoutView,
			layout: { color: layout.color, icon: layout.icon },
		} );

		expect( container.querySelector( '.shelf-feed-standard-list' ) ).toBeInTheDocument();
		expect( container.querySelector( '.shelf-feed-gallery' ) ).not.toBeInTheDocument();
	} );

	it( 'shows the empty state for the legacy layout when the legacy stream has no posts', () => {
		render( makeShelf( 'work-id', 'Work', 'legacy' ) );
