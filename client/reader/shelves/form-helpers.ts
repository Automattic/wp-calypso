...
type TranslateFn = LocalizeProps[ 'translate' ];
...
/** Validate the selected feed count against the server-enforced limit. */
export function validateFeeds( count: number, translate: TranslateFn ): string | null {
	if ( count > MAX_SHELF_FEEDS ) {
		return translate( 'You can add up to %(max)d feeds.', {
			args: { max: MAX_SHELF_FEEDS },
		} ) as string;
	}
	return null;
}

/** Validate the selected tag count against the server-enforced limit. */
export function validateTags( count: number, translate: TranslateFn ): string | null {
	if ( count > MAX_SHELF_TAGS ) {
		return translate( 'You can add up to %(max)d tags.', {
			args: { max: MAX_SHELF_TAGS },
		} ) as string;
	}
	return null;
}

/** Validate the selected language count against the server-enforced limit. */
export function validateLanguages( count: number, translate: TranslateFn ): string | null {
	if ( count > MAX_SHELF_LANGUAGES ) {
		return translate( 'You can add up to %(max)d languages.', {
			args: { max: MAX_SHELF_LANGUAGES },
		} ) as string;
	}
	return null;
}
...
export function getShelfErrorMessage( error: unknown, translate: TranslateFn ): string {
	...
	switch ( code ) {
		...
		case 'reader_shelves_title_too_long':
			return translate( 'The name must be %d characters or fewer', {
				args: [ MAX_SHELF_NAME_LENGTH ],
			} ) as string;
		case 'reader_shelves_too_many_feeds':
			return translate( 'You can add up to %(max)d feeds.', {
				args: { max: MAX_SHELF_FEEDS },
			} ) as string;
		case 'reader_shelves_too_many_tags':
			return translate( 'You can add up to %(max)d tags.', {
				args: { max: MAX_SHELF_TAGS },
			} ) as string;
		case 'reader_shelves_too_many_languages':
			return translate( 'You can add up to %(max)d languages.', {
				args: { max: MAX_SHELF_LANGUAGES },
			} ) as string;
		case 'reader_shelves_rate_limited':
			return translate(
