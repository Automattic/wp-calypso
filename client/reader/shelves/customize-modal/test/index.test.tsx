...
// The modal refetches the shelf by slug on open (`refetchOnMount: 'always'`) and
// seeds the draft from that fresh detail, so the by-slug GET must resolve. Echo
// the shelf's fields back in the wire (detail) shape.
function mockShelfBySlugEndpoint( shelf: ReadShelfDetails = SHELF ) {
	return (
		nock( 'https://public-api.wordpress.com' )
			// Mirror the fetcher's path encoding so the matcher holds for any slug.
			.get(
				`/wpcom/v2/reader/shelves/slug/${ encodeURIComponent(
					canonicalizeReadShelfSlug( shelf.slug )
				) }`
			)
			.reply( 200, {
				id: Number( shelf.id ),
				...
				layout: shelf.layout,
				tags: shelf.tags,
				languages: shelf.languages,
				follows: shelf.sources.map( ( source ) => ( {
...
function mockUpdateEndpoint( onBody?: ( body: Record< string, unknown > ) => void ) {
	return nock( 'https://public-api.wordpress.com' )
		.put( '/wpcom/v2/reader/shelves/7' )
		.reply( 200, ( _uri, body: Record< string, unknown > ) => {
			onBody?.( body );
			const title = body.title ?? SHELF.name;
			return {
				id: 7,
				// The server re-derives the slug from the title, so echo that here.
				slug: String( title ).toLowerCase().replace( /\s+/g, '-' ),
				title,
				layout: body.layout ?? SHELF.layout,
				follows: [],
				tags: body.tags ?? SHELF.tags,
				languages: body.languages ?? SHELF.languages,
			};
		} );
}
...
function mockDeleteEndpoint() {
	return nock( 'https://public-api.wordpress.com' )
		.delete( '/wpcom/v2/reader/shelves/7' )
...
function render( {
	...
	queryClient.setQueryData( readShelvesQuery().queryKey, [ summary, ...others ] );
	// The modal resolves the shelf by slug (sharing the view's cache); the id-keyed
	...
	mockShelfBySlugEndpoint( shelf );

	const view = renderWithProvider(
		<CustomizeModal isOpen slug={ shelf.slug } onClose={ onClose } />,
		{
			queryClient,
...
describe( 'CustomizeModal', () => {
	...
	it( 'seeds the identity fields from the shelf detail', async () => {
		render();
		...
		expect(
			screen.getByText( 'Changes the color of post titles and actions in this shelf.' )
		).toBeVisible();
	...
	it( 'switches between the Identity, Layout, Feeds, Topics and Delete tabs', async () => {
		...
		expect( screen.queryByRole( 'button', { name: 'Delete shelf' } ) ).not.toBeInTheDocument();
		...
		expect( screen.getByRole( 'button', { name: 'Delete shelf' } ) ).toBeVisible();
	} );
	...
	it( 'seeds saved tags from the fresh detail even when the cached snapshot has none', async () => {
		...
		const queryClient = new QueryClient( { defaultOptions: { queries: { retry: false } } } );
		const { sources, tags, languages, ...summary } = SHELF;
		queryClient.setQueryData( readShelvesQuery().queryKey, [ summary ] );
		...
		queryClient.setQueryData( readShelfBySlugQuery( SHELF.slug ).queryKey, {
			...SHELF,
			tags: [],
		} );
		mockShelfBySlugEndpoint( SHELF );

		renderWithProvider( <CustomizeModal isOpen slug={ SHELF.slug } onClose={ jest.fn() } />, {
			queryClient,
			initialState: { currentUser: { id: 1 } },
		} );
	...
	it( 'does not seed stale cached detail when the open-time refetch fails', async () => {
		...
		const failedRefetch = nock( 'https://public-api.wordpress.com' )
			.get( `/wpcom/v2/reader/shelves/slug/${ SHELF.slug }` )
			.reply( 404, { error: 'reader_shelves_not_found' } );
		...
		await waitFor( () => {
			const queryState = queryClient.getQueryState( readShelfBySlugQuery( SHELF.slug ).queryKey );
	...
	it( 'defaults an unset width to Wide and lets the user switch to Regular', async () => {
		...
		// The shelf has no stored width, so it seeds to the Wide default.
	...
	it( 'seeds the width control from the stored layout width', async () => {
		const user = userEvent.setup();
		render( { shelf: { ...SHELF, layout: { ...SHELF.layout, width: 'regular' } } } );
		await screen.findByLabelText( 'Name' );
	...
	it( 'allows keeping the current name but blocks a name that collides with another shelf', async () => {
		const user = userEvent.setup();
		render( {
			others: [
		...
		expect( await screen.findByText( 'A shelf with this name already exists' ) ).toBeVisible();
		expect( screen.getByRole( 'button', { name: 'Save changes' } ) ).toBeDisabled();
	} );

	it( 'deletes the shelf after confirming, then navigates away and closes', async () => {
		const user = userEvent.setup();
		const { queryClient, onClose } = render();
		mockDeleteEndpoint();
		await screen.findByLabelText( 'Name' );
		...
		const dialog = screen.getByRole( 'dialog', { name: 'Delete shelf' } );
		await user.click( within( dialog ).getByRole( 'button', { name: 'Delete shelf' } ) );

		await waitFor( () => expect( onClose ).toHaveBeenCalled() );

		expect( page ).toHaveBeenCalledWith( '/reader' );
		const list = queryClient.getQueryData< ReadShelf[] >( readShelvesQuery().queryKey );
		expect( list ).toEqual( [] );
	} );
} );
