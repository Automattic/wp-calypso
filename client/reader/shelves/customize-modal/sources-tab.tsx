...
export function SourcesTab( { selectedSourceKeys, onAddDraftSource, onRemoveDraftSource }: Props ) {
	...
	return (
		<VStack spacing={ 4 } justify="flex-start" className="shelf-sources">
			<p className="customize-shelf-modal__field-help">
				{ translate( 'Pick the subscriptions whose posts make up this shelf’s main feed.' ) }
			</p>
			<SearchControl
				...
				className="shelf-sources__search"
			/>

			<HStack justify="flex-start" spacing={ 2 } className="shelf-sources__filters">
				<Button
...
function SourcesTabContent( {
	...
	switch ( state ) {
		...
		case 'error':
			return (
				<p className="shelf-sources__empty" role="alert">
		...
		case 'empty':
			return (
				...
				<p className="shelf-sources__empty" role="status">
					{ filter === 'selected'
						? translate( 'No subscriptions added to this shelf yet.' )
...
function SourcesVirtualList( {
	...
	return (
		<div className="shelf-sources__list" ref={ setScrollElement }>
			<div { ...getListProps( { className: 'shelf-sources__virtualized-list' } ) } role="list">
				{ items.map( ( virtualRow ) => {
					...
					return (
						<div
							...
							ref={ measureElement }
							role="presentation"
							className="shelf-sources__virtual-row"
		...
		>
			{ Array.from( { length: 6 }, ( _value, index ) => (
				<HStack
					...
					className="shelf-sources__skeleton-row"
					style={ { minHeight: 56 } }
				>
					<Skeleton shape="circle" width="40px" height="40px" />
					<VStack spacing={ 2 } className="shelf-sources__skeleton-text">
