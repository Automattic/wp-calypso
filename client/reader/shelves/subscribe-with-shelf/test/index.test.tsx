...
import { SubscribeWithShelfButton } from '../index';
...
jest.mock( '@automattic/calypso-config', () => {
	...
	const isEnabled = ( flag: string ) => ( flag === 'reader/shelves' ? mockFlagEnabled : false );
	return {
		__esModule: true,
...
const mockUpdateShelf = jest.fn().mockResolvedValue( undefined );
let mockShelves: ReadShelf[] = [];
let mockDetailsById: Record< string, ReadShelfDetails | undefined > = {};
let mockDetailsLoading = false;
let mockDetailsError = false;
jest.mock( 'calypso/reader/data/shelves', () => ( {
	useShelves: () => mockShelves,
	useShelvesDetails: () => ( {
		byId: mockDetailsById,
		isLoading: mockDetailsLoading,
		isError: mockDetailsError,
	} ),
	useUpdateShelf: () => ( { mutateAsync: mockUpdateShelf, isPending: false } ),
} ) );
...
beforeEach( () => {
	...
	mockShelves = [ shelf ];
	mockDetailsById = { [ shelf.id ]: { ...shelf, sources: [], tags: [], languages: [] } };
	mockDetailsLoading = false;
	mockDetailsError = false;
	mockFollowSite.mockClear();
	mockUpdateShelf.mockClear();
	mockRecordReaderTracksEvent.mockClear();
} );

const openPicker = async ( user: ReturnType< typeof userEvent.setup > ) => {
	await user.click( screen.getByRole( 'button', { name: 'Move site to a shelf' } ) );
	return screen.findByRole( 'dialog', { name: 'Move site to a shelf' } );
};

it( 'shows a loading skeleton while the shelf details are loading', async () => {
	mockDetailsLoading = true;
	const user = userEvent.setup();
	renderWithProvider( <SubscribeWithShelfButton { ...props } /> );

	await openPicker( user );

	expect( screen.getByRole( 'status', { name: 'Loading your shelves' } ) ).toBeVisible();
	// The interactive shelf rows are not rendered while loading.
...
it( 'renders only the plain follow button when the flag is off', () => {
	...
	expect(
		screen.queryByRole( 'button', { name: 'Move site to a shelf' } )
	).not.toBeInTheDocument();
} );

it( 'renders the shelves button alongside subscribe when the flag is on', () => {
	renderWithProvider( <SubscribeWithShelfButton { ...props } /> );

	expect( screen.getByRole( 'button', { name: 'Subscribe' } ) ).toBeVisible();
	expect( screen.getByRole( 'button', { name: 'Move site to a shelf' } ) ).toBeVisible();
} );

it( 'subscribes and opens the picker when the shelves button is clicked', async () => {
	const user = userEvent.setup();
	renderWithProvider( <SubscribeWithShelfButton { ...props } /> );
...
it( 'tracks when the full post shelves button is clicked', async () => {
	const user = userEvent.setup();
	renderWithProvider( <SubscribeWithShelfButton { ...props } /> );
...
it( 'only applies the added shelf on Save (not while toggling), via the update endpoint', async () => {
	mockIsSubscribed = true;
	const user = userEvent.setup();
	renderWithProvider( <SubscribeWithShelfButton { ...props } /> );
	...
	expect( mockUpdateShelf ).not.toHaveBeenCalled();
...
it( 'removes the feed from a shelf on Save by replacing its feed list', async () => {
	mockIsSubscribed = true;
	mockDetailsById = {
		[ shelf.id ]: {
			...shelf,
...
it( 'discards the draft on Cancel without writing anything', async () => {
	...
	expect( mockUpdateShelf ).not.toHaveBeenCalled();
	expect(
		screen.queryByRole( 'dialog', { name: 'Move site to a shelf' } )
	).not.toBeInTheDocument();
} );
...
it( 'keeps the shelf rows and Save disabled until the feed is subscribed', async () => {
...
it( 'does not allow editing when shelf details fail to load', async () => {
	mockIsSubscribed = true;
	mockDetailsById = { [ shelf.id ]: undefined };
...
it( 'shows the empty state when the user has no shelves', async () => {
	mockShelves = [];
	const user = userEvent.setup();
	renderWithProvider( <SubscribeWithShelfButton { ...props } /> );
	...
	expect( await screen.findByText( 'You don’t have any shelves yet.' ) ).toBeVisible();
} );
