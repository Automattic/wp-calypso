...
/**
 * A selectable language for a Shelf. `code` is the base ES language code the API
 ...
 */
export interface ShelfLanguageOption {
	code: string;
	name: string;
}
...
const optionsByCode = new Map< string, ShelfLanguageOption >();
...
export const SHELF_LANGUAGE_OPTIONS: ShelfLanguageOption[] = Array.from(
	optionsByCode.values()
).sort( ( a, b ) => a.name.localeCompare( b.name ) );

/** Suggestion strings (display names) for the FormTokenField dropdown. */
export const SHELF_LANGUAGE_SUGGESTIONS: string[] = SHELF_LANGUAGE_OPTIONS.map(
	( option ) => option.name
);

const nameByCode = new Map(
	SHELF_LANGUAGE_OPTIONS.map( ( option ) => [ option.code, option.name ] )
);
const codeByLowerName = new Map(
	SHELF_LANGUAGE_OPTIONS.map( ( option ) => [ option.name.toLowerCase(), option.code ] )
);
