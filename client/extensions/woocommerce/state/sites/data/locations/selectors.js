 ...
 */
export const getStates = createSelector((state, countryCode, siteId = getSelectedSiteId(state)) => {
	if (!areLocationsLoaded(state, siteId)) {
		return [];
	}
	const country = find(flatMap(getRawLocations(state, siteId), 'countries'), {
 ...
 */
export const getAllCountryNames = createSelector((state, siteId = getSelectedSiteId(state)) => {
