...
describe('HorizontalBar', () => {
	test('should render a bar with correct dimensions', () => {
		const width = 34;
		const height = 15;
		const bar = mount(<HorizontalBar width={width} height={height} data={10} extent={[0, 10]} />);
	...
	test('should render text currency formatted if supplied', () => {
		...
		const dataMax = 100;
		const extent = [0, dataMax];
		const bar = mount(<HorizontalBar width={width} data={data} extent={extent} currency="NZD" />);
