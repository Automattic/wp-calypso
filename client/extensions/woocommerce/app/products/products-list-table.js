...
const ProductsListTable = ({ translate, products, site, isRequesting }) => {
	const headings = (
		<TableRow isHeader className={classNames({ 'products__list-placeholder': !products.length })}>
	...
	return (
		<div>
			...
			>
				{!!products.length &&
					products.map((product, i) => <ProductsListRow key={i} site={site} product={product} />)}
			</Table>
