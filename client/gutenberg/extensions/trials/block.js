...
class Heading extends Component {
	render() {
		const { className, edit, attributes: { anchor } } = this.props;
		return [
			anchor ? <h2 key="heading" id={ anchor }>{ this.richText() }</h2> : <h2 key="heading">{ this.richText() }</h2>,
			...
			edit ?
				<InnerBlocks key="children" allowedBlocks={ [ 'a8c/trials-list-item' ] } template={ [ [ 'a8c/trials-list-item', {} ] ] } templateLock={ false } />
				:
