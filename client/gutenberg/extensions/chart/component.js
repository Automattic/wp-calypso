...
export class Chart extends Component {
	...
	resizeChartAfterWindowResize = () => {
		this.debounce('resizeChart', function() {
			this.resizeChart();
		}, 150);
	}
	...
	googlesheet_urlChanged() {
		if (!this.validateURL(this.googlesheet_url)) {
			...
			this._reversionTimeout = setTimeout(function() {
	...
	makeChart( header, rows, type ) {
		...
		if (containerHeight > 0 && this._lastStringifiedC3 && stringifiedC3 === this._lastStringifiedC3) {
	...
	makeLegend( data ) {
		...
		const legendItems = [],
			getYAxisOffset = function() {
				const blockLeft = this.chartRef.current.getBoundingClientRect().left,
					axisLeft = this.chartRef.current.querySelector('.c3-axis.c3-axis-y').getBoundingClientRect().left;
		...
		jQuery( this.chartRef.current ).find('.top-legend').remove();
		...
		d3.select( this.chartRef.current )
			...
			.append('span')
			.attr({
				'data-id': function(id) { return id; },
				class: 'top-legend-item'
			})
			.html( function( id ) { return '<span class="legend-label">' + id + '</span>'; })
			.each( function( id ) {
	...
	downloadAndStoreGoogleSheetsData() {
		...
		if ( ! loaded || ! this.validateURL( googlesheet_url ) || googlesheet_url === lastLoadedSheet) {
			return;
		}
	...
	assessTimeseries( header ) {
		...
		for ( let i = 0; i < header.length; i++ ) {
			const date = ( this.validateYear( header[i] ) ) ? new Date( header[i], 0, 1 ) : new Date( header[i] );
