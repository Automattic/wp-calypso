...
const SignupForm = ( { onRequestClose }: Props ) => {
	...
	const { isFetchingNewUser, newUserError } = useSelect( ( select ) => ( {
		isFetchingNewUser: select( USER_STORE ).isFetchingNewUser(),
		newUserError: select( USER_STORE ).getNewUserError(),
	} ) );
