...
export interface AnchorStateType {
...
export type AnchorStateKeyType = keyof AnchorStateType;
...
interface UseAnchorParameterType {
	...
	locationStateParamName: AnchorStateKeyType;
	sanitize: ( arg0: string ) => string;
}

export default function useAnchorParameter( {
	queryParamName,
	locationStateParamName,
	sanitize,
}: UseAnchorParameterType ): string | null {
	const { state: locationState = {}, search } = useLocation< AnchorStateType >();
