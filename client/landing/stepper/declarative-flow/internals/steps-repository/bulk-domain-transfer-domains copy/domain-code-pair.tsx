...
/* useEffect( () => {
        if ( formState === 'VALIDATING' && validationResults && ! isValidating ) {
            let hasErrors = false;
            const newDomainsState = { ...domainsState };
            const domains = Object.values( newDomainsState );
            for ( let i = 0; i < domains.length; i++ ) {
                if (
                    validationResults[ i ].unlocked === true &&
                    validationResults[ i ].codeValid === true
                ) {
                    domains[ i ].passedValidation = true;
                    continue;
                } else if ( validationResults[ i ].unlocked === 'error' ) {
                    hasErrors = true;
                    domains[ i ].errorMessage = __(
                        'An an error occurred while checking if the domain is unlocked. Please try again later or contact our support team.'
                    );
                } else if ( validationResults[ i ].unlocked === false ) {
                    hasErrors = true;
                    domains[ i ].errorMessage = __(
                        'The domain is locked. Please unlock it before transferring.'
                    );
                } else if ( validationResults[ i ].unlocked === null ) {
                    hasErrors = true;
                    domains[ i ].errorMessage = __( 'The domain does not seem registered.' );
                } else if ( ! validationResults[ i ].codeValid ) {
                    hasErrors = true;
                    domains[ i ].errorMessage = __( 'The auth code provided for this domain is incorrect.' );
                }
            }
    ...
    }, [ formState, validationResults, domainsState, __, isValidating ] );
*/
