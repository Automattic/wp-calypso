...
import SiteMenu from 'my-sites/sidebar/site-menu';
...
} from 'my-sites/sidebar/constants';
...
export class MySitesSidebar extends Component {
	...
	handleExpandableMenuClick( path ) {
		return () => this.toggleSelectionSection( path );
	}
	...
	plugins() {
		const { translate, path, site, canUserManagePlugins } = this.props;

		if ( ! site || ! canUserManagePlugins ) {
			return null;
		}
		...
		return (
			<SidebarItem
				...
				selected={ itemLinkMatches( '/plugins', path ) }
				link={ '/plugins' + this.props.siteSuffix }
