...
async function discoverActorUrl( instance: string, token: string ): Promise< string | null > {
	...
	if ( authState?.actorUrl ) {
		...
		console.log( '[C2S] Actor URL from auth state:', authState.actorUrl );
	...
	if ( authState?.introspectionEndpoint ) {
		try {
			...
			// eslint-disable-next-line no-console
			console.log( '[C2S] Introspection response:', introResponse.status );
			if ( introResponse.ok ) {
				const introData = await introResponse.json();
				// eslint-disable-next-line no-console
				console.log( '[C2S] Introspection data:', JSON.stringify( introData ) );
				if ( introData.sub ) {
			...
			// eslint-disable-next-line no-console
			console.log( '[C2S] Introspection failed:', err );
		}
	} else {
		// eslint-disable-next-line no-console
		console.log( '[C2S] No introspection endpoint stored' );
	}
	...
	try {
		...
		// eslint-disable-next-line no-console
		console.log( '[C2S] verify_credentials response:', credResponse.status );
		if ( credResponse.ok ) {
	...
	try {
		...
		// eslint-disable-next-line no-console
		console.log( '[C2S] WP users/me response:', wpMeResponse.status );
		if ( wpMeResponse.ok ) {
			const wpUser = await wpMeResponse.json();
			// eslint-disable-next-line no-console
			console.log( '[C2S] WP user data:', JSON.stringify( wpUser ) );
			if ( wpUser.link ) {
				return wpUser.link;
			}
		}
	...
	// eslint-disable-next-line no-console
	console.log( '[C2S] All actor discovery methods failed' );
	return null;
}
...
async function discoverOutboxFromActor(
	...
	try {
		...
		// eslint-disable-next-line no-console
		console.log( '[C2S] Actor fetch:', actorUrl, '→', response.status );
		...
		// eslint-disable-next-line no-console
		console.log( '[C2S] Actor outbox:', actor.outbox );
		return actor.outbox || null;
	} catch ( err ) {
		// eslint-disable-next-line no-console
		console.log( '[C2S] Actor fetch failed:', err );
		return null;
	}
}
...
async function discoverOutbox( instance: string, token: string ): Promise< string | null > {
	// eslint-disable-next-line no-console
	console.log( '[C2S] Starting outbox discovery for instance:', instance );
	const actorUrl = await discoverActorUrl( instance, token );
	// eslint-disable-next-line no-console
	console.log( '[C2S] Discovered actor URL:', actorUrl );
	if ( ! actorUrl ) {
		return null;
	}
...
async function followViaC2S(
	...
	// eslint-disable-next-line no-console
	console.log( '[C2S] POST Follow to outbox:', outbox, JSON.stringify( activity ) );
	...
	// eslint-disable-next-line no-console
	console.log( '[C2S] Follow response:', response.status );

	if ( ! response.ok && response.status !== 201 && response.status !== 202 ) {
		const body = await response.text().catch( () => '' );
		// eslint-disable-next-line no-console
		console.log( '[C2S] Follow error body:', body );
