...
class KeyedSuggestions extends React.Component {
  ...
  setInitialState = input => {
    ...
    const taxonomySuggestionsArray = this.createTaxonomySuggestionsArray(
      suggestions
    );
    this.setState({
      suggestions,
  ...
  incPosition = () => {
    const position =
      (this.state.suggestionPosition + 1) % this.countSuggestions();
    this.setState({
  ...
  narrowDownAndSort = (input, showAll = "") => {
    ...
    for (const key in terms) {
      ...
      if (showAll === key) {
        ...
        const matchingSorted = sortBy(parts[0], term =>
          term.indexOf(filterTerm)
        );
  ...
  createTaxonomySuggestionsArray = suggestions => {
    ...
    for (const key in suggestions) {
      ...
      taxonomySuggestionsArray.push(
        ...suggestions[key].map(value => key + ":" + value)
      );
    }
    ...
    for (const key in suggestions) {
      ...
      rendered.push(
        <div className="keyed-suggestions__category" key={key}>
          ...
          {Object.keys(this.props.terms[key]).length >
      ...
      rendered.push(
        suggestions[key].map((value, i) => {
          ...
          const hasHighlight =
            noOfSuggestions + i === this.state.suggestionPosition;
          ...
          return (
            <span
              ...
              key={key + "_" + i}
            >
              <span className="keyed-suggestions__value-category">
                {key + ":" + value + " "}
              </span>
              <span className="keyed-suggestions__value-label-wigh-highlight">
                {this.createTextWithHighlight(
                  taxonomyName,
