	...
	if ( countLowSeverity ) {
		lowThreatsSummary = String(
			translate( '%(lowCount)s low risk item', '%(lowCount)s low risk items', {
				args: {
					lowCount: numberFormat( countLowSeverity, 0 ), // TODO clk numberFormat no need  for default 0 but confirm
				},
	...
	if ( countHighSeverity ) {
		highThreatsSummary = String(
			translate( '%(threatCount)s threat', '%(threatCount)s threats', {
				args: {
					threatCount: numberFormat( countHighSeverity, 0 ), // TODO clk numberFormat no need  for default 0 but confirm
				},
