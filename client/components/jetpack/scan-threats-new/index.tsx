...
import FixAllThreatsDialog from 'calypso/components/jetpack/fix-all-threats-dialog-new';
import SecurityIcon from 'calypso/components/jetpack/security-icon';
import ThreatDialog from 'calypso/components/jetpack/threat-dialog-new';
import ThreatItem from 'calypso/components/jetpack/threat-item-new';
import {
	FixableThreat,
	Threat,
	ThreatAction,
} from 'calypso/components/jetpack/threat-item-new/types';
...
const ScanError: React.FC< { site: Site } > = ( { site } ) => {
	...
	return (
		<div className="scan-threats-new__error">
			{ translate(
				'The scanner was unable to check all files and errored before completion. Deal with the threats found above and run the {{runScan}}scan again{{/runScan}}. If the error persists, we are {{linkToSupport}}here to help{{/linkToSupport}}.',
				{
					components: {
						runScan: (
							<Button className="scan-threats-new__run-scan-button" onClick={ dispatchScanRun } />
						),
						linkToSupport: (
	...
	return (
		<>
			<Card>
				...
				{ hasFixableThreats && (
					<>
						<div className="scan-threats-new__buttons">
							<p>{ fixSummary }</p>
							<Button
								primary
								className="scan-threats-new__fix-all-threats-button"
			...
			<ThreatListHeader />
			<div className="scan-threats-new__threats">
				{ highSeverityThreats &&
			...
			{ ! error && (
				<div className="scan-threats-new__rerun">
					<p className="scan-threats-new__rerun-help">
						{ translate(
							'If you have manually fixed any of the threats listed above, you can {{button}}run a manual scan now{{/button}} or wait for Jetpack to scan your site later today.',
							{
								components: {
									button: (
										<Button
											className="scan-threats-new__run-scan-button"
