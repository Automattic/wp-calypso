...
export class RewindCredentialsForm extends Component {
  ...
  handleSubmit = () => {
    ...
    const errors = Object.assign(
      !payload.host && {
        host: translate("Please enter a valid server address.")
      },
      ...
      !payload.pass &&
        !payload.kpri && {
          pass: translate("Please enter your server password.")
        },
      !payload.path &&
        requirePath && { path: translate("Please enter a server path.") }
    );
  ...
  componentWillReceiveProps(nextProps) {
    ...
    nextForm.host =
      isEmpty(nextForm.host) && credentials ? credentials.host : nextForm.host;
    nextForm.port =
      isEmpty(nextForm.port) && credentials ? credentials.port : nextForm.port;
    nextForm.user =
      isEmpty(nextForm.user) && credentials ? credentials.user : nextForm.user;
    nextForm.path =
      isEmpty(nextForm.path) && credentials ? credentials.path : nextForm.path;

    // Populate the host field with the site slug if needed
    nextForm.host =
      isEmpty(nextForm.host) && siteSlug
        ? siteSlug.split("::")[0]
        : nextForm.host;
  ...
  render() {
    ...
    return (
      <div className="rewind-credentials-form">
        ...
        <FormFieldset>
          <FormLabel htmlFor="protocol-type">
...
const mapStateToProps = (state, { siteId }) => ({
  formIsSubmitting:
    "pending" === getJetpackCredentialsUpdateStatus(state, siteId),
