...
class GoogleAppsUsers extends React.Component {
  componentWillMount() {
    this.props.onChange(
      this.props.fields ? this.props.fields : this.getInitialFields()
    );
  }
  ...
  render() {
    ...
    return (
      <div
        className="google-apps-dialog__users"
        key="google-apps-dialog__users"
      >
  ...
  updateField = (index, event) => {
    ...
    const newValue =
      fieldName === "email" ? event.target.value.trim() : event.target.value;
