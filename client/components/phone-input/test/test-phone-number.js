...
describe("metadata:", () => {
  ...
  describe("applyTemplate( number, template, positionTracking )", () => {
    ...
    test("should be able to partially apply templates", () => {
      equal(
        applyTemplate("4259", "...-....".replace(/\./g, DIGIT_PLACEHOLDER)),
        "425-9"
      );
    });
  ...
  describe("findCountryFromNumber( number )", () => {
    ...
    test("should guess a shared dial code with area code", () => {
      equal(
        findCountryFromNumber("+1604").isoCode,
        "CA",
        "Failed to figure out Canadian"
      );
      equal(
        findCountryFromNumber("+1425").isoCode,
        "US",
        "Failed to figure out U.S."
      );
      equal(
        findCountryFromNumber("+14").isoCode,
        "US",
        "Failed to figure out U.S."
      );
    });
