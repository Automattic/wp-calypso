...
class PhoneInput extends React.PureComponent {
  ...
  componentWillUpdate(nextProps) {
    ...
    if (currentFormat !== nextFormat) {
      if (currentCursorPoint >= currentFormat.length) {
        ...
        newCursorPoint =
          currentCursorPoint + nonDigitCountNew - nonDigitCountOld;
      }
    }
  ...
  shouldGuessCountry(value) {
    if (
      !value ||
      value.length < MIN_LENGTH_TO_FORMAT ||
      this.state.freezeSelection
    ) {
      return false;
    }
    const dialCode =
      this.getCountry().countryDialCode || this.getCountry().dialCode;
   ...
   * @returns {countryMetadata} - Country Metadata
   */
  guessCountryFromValueOrGetSelected(
    value,
    fallbackCountryCode = this.props.countryCode
  ) {
  ...
  calculateInputAndCountryCode(value, countryCode) {
    const calculatedCountry = this.guessCountryFromValueOrGetSelected(
        value,
        countryCode
      ),
  ...
  handleCountrySelection(event) {
    ...
    if (this.props.value[0] !== "+") {
      ...
      inputValue =
        "+" + this.getCountry(newCountryCode).dialCode + nationalNumber;
    }
    this.props.onChange({
      ...
      <div className={classnames(this.props.className, "phone-input")}>
        ...
        <div className="phone-input__select-container">
          <div className="phone-input__select-inner-container">
            ...
            />
            <CountryFlag
              countryCode={this.getCountry().isoCode.toLowerCase()}
            />
          </div>
        </div>
      </div>
