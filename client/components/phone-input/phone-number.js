...
export function makeTemplate(phoneNumber, patterns) {
  const selectedPattern = find(patterns, pattern => {
    ...
    if (
      pattern.leadingDigitPattern &&
      phoneNumber.search(pattern.leadingDigitPattern) !== 0
    ) {
      return false;
    }
  ...
  const matchingNumber = LONGEST_NUMBER.match(
    new RegExp(selectedPatternMatch)
  )[0];
...
export function applyTemplate(
  ...
  for (
    let i = 0;
    i < template.length && phoneNumberIndex < phoneNumber.length;
    i++
  ) {
...
export function processNumber(inputNumber, numberRegion) {
  ...
  const nationalNumber = stripNonDigits(inputNumber).replace(
    new RegExp(
      "^(" + numberRegion.dialCode + ")?(" + numberRegion.nationalPrefix + ")?"
    ),
    ""
  );

  debug(
    `National Number: ${nationalNumber} for ${inputNumber} in ${
      numberRegion.isoCode
    }`
  );
...
export function formatNumber(inputNumber, country) {
  ...
  if (
    digitCount < MIN_LENGTH_TO_FORMAT ||
    digitCount < (country.dialCode || "").length
  ) {
    if (inputNumber[0] === "+") {
  ...
  if (pattern) {
    ...
    return (
      prefix +
      nationalNumber.replace(new RegExp(pattern.match), pattern.replace)
    );
  }
  ...
  if (template) {
    debug(
      `Will replace "${nationalNumber}" with "${template}" with prefix "${prefix}"`
    );
...
export function toIcannFormat(inputNumber, country) {
  ...
  const { nationalNumber } = processNumber(inputNumber, country),
    ...
    dialCode =
      country.countryDialCode && country.regionCode ? country.regionCode : "";
