...
class ScrollHelper {
  ...
  forEachInRow(index, callback, context) {
    ...
    const firstIndexInRow = index - (index % this.props.itemsPerRow),
      lastIndexInRow =
        Math.min(
          firstIndexInRow + this.props.itemsPerRow,
      ...
      function(item) {
        ...
        if (itemBounds) {
          if ("bottom" === fromDirection) {
            ...
            height =
              itemBounds.bottom -
              (this.containerTop + this.topPlaceholderHeight);
          }
        } else {
  ...
  updateContextHeight(contextHeight) {
    ...
    this.bottomHideLevelSoft =
      contextHeight +
      Math.max(2 * contextHeight, 10 * this.props.guessedItemHeight);

    this.bottomHideLevelUltraSoft =
      contextHeight +
      Math.max(3 * contextHeight, 15 * this.props.guessedItemHeight);
  }
  ...
  shouldHideItemsAbove() {
    ...
    //
    return (
      this.containerTop + this.topPlaceholderHeight < this.topHideLevelSoft
    );
  }
  ...
  shouldShowItemsAbove() {
    ...
    //
    return (
      this.containerTop + this.topPlaceholderHeight > this.topHideLevelHard
    );
  }
  ...
  showItemsAbove() {
    ...
    while (this.firstRenderedIndex > 0) {
      ...
      newPlaceholderBottom =
        this.containerTop + this.topPlaceholderHeight - rowHeight;

      if (newPlaceholderBottom < this.topHideLevelSoft) {
        break;
      }

      this.deleteRowItemHeights(
        this.firstRenderedIndex - this.props.itemsPerRow
      );
      this.firstRenderedIndex -= this.props.itemsPerRow;
      ...
      if (this.firstRenderedIndex <= 0) {
        ...
        this.topPlaceholderHeight = Math.max(
          0,
          this.topPlaceholderHeight - rowHeight
        );
      }
  ...
  showItemsBelow() {
    ...
    while (this.lastRenderedIndex < this.props.items.length - 1) {
      ...
      this.lastRenderedIndex += this.props.itemsPerRow;
      this.lastRenderedIndex = Math.min(
