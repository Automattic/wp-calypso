...
function getStateFromStores(props) {
  return {
    ...
    emailForwarding: EmailForwardingStore.getByDomainName(
    ...
    wapiDomainInfo: WapiDomainInfoStore.getByDomainName(
      props.selectedDomainName
    ),
...
class DomainManagementData extends React.Component {
  ...
  loadData = prevProps => {
    ...
    if (
      needsGoogleApps &&
      (prevProps.needsGoogleApps !== needsGoogleApps ||
    ...
    if (
      needsUsers &&
      (prevProps.needsUsers !== needsUsers ||
        prevProps.selectedSite !== selectedSite)
    ) {
    ...
    return (
      <div>
        <PageViewTracker
          ...
          title={this.props.analyticsTitle}
        />
        {selectedSite && needsContactDetails && <QueryContactDetailsCache />}
        {selectedSite &&
          needsDomains && <QuerySiteDomains siteId={selectedSite.ID} />}
        {selectedSite &&
          needsPlans && <QuerySitePlans siteId={selectedSite.ID} />}
