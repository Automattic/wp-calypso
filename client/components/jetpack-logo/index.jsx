...
const LogoPathSize32 = ( { variant = 'color' } ) => {
	let primary = COLOR_JETPACK;
	let secondary = COLOR_WHITE;
	if ( variant === 'black' ) {
		primary = 'black';
		secondary = 'white';
 ...
 * @param {'color'|'black'|'white'} [props.variant] - The color variant of the logo. Takes precedence over monochrome.
 ...
 */
const JetpackLogo = ( { full = false, variant, monochrome, size = 32, className, aria } ) => {
	const classes = clsx( 'jetpack-logo', className );
	const ariaProps = useAriaProps( aria );

	let effectiveVariant = variant;
	if ( ! effectiveVariant && monochrome ) {
		effectiveVariant = 'black';
	}
	if ( ! effectiveVariant ) {
		effectiveVariant = 'color';
	}
	const textFill = effectiveVariant === 'white' ? 'white' : 'black';

	if ( full === true ) {
		return (
			<svg height={ size } className={ classes } viewBox="0 0 118 32" { ...ariaProps }>
				<title>Jetpack</title>
				<LogoPathSize32 variant={ effectiveVariant } />
				<path
