...
describe("contact form validations", () => {
  ...
  describe("#validateSettingsToEmail()", () => {
    ...
    test("should validate multiple comma-separated e-mails", () => {
      expect(
        validateSettingsToEmail("something@example.com,another@example.com")
      ).to.be.true;
    });
    ...
    test("should ignore trailing and leading whitespace", () => {
      expect(
        validateSettingsToEmail(
          "  something@example.com ,  another@example.com  "
        )
      ).to.be.true;
    });

    test("should fail if one e-mail is invalid", () => {
      expect(validateSettingsToEmail("something@example.com, hocus_pocus")).to
        .be.false;
    });
  });
});
