...
const ContactFormDialog = React.createClass({
    ...
    getActionButtons() {
        const isValidForm = validateFormFields(this.props.contactForm.fields) &&
            validateSettingsToEmail(this.props.contactForm.to);
        const actionButtons = [
            <FormButton key="save" disabled={!isValidForm} onClick={this.props.onInsert}>
                {this.props.isEdit ? this.translate('Update') : this.translate('Insert')}
            </FormButton>,
            <FormButton key="cancel" isPrimary={false} onClick={this.props.onClose}>
        ...
        if (this.props.activeTab === 'fields') {
            return [
                <div
                    ...
                    className="editor-contact-form-modal__secondary-actions"
                >
                    <FormButton key="add" isPrimary={false} onClick={this.props.onFieldAdd}>
