...
export class Mentions extends Component {
  ...
  getMatchingSuggestions(suggestions, query) {
    if (query) {
      ...
      suggestions = suggestions.filter(
        ({ user_login: login, display_name: name }) =>
  ...
  getQueryText() {
    ...
    const textBeforeCaret = range.startContainer.textContent.slice(
      0,
      range.startOffset
    );
  ...
  insertSuggestion = ({ user_login: userLogin }) => {
    ...
    selection.setContent(
      selection
        .getContent()
        .replace(/@\S*/, ReactDomServer.renderToStaticMarkup(markup))
    );
  ...
  onKeyDown = ({ keyCode, preventDefault }) => {
    ...
    this.setState({
      selectedSuggestionId: this.matchingSuggestions[nextIndex].ID
    });
  };
  ...
  render() {
    ...
    const selectedSuggestionId =
      ...
      get(head(this.matchingSuggestions), "ID");
