...
const productFormToCustomPost = state =>
  productToCustomPost(getProductFormValues(state));
...
const createMembershipButton = siteId => (dispatch, getState) => {
  ...
  if (values.stripe_account === "create" && values.email) {
    ...
    return wpcom.req
      ...
      .then(newAccount => {
        values.stripe_account =
          newAccount.result.account.connected_destination_account_id;
...
class SimplePaymentsDialog extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      activeTab:
        editPaymentId || isEmptyArray(paymentButtons) ? "form" : "list",
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      this.props.paymentButtons === null &&
      isEmptyArray(nextProps.paymentButtons)
    ) {
      this.showButtonForm(null);
    }
  }
  ...
  checkUnsavedForm() {
    const formIsUnsaved =
      this.state.activeTab === "form" && this.props.formIsDirty;
    ...
    return new Promise(resolve => {
      ...
      accept(
        translate(
          "Wait! You have unsaved changes. Do you really want to discard them?"
        ),
        resolve,
        translate("Discard"),
        null,
  ...
  handleChangeTabs = activeTab => {
    if (activeTab === "form") {
      ...
      this.checkUnsavedForm().then(
        accepted => accepted && this.showButtonList()
      );
    }
  };
  ...
  showError = errorMessage =>
    this._isMounted && this.setState({ errorMessage });
  ...
  handleInsert = () => {
    ...
    productId
      .then(id =>
        dispatch((d, getState) =>
          getSimplePayments(getState(), this.props.siteId, id)
        )
      )
      .then(product => {
        this.props.onInsert({
          id: product.ID,
          isMembership: !!product.recurring
        });
        dispatch(
          recordTracksEvent("calypso_simple_payments_button_insert", {
            id: product.ID
          })
        );
      })
      .catch(() =>
        this.showError(translate("The payment button could not be inserted."))
      )
  ...
  handleSave = () => {
    ...
    dispatch(updateAction)
      ...
      .catch(() =>
        this.showError(translate("The payment button could not be updated."))
      )
      .then(() => this.setIsSubmitting(false));
  };
  ...
  handleTrash = paymentId => {
    ...
    accept(
      ...
      accepted => {
        ...
        dispatch(
          recordTracksEvent("calypso_simple_payments_button_delete", {
            id: paymentId
          })
        );
        dispatch(trashPaymentButton(siteId, paymentId))
          .catch(() =>
            this.showError(
              translate("The payment button could not be deleted.")
            )
          )
          .then(() => this.setIsSubmitting(false));
      },
  ...
  makeDirtyAfterImageEdit = () =>
    this.setState({ isDirtyAfterImageEdit: true });
  ...
  getActionButtons() {
    ...
    const {
      ...
      editedPaymentId,
      isSubmitting,
      isDirtyAfterImageEdit
    } = this.state;

    const formCanBeSubmitted =
      formIsValid && (formIsDirty || isDirtyAfterImageEdit);
    ...
    const isUploadingImage =
      !isEmpty(featuredImageId) && !isNumber(featuredImageId);
  ...
  render() {
          ...
          action={
            <Banner
              ...
              description={translate(
                "Simple Payments requires Jetpack version 5.2 or later."
              )}
        ...
        <EmptyContent
          ...
          action={
            <UpgradeNudge
              ...
              title={translate(
                "Upgrade your plan to our Premium or Business plan!"
              )}
              message={translate(
    ...
    return (
      ...
      >
        ...
        {(!currencyCode || shouldQuerySitePlans) && (
...
export default connect((state, { siteId }) => {
  ...
  return {
    ...
    isJetpackNotSupported:
      ...
      !isJetpackMinimumVersion(state, siteId, "5.2"),
    planHasSimplePaymentsFeature: hasFeature(
      state,
      siteId,
    ...
    featuredImageId: get(
      getFormValues(REDUX_FORM_NAME)(state),
      "featuredImageId"
    )
  };
