...
import ReduxFormFieldset, {
  FieldsetRenderer
} from "components/redux-forms/redux-form-fieldset";
...
export const getProductFormValues = state =>
...
const validate = (values, props) => {
  ...
  if (!values.price || parseFloat(values.price) === 0) {
    errors.price = translate(
      "Everything comes with a price tag these days. Add yours here."
    );
  } else if (parseFloat(values.price) === NaN) {
    errors.price = translate("Invalid price");
  } else if (parseFloat(values.price) < 0) {
    errors.price = translate(
      "Your price is negative — now that doesn't sound right, does it?"
    );
    ...
    return (
      <div>
        ...
        <ReduxFormFieldset
          ...
          explanation={translate(
            "After what time should the subscription renew?"
          )}
    ...
    state => {
      return {
        ...
        isChoosingToAuthorizeStripeAccount:
          getEditedSimplePaymentsStripeAccount(state, REDUX_FORM_NAME) ===
          "authorize",
