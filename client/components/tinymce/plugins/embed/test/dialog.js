...
describe("EmbedDialog", () => {
  ...
  test("should render", () => {
    ...
    assert.strictEqual(
      wrapper.find(FormTextInput).get(0).props.defaultValue,
      url
    );
  });
  ...
  describe("EmbedDialog parseEmbedEndpointResult", () => {
    ...
    test("should parse API error", () => {
      ...
      instance.parseEmbedEndpointResult(url)(
        {
          error: "invalid_embed_url",
          message: "The embed_url parameter must be a valid URL."
        },
        null
      );
    ...
    test("should parse API layer error", () => {
      ...
      assert.strictEqual(
        "custom_error",
        wrapper.state().previewMarkup[url].renderMarkup
      );
    });
