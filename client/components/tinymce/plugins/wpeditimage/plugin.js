...
function wpEditImage(editor) {
    var toolbar, serializer, each = tinymce.each, iOS = tinymce.Env.iOS;
    ...
    function parseShortcode(content) {
        return content.replace(
            ...
            function(a, b, c) {
                var id, align, classes, caption, img, width, trim = tinymce.trim;
                ...
                return '<div class="mceTemp"><dl id="' +
                    id +
                    '" class="wp-caption ' +
                    align +
                    classes +
    ...
    editor.on('init', function() {
        ...
        editor.on('wpImageFormSubmit', function(event) {
            ...
            if (!imgNode) {
                ...
                if (caption) {
                    ...
                    html = '<dl class="wp-caption alignnone"' +
                        captionWidth +
                        '>' +
                        '<dt class="wp-caption-dt">' +
                        html +
        ...
        editor.on('wpLoadImageData', function(event) {
            var parent, data = event.imgData.data, imgNode = event.imgData.node;
        ...
        if (tinymce.Env.ie && tinymce.Env.ie > 10) {
            ...
            dom.bind(editor.getBody(), 'mscontrolselect', function(event) {
                if (event.target.nodeName === 'IMG' && dom.getParent(event.target, '.wp-caption')) {
                    ...
                    event.target.nodeName === 'DL' && dom.hasClass(event.target, 'wp-caption')
                ) {
    ...
    editor.on('ObjectResized', function(event) {
        ...
        if (node.nodeName === 'IMG') {
            editor.undoManager.transact(function() {
                var parent, width, dom = editor.dom;
    ...
    editor.on('BeforeExecCommand', function(event) {
        var node, p, DL, align, replacement, cmd = event.command, dom = editor.dom;
        ...
        if (cmd === 'mceInsertContent') {
            ...
            node.className = node.className.replace(/ ?align(left|center|right|none)/g, '') +
                replacement;
