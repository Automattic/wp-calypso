...
function mediaButton(editor) {
  ...
  function renderDropZone({ visible }) {
    ...
    if (!nodes.dropzone) {
      ...
      editor
        .getContainer()
        .parentNode.insertBefore(nodes.dropzone, editor.getContainer());
    }
  ...
  updateMedia = debounce(function() {
    ...
    if (
      !transients &&
      isEditorSaveBlocked(getState(), "MEDIA_MODAL_TRANSIENT_INSERT")
    ) {
  ...
  function resize(increment) {
    ...
    const size =
      findFn(SIZE_ORDER, isMatchingSize) || SIZE_ORDER[SIZE_ORDER.length - 1];

    // Generate updated markup
    const markup = MediaMarkup.get(
      selectedSite,
      media,
      assign(parsed.appearance, { size })
    );
  ...
  function toggleSizingControls(increase, event) {
    ...
    if (increase) {
      ...
      const isDisabled =
        currentRatio <= thumbRatio || SIZE_ORDER[0] === parsed.appearance.size;
      this.disabled(isDisabled);
    }
  }
  ...
  editor.addCommand("wpcomEditGallery", function(content) {
    ...
    gallery = assign(
      {},
      MediaConstants.GalleryDefaultAttrs,
      gallery.attrs.named
    );
