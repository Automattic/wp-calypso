...
function initialize(editor) {
	...
	const filteredMenuItems = menuItems.filter((item) => !item.condition || item.condition(editor));
	...
	editor.addButton('wpcom_insert_menu', {
		...
		onPostRender() {
			...
			insertContentElm.innerHTML = renderToString(
				<GridiconButton icon={<Gridicon icon="add-outline" />} label={i18n.translate('Add')} />
			);
		},
	});
}
