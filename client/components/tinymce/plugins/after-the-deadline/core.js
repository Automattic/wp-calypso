...
AtDCore.prototype.processXML = function(responseXML) {
  ...
  for (i = 0; i < errors.length; i++) {
    if (errors[i].getElementsByTagName("string").item(0).firstChild !== null) {
      errorString = errors[i].getElementsByTagName("string").item(0).firstChild
        .data;
      errorType = errors[i].getElementsByTagName("type").item(0).firstChild
        .data;
      errorDescription = errors[i].getElementsByTagName("description").item(0)
        .firstChild.data;
      ...
      if (this.ignore_strings[errorString] === undefined) {
        ...
        if (errors[i].getElementsByTagName("url").item(0) !== null) {
          errorUrl = errors[i].getElementsByTagName("url").item(0).firstChild
            .data;
...
AtDCore.prototype.findSuggestion = function(element) {
  let text = element.innerHTML,
    context = (this.getAttrib(element, "pre") + "").replace(
      /[\\,!\\?\\."\s]/g,
      ""
    ),
