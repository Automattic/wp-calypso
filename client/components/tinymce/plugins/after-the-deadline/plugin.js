...
function plugin(editor) {
  ...
  const SuggestionsMenu = tinymce.ui.Menu.extend({
    ...
    hide() {
      ...
      editor.off(
        "SpellcheckEnd click touchstart focusout",
        this.boundHideIfNotMarked
      );
    },
    ...
    reposition() {
      ...
      if (rootNode.nodeName === "BODY") {
        targetPos.x -=
          rootNode.ownerDocument.documentElement.scrollLeft ||
          rootNode.scrollLeft;
        targetPos.y -=
          rootNode.ownerDocument.documentElement.scrollTop ||
          rootNode.scrollTop;
      } else {
  ...
  function initAtDCore() {
    ...
    atdCore.create = function(node_html) {
      return dom.create(
        "span",
        { class: "mceItemHidden", "data-mce-bogus": 1 },
        node_html
      );
    };
    ...
    atdCore.setIgnoreStrings(
      editor.getParam("atd_ignore_strings", []).join(",")
    );
  ...
  function isMarkedNode(node) {
    return (
      node.className &&
      /\bhidden(GrammarError|SpellError|Suggestion)\b/.test(node.className)
    );
  }
  ...
  function finish() {
    let node,
      regex = new RegExp(
        "mceItemHidden|hidden(((Grammar|Spell)Error)|Suggestion)"
      ),
  ...
  function showSuggestions(target) {
    ...
    if (!errorDescription) {
      items.push({
        text: translate("No suggestions", {
          comment: "Editor proofreading no suggestions"
        }),
    ...
    if (errorDescription && errorDescription.moreinfo) {
      ...
      items.push({
        text: translate("Explain…", {
          comment: "Editor proofreading menu item"
        }),
        onclick: function() {
    ...
    items.push.apply(items, [
      { text: "-" }, // separator
      {
        text: translate("Ignore suggestion", {
          comment: "Editor proofreading menu item"
        }),
        onclick: function() {
    ...
    editor.addCommand("mceWritingImprovementTool", function(callback) {
      ...
      sendRequest(
        ...
        function(data, request) {
          ...
          if (
            request.responseXML.getElementsByTagName("message").item(0) !== null
          ) {
            editor.windowManager.alert(
  ...
  editor.addMenuItem("spellchecker", {
    text: translate("Proofread Writing", {
      comment: "Editor proofreading toolbar tooltip"
    }),
    context: "tools",
  ...
  editor.addButton("spellchecker", {
    tooltip: translate("Proofread Writing", {
      comment: "Editor proofreading button tooltip"
    }),
    cmd: "mceWritingImprovementTool",
    onPostRender: function() {
      const self = this;
