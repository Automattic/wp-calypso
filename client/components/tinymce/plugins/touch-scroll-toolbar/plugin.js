...
function touchScrollToolbar(editor) {
  ...
  function disableToolbarTouchEvents() {
    editor
      .$(".mce-toolbar:not(.mce-menubar)", document.body)
      .each((i, toolbar) => {
          ...
          throttle(({ target }) => {
            ...
            if (action) {
              ...
              if (!elements.hasClass("mce-container-body")) {
                elements.add(
                  tinymce.DOM.getParent(target, ".mce-container-body")
                );
              }
  ...
  function toggleToolbarsScrollableOnResize() {
    function toggleToolbarsScrollableClass() {
      editor
        .$(".mce-toolbar-grp", editor.theme.panel.getEl())
        .each((i, toolbar) => {
    ...
    window.addEventListener(
      "resize",
      throttle(toggleToolbarsScrollableClass, 200)
    );
    ...
    editor.on("wptoolbar", event => {
      ...
      setTimeout(() => {
        ...
        const isScrollable =
          toolbar.firstChild.scrollWidth > toolbar.clientWidth;
