...
export class LanguagePickerModal extends PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      this.state.showingDefaultFilter &&
  ...
  getFilteredLanguages() {
    ...
    if (search) {
      ...
      return languages.filter(language => {
        ...
        const names = [
          ...
          deburr(
            this.getLocalizedLanguageTitle(language.langSlug)
          ).toLowerCase(),
    ...
    if (filter) {
      switch (filter) {
        ...
        default:
          const languageGroup = getLanguageGroupById(filter);
          const subTerritories = languageGroup
            ? languageGroup.subTerritories
            : null;
          return languages
            .filter(language =>
  ...
  getSuggestedLanguages() {
    ...
    for (const langSlug of navigator.languages) {
      ...
      if (!language) {
        language = find(languages, lang =>
          startsWith(lcLangSlug, lang.langSlug + "-")
        );
      }
