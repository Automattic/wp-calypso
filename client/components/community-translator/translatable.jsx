    ...
    if (!this.hasDataLoaded()) {
      getSingleTranslationData(locale, { singular, context, plural })
        .then(originalData =>
          this.setState({
            originalData,
            translationUrl: getTranslationPermaLink(
              originalData.originalId,
              locale
            ),
            formState: {
      ...
      buttons.push(
        <Button
          ...
          disabled={
            ...
            this.state.originalData.translatedPlural ===
  ...
  getErrorMessage(errorType) {
    ...
    switch (errorType) {
      case TRANSLATION_FETCH_ERROR:
        return translate(
          "Sorry, we couldn't find the translation for this string."
        );
      case TRANSLATION_SUBMIT_ERROR:
        return translate(
          "Sorry, we couldn't submit the translation for this string."
        );
      default:
  ...
  renderTranslatableContent() {
    ...
    return [
      originalData.comment && (
        <p key="translationComment">{originalData.comment}</p>
      ),
          ...
          <nav>
            {this.state.translationUrl && (
              <a
                ...
                title={translate(
                  "Open this translation in translate.wordpress.com"
                )}
