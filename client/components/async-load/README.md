...
| `loadFailureFallback` | PropTypes.node    | no       | A fallback to render if the async `require()` import rejects (e.g. the chunk is blocked or missing). Runtime errors thrown inside the loaded component are not caught and propagate normally. If omitted, the import rejection propagates too.                                                        |
| `onLoadFailure`       | Function          | no       | Called with the rejection reason when the async import fails. Only invoked when `loadFailureFallback` is provided.                                                                                                                                                                                  |
