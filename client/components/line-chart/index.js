...
import {
  line as d3Line,
  area as d3Area,
  curveMonotoneX as d3MonotoneXCurve
} from "d3-shape";
import {
...
const dateFormatFunction = displayMonthTicksOnly => (date, index, tickRefs) => {
  const everyOtherTickOnly =
    !displayMonthTicksOnly && tickRefs.length > X_AXIS_TICKS_MAX;
        ...
        tickRefs
          .map(
            (tickRef, tickRefIndex) =>
              tickRef.__data__.getMonth() === date.getMonth()
                ? tickRefIndex
                : null
          )
          .filter(e => e !== null)
...
class LineChart extends Component {
  ...
  drawLines = (svg, params) => {
    ...
    data.forEach((dataSeries, index) => {
      ...
      svg
        ...
        .attr(
          "class",
          `line-chart__line-color-${colorNum} line-chart__line-${index}`
        )
    ...
    if (this.state.fillArea) {
      ...
      data.forEach((dataSeries, index) => {
        ...
        svg
          ...
          .attr(
            "class",
            `line-chart__area-color-${colorNum} line-chart__area-${index}`
          )
  ...
  drawPoints = (svg, params) => {
    ...
    data.forEach((dataSeries, dataSeriesIndex) => {
      ...
      (drawFullSeries
        ? dataSeries
        : [first(dataSeries), last(dataSeries)]
      ).forEach(datum => {
        svg
          .append("circle")
          .attr(
  ...
  handleMouseMove = (X, Y, params) => {
    ...
    const startDateBar =
      closestDate + Math.round((prevClosestDate - closestDate) / 2);
    const endDateBar =
      closestDate + Math.round((nextClosestDate - closestDate) / 2);
    ...
    if (bar.empty()) {
      ...
      svg
        ...
        .attr(
          "class",
          `line-chart__date-range-selected line-chart__date-range-${closestDate}`
        )
      ...
      if (drawFullSeries) {
        ...
        selectedPoints.attr(
          "r",
          Math.floor(POINTS_SIZE * POINT_HIGHLIGHT_SIZE_FACTOR)
        );
  ...
  getXAxisParams = (concatData, data, margin, newWidth) => {
    const [minTimestamp, maxTimestamp] = d3Extent(
      concatData,
      datum => datum.date
    );
    ...
    const displayMonthOnly =
      maxTimestamp - minTimestamp > APPROXIMATELY_A_MONTH_IN_MS;
    const months =
  ...
  handleDataSeriesSelected = selectedItemIndex => {
    ...
    data.forEach((dataSeries, dataSeriesIndex) => {
      ...
      const lineSelection = svg.select(
        `path.line-chart__line-${dataSeriesIndex}`
      );
      const areaSelection = svg.select(
        `path.line-chart__area-${dataSeriesIndex}`
      );
  ...
  renderTooltips = () => {
    ...
    const selectPointsValues = selectedPoints.map(
      point => d3Select(point).datum().value
    );
    ...
    return selectedPoints.map(point => {
      ...
      const uniqueKey = `tooltip-${pointData.dataSeriesIndex}-${
        pointData.date
      }`;
