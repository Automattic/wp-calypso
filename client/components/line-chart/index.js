...
class LineChart extends Component {
	...
	drawYAxis = (svg, params) => {
		...
		g.selectAll('.tick text').style('text-anchor', 'end').attr('transform', 'translate(-10,12)');
	};
	...
	drawPoints = (svg, params) => {
		...
		data.forEach((dataSeries, dataSeriesIndex) => {
			...
			(drawFullSeries ? dataSeries : [first(dataSeries), last(dataSeries)]).forEach((datum) => {
				svg
					.append('circle')
					.attr(
						'class',
	...
	handleMouseMove = (X, Y, params) => {
		...
		firstDataSerie.forEach((datum, index) => {
			if (Math.abs(xDate - datum.date) < Math.abs(xDate - closestDate)) {
				...
				prevClosestDate = firstDataSerie[index - 1] ? firstDataSerie[index - 1].date : datum.date;
				nextClosestDate = firstDataSerie[index + 1] ? firstDataSerie[index + 1].date : datum.date;
			}
		});
		...
		if (bar.empty()) {
			svg
				.select(`rect.line-chart__date-range-selected:not(.line-chart__date-range-${closestDate})`)
				.remove();
	...
	getXAxisParams = (concatData, data, margin, newWidth) => {
		...
		const months =
			dateToAbsoluteMonth(new Date(maxTimestamp)) - dateToAbsoluteMonth(new Date(minTimestamp));
	...
	getTooltipPositionMap = (values) => {
		const sortedUniqValues = uniq(values).sort((leftValue, rightValue) => leftValue - rightValue);
	...
	render() {
		...
		return (
			<div className="line-chart">
				{legendInfo && (
					<LineChartLegend data={legendInfo} onDataSeriesSelected={this.handleDataSeriesSelected} />
				)}
