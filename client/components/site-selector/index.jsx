...
class SiteSelector extends Component {
  ...
  computeHighlightedSite() {
    ...
    if (this.state.isKeyboardEngaged) {
      ...
      debug(
        `restoring highlight from last mouse hover (${this.lastMouseHover})`
      );
  ...
  onMouseMove = event => {
    ...
    if (
      event.pageX !== this.lastMouseMoveX ||
  ...
  renderSites() {
    ...
    if (this.props.sitesFound) {
      ...
      if (showRecentSites && this.shouldShowGroups() && size(recentSites)) {
        sites = filter(
          sites,
          ({ ID: siteId }) => !includes(recentSites, siteId)
        );
      }
    }
...
const navigateToSite = (
  siteId,
  { allSitesPath, allSitesSingleUser, siteBasePath }
) => (dispatch, getState) => {
  ...
  function getSiteBasePath(site) {
    ...
    const postsBase =
      site.jetpack || site.single_user_site ? "/posts" : "/posts/my";
    ...
    if (path.match(/^\/store\/stats\//)) {
      const isStore =
        site.jetpack && site.options && site.options.woocommerce_is_active;
      if (!isStore) {
        path = "/stats/day";
      }
    }
