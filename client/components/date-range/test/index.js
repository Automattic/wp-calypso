...
describe('DateRange', () => {
	...
	describe('Input elements', () => {
		...
		test('should update end date selection on end date input blur event', () => {
			...
			expect(dateToLocaleString(wrapper.state().endDate)).toEqual(dateToLocaleString('05/28/2018'));
		});
		...
		test('should not update date selection on input change event', () => {
			...
			expect(dateToLocaleString(wrapper.state().endDate)).toEqual(dateToLocaleString(endDate));

			expect(dateToLocaleString(wrapper.state().startDate)).toEqual(dateToLocaleString(startDate));
		});
	...
	describe('Callback props', () => {
		test('should call onDateSelect function when a date is selected', () => {
			...
			expect(dateToLocaleString(callback.mock.calls[1][1])).toEqual(dateToLocaleString(newEndDate));
		});
		...
		test('should call onDateCommit function when a date is committed/applied', () => {
			...
			expect(dateToLocaleString(callback.mock.calls[0][1])).toEqual(dateToLocaleString(newEndDate));
		});
	});
	...
	describe('Actions and Information UI', () => {
		test('should only persist date selection when user clicks "Apply" button', () => {
			...
			expect(dateToLocaleString(wrapper.state().endDate)).toEqual(dateToLocaleString(newEndDate));
		});
		...
		test('Should reset selection when reset UI clicked', () => {
			...
			expect(dateToLocaleString(wrapper.state().endDate)).toEqual(dateToLocaleString('09/20/2018'));
	...
	describe('Render props', () => {
		test('should allow for render prop to overide trigger render', () => {
			...
			shallow(<DateRange translate={translate} moment={moment} renderTrigger={spyComponent} />);
		...
		test('should allow for render prop to overide header render', () => {
			...
			shallow(<DateRange translate={translate} moment={moment} renderHeader={spyComponent} />);
		...
		test('should allow for render prop to overide inputs render', () => {
			...
			shallow(<DateRange translate={translate} moment={moment} renderInputs={spyComponent} />);
