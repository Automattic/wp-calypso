 ...
 */
import AppleIcon from 'components/social-icons/google';
...
class AppleLoginButton extends Component {
	...
	async loadDependency() {
		if ( ! window.appleLoginApi ) {
			await loadScript(
				'https://appleid.cdn-apple.com/appleauth/static/jsapi/appleid/1/en_US/appleid.auth.js'
			);
		}

		return window.appleLoginApi;
	}
		...
		this.initialized = this.loadDependency()
			...
			.catch( error => {
				...
				if ( 'idpiframe_initialization_failed' === error.error ) {
					...
					this.setState( {
						error: translate(
							'Please enable "third-party cookies" to connect your Apple account.'
						),
					} );
				}
					...
					>
						...
						<span className="social-buttons__service-name">
							{ this.props.translate( 'Continue with %(service)s', {
								args: { service: 'Apple' },
								comment:
									'%(service)s is the name of a Social Network, e.g. "Apple", "Facebook", "Twitter" ...',
							} ) }
						</span>
					</button>
				) }
