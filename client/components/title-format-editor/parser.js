    ...
    ([output, lastIndex, remainingText], next) => {
      ...
      const textBlock =
        next.offset > lastIndex
          ? {
              type: "string",
              value: remainingText.slice(lastIndex, next.offset)
            }
          : null;
 ...
 */
export const mapTokenTitleForEditor = title =>
  `\u205f\u205f${title}\u205f\u205f`;
 ...
 */
const tokenTitle = (type, tokens) =>
  mapTokenTitleForEditor(get(tokens, type, "").trim());
    ...
    ([block, lastIndex, entityGuide], piece) => [
      {
        ...
        entityRanges: isTextPiece(piece)
          ...
          : [
              ...block.entityRanges,
              newEntityAt(lastIndex, piece.type, tokens, entityGuide)
            ],
        text:
          block.text +
      ...
      lastIndex +
        (piece.value
          ? piece.value.length
          : tokenTitle(piece.type, tokens).length),
