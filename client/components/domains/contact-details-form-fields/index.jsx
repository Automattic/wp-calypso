...
export class ContactDetailsFormFields extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    return (
      ...
      (nextProps.needsFax !== this.props.needsFax ||
        ...
        nextProps.needsOnlyGoogleAppsDetails !==
  ...
  recordSubmit() {
    ...
    const tracksEventObject = formState
  ...
  focusFirstError() {
    const firstErrorName = kebabCase(
      head(formState.getInvalidFields(this.state.form)).name
    );
  ...
  getFieldProps = (name, needsChildRef = false) => {
    ...
    return {
      ...
      disabled:
        getIsFieldDisabled(name) || formState.isFieldDisabled(form, name),
      isError: formState.isFieldInvalid(form, name),
      errorMessage: (
        formState.getFieldErrorMessages(form, camelCase(name)) || []
      ).join("\n"),
  ...
  createField = (name, componentClass, additionalProps, needsChildRef) => (
    <div
      className={`contact-details-form-fields__container ${kebabCase(name)}`}
    >
      {createElement(
        componentClass,
        Object.assign(
      ...
      <div className="contact-details-form-fields__contact-details">
        {this.createField(
          ...
          HiddenInput,
          {
            ...
            text:
              labelTexts.organization || translate("+ Add organization name")
          },
          true
        )}
