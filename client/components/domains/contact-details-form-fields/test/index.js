...
describe("ContactDetailsFormFields", () => {
  ...
  describe("default fields", () => {
    ...
    test("should render fields when contact details contains no values", () => {
      ...
      expect(
        wrapper.find(".contact-details-form-fields__container.first-name")
      ).toHaveLength(1);
      expect(
        wrapper.find(".contact-details-form-fields__container.last-name")
      ).toHaveLength(1);
      expect(
        wrapper.find(".contact-details-form-fields__container.phone")
      ).toHaveLength(1);
    });
  });
  ...
  describe("Google Apps Form UI state", () => {
    test("should not render GAppsFieldset in place of the default contact fields by default", () => {
      ...
      expect(wrapper.find(".contact-details-form-fields__g-apps")).toHaveLength(
        0
      );
  ...
  describe("Country selection", () => {
    ...
    test("should not render address fieldset when no country selected", () => {
      const newProps = update(defaultProps, {
        contactDetails: { countryCode: { $set: "" } }
      });
  ...
  describe("Fax field", () => {
    test("should not render fax field by default", () => {
      ...
      expect(
        wrapper.find(".contact-details-form-fields__container.fax")
      ).toHaveLength(0);
    });

    test("should render fax field when fax required", () => {
      const wrapper = shallow(
        <ContactDetailsFormFields {...defaultProps} needsFax={true} />
      );
  ...
  describe("onCancel", () => {
    test("should not render cancel button by default", () => {
      ...
      expect(
        wrapper.find(".contact-details-form-fields__cancel-button")
      ).toHaveLength(0);
    });

    test("should render cancel button when `onCancel` method prop passed", () => {
      const wrapper = shallow(
        <ContactDetailsFormFields {...defaultProps} onCancel={noop} />
      );
  ...
  describe("Setting phone input country", () => {
    test("should use address country if available", () => {
      ...
      expect(wrapper.find("FormPhoneMediaInput").props().countryCode).toEqual(
        "JP"
      );
    });
