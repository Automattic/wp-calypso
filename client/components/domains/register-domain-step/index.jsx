...
const PAGE_SIZE = 10;
const MAX_PAGES = 3;
const SUGGESTION_QUANTITY = isPaginationEnabled
  ? PAGE_SIZE * MAX_PAGES
  : PAGE_SIZE;
...
function reportSearchStats({ query, section, timestamp }) {
  ...
  if (lastSearchTimestamp) {
    timeDiffFromLastSearchInSeconds = Math.floor(
      (timestamp - lastSearchTimestamp) / 1000
    );
  }
...
class RegisterDomainStep extends React.Component {
  ...
  getState() {
    const suggestion = this.props.suggestion
      ? getFixedDomainSearch(this.props.suggestion)
      : "";
    ...
    return {
      ...
      loadingSubdomainResults:
        this.props.includeWordPressDotCom && loadingResults,
      showNotice: false,
      pageNumber: 1,
      searchResults: null,
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      this.props.defaultSuggestionsError ===
        nextProps.defaultSuggestionsError ||
      (!this.props.defaultSuggestionsError &&
        !nextProps.defaultSuggestionsError)
    ) {
      return;
  ...
  isLoadingSuggestions() {
    return (
      !this.props.defaultSuggestions && !this.props.defaultSuggestionsError
    );
  }
  ...
  getSuggestionsFromProps() {
    ...
    if (
      ...
      if (this.state.loadingSubdomainResults && !this.state.loadingResults) {
        ...
        this.state.subdomainSearchResults.length
      ) {
      ...
      <div className="register-domain-step">
        <StickyPanel className="register-domain-step__search">
          <CompactCard>
            <Search
              additionalClasses={
                this.state.clickedExampleSuggestion ? "is-refocused" : undefined
              }
        ...
        {showTldFilterBar && (
          <TldFilterBar
            ...
            showPlaceholder={
              this.state.loadingResults || !this.getSuggestionsFromProps()
            }
          />
        )}
        {message && (
  ...
  repeatSearch = (
    ...
    debug(
      "Repeating a search with the following input for setState",
      nextState
    );
  ...
  handleDomainSuggestions = domain => results => {
    ...
    flatten(compact(results)).forEach(result => {
      ...
      suggestionMap.has(domainName)
        ? suggestionMap.set(domainName, {
            ...suggestionMap.get(domainName),
            ...result
          })
  ...
  getSubdomainSuggestions = (domain, timestamp) => {
    const includeWordPressDotCom =
      this.props.surveyVertical && this.props.includeDotBlogSubdomain
        ? false
        : true;
  ...
  handleSubdomainSuggestions = (domain, timestamp) => subdomainSuggestions => {
    this.props.onDomainsAvailabilityChange(true);
    const timeDiff = Date.now() - timestamp;
    const analyticsResults = subdomainSuggestions.map(
      suggestion => suggestion.domain_name
    );
  ...
  showNextPage = () => {
    ...
    debug(
      `Showing page ${pageNumber} with query "${
        this.state.lastQuery
      }" in section "${this.props.analyticsSection}"`
    );
  ...
  renderInitialSuggestions() {
    ...
    if (this.isLoadingSuggestions() || isEmpty(this.props.products)) {
      domainRegistrationSuggestions = times(
        INITIAL_SUGGESTION_QUANTITY + 1,
        function(n) {
      ...
      suggestions = (this.props.defaultSuggestions || []).slice(
        0,
        INITIAL_SUGGESTION_QUANTITY
      );
  ...
  renderSearchResults() {
    ...
    const matchesSearchedDomain = suggestion =>
