...
class DomainSearchResults extends React.Component {
  ...
  renderDomainAvailability() {
    ...
    if (
      ...
      const components = {
        a: <a href="#" onClick={this.handleAddMapping} />,
        small: <small />
      };
      ...
      if (isNextDomainFree(this.props.cart)) {
        ...
        offer = translate(
          ...
          {
            args: { domain, cost: this.props.products.domain_map.cost_display },
            components
          }
        );
      } else {
      ...
      let domainUnavailableMessage = includes(
        [TLD_NOT_SUPPORTED, UNKNOWN],
        lastDomainStatus
      )
        ? translate(
      ...
      if (this.props.offerUnavailableOption) {
        if (
          ...
          availabilityElement = (
            <Card
              className="domain-search-results__transfer-card"
              highlight="info"
            >
                  ...
                  components: {
                    a: (
                      <a
                        ...
                        data-tracks-button-click-source={
                          this.props.tracksButtonClickSource
                        }
                      />
                    )
                  }
    ...
    if (!this.props.isLoadingSuggestions && this.props.suggestions) {
      suggestionCount = (
        <div aria-live="polite">
          <ScreenReaderText>
            {this.props.translate("%s domains found", {
              args: this.props.suggestions.length
            })}
          </ScreenReaderText>
        </div>
      );
      ...
      const bestMatchSuggestions = suggestions.filter(
        suggestion => suggestion.isRecommended
      );
      ...
      suggestionElements = regularSuggestions.map((suggestion, i) => {
        ...
        return (
          <DomainRegistrationSuggestion
            ...
            uiPosition={i}
            fetchAlgo={
              endsWith(suggestion.domain_name, ".wordpress.com")
                ? "wpcom"
                : this.props.fetchAlgo
            }
            query={this.props.lastDomainSearched}
            onButtonClick={this.props.onClickResult}
          />
