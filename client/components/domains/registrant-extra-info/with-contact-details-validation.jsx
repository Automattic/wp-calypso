...
export function disableSubmitButton(children) {
  ...
  return map(castArray(children), (child, index) =>
    React.cloneElement(child, {
      disabled:
        !!child.props.className.match(/submit-button/) || child.props.disabled,
      key: index
    })
  );
}
...
export default function WithContactDetailsValidation(tld, WrappedComponent) {
  ...
  class ComponentWithValidation extends Component {
    ...
    displayName =
      "WithContactDetailsValidation(" + tld + ", " + wrappedComponentName + ")";
    ...
    compileValidator() {
      ...
      debug(
        `compiling validation schema for ${tld}`,
    ...
    validateContactDetails() {
      ...
      try {
        ...
        this.props.recordTracksEvent(
          "calypso_domain_contact_validation_exception",
          {
            tld,
        ...
        return {
          "": ["There was an unexpected error validating these details"]
        };
      }
