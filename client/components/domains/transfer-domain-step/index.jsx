...
class TransferDomainStep extends React.Component {
  ...
  componentWillMount() {
    if (this.props.initialState) {
      this.setState(
        Object.assign({}, this.props.initialState, this.getDefaultState())
      );
    }
  ...
  addTransfer() {
    ...
    const domainsWithPlansOnlyButNoPlan =
      domainsWithPlansOnly &&
      ((selectedSite && !isPlan(selectedSite.plan)) || isSignupStep);
    ...
    return (
      <div>
        ...
        <Card className="transfer-domain-step__map-option">
          <strong>
            {translate("Manage your domain and site separately.")}
          </strong>
          <p>
            {translate(
    ...
    startInboundTransfer(selectedSite.ID, domain, authCode, (error, result) => {
      if (result) {
        ...
        this.props.errorNotice(
          translate("We were unable to start the transfer.")
        );
      }
    });
  };
  ...
  transferIsRestricted = () => {
    ...
    return (
      ...
      "not_restricted" !== transferRestrictionStatus
    );
  };
    ...
    Promise.all([this.getInboundTransferStatus(), this.getAvailability()]).then(
      () => {
        this.setState(prevState => {
          ...
          return {
            precheck:
              prevState.domain && !submittingAvailability && !submittingWhois
          };
        });
...
const recordGoButtonClickInTransferDomain = (domain_name, section) =>
  recordTracksEvent("calypso_transfer_domain_go_click", {
    domain_name,
    section
  });
...
const recordMapDomainButtonClick = section =>
  composeAnalytics(
    ...
    recordTracksEvent("calypso_transfer_domain_mapping_button_click", {
      section
    })
  );
