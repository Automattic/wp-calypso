...
class TransferDomainPrecheck extends React.Component {
  ...
  onClick = () => {
    ...
    this.props.recordContinueButtonClick(
      domain,
      losingRegistrar,
      losingRegistrarIanaId
    );
  ...
  getSection(heading, message, buttonText, step, stepStatus, onButtonClick) {
    ...
    const sectionIcon = isStepFinished ? (
      <Gridicon icon="checkmark-circle" size={36} />
    ) : (
      step
    );
    ...
    return (
      <Card compact>
        <div className={sectionClasses}>
          <span className="transfer-domain-step__section-heading-number">
            {sectionIcon}
          </span>
  ...
  getStatusMessage() {
    ...
    let lockStatusClasses =
      "transfer-domain-step__lock-status transfer-domain-step__unavailable";
    if (true === unlocked) {
      lockStatusClasses =
    ...
    const message = (
      <div>
        ...
        <div>
          ...
          {authCodeInvalid && (
            <FormInputValidation
              text={translate("Auth Code invalid")}
              isError
            />
          )}
        </div>
...
const recordNextStep = (domain_name, show_step) =>
  recordTracksEvent("calypso_transfer_domain_precheck_step_change", {
    domain_name,
    show_step
  });

const recordContinueButtonClick = (
  domain_name,
  losing_registrar,
