/**
 * A high-order component that will scroll to the bottom of a scrollable container whenever it's rendered.
 ...
 *
 * To use, wrap a component with `withAutoScroll` and then call the `this.props.setupAutoscroll( node )`
 * method in your component where `node` is an html element with scrolling enabled.
 *
...
const withAutoScroll = ( WrappedComponent ) => {
	class AutoScroll extends React.Component {
		static displayName = `withAutoScroll(${
		...
		render() {
			return <WrappedComponent { ...this.props } setupAutoscroll={ this.setupAutoscroll } />;
		}
	}
