...
const storageToUpsellProduct: Record< number, string > = {
	[ BYTES_10GB ]: PRODUCT_JETPACK_BACKUP_ADDON_10GB_MONTHLY,
	[ BYTES_100GB ]: PRODUCT_JETPACK_BACKUP_ADDON_100GB_MONTHLY,
	[ BYTES_1TB ]: PRODUCT_JETPACK_BACKUP_ADDON_1TB_MONTHLY,
};
...
export default ( siteId: number | null ) => {
	...
	const upsellSlug = useMemo( () => {
		const ADD_ON_STORAGE_MAP = {
			[ PRODUCT_JETPACK_BACKUP_ADDON_10GB_MONTHLY ]: TEN_GIGABYTES,
			[ PRODUCT_JETPACK_BACKUP_ADDON_100GB_MONTHLY ]: HUNDRED_GIGABYTES,
			[ PRODUCT_JETPACK_BACKUP_ADDON_1TB_MONTHLY ]: ONE_TERABYTES,
		};
		...
		if ( sitePurchases.some( isJetpackProductSlugMatch( UPGRADEABLE_STORAGE_PRODUCT_SLUGS_T2 ) ) ) {
			return {
				...slugToSelectorProduct( PRODUCT_JETPACK_BACKUP_ADDON_STORAGE_1TB_MONTHLY ),
				storage: ADD_ON_STORAGE_MAP[ PRODUCT_JETPACK_BACKUP_STORAGE_ADDON_1TB_MONTHLY ],
			} as SelectorProductWithStorage;
		}
		return {
			...slugToSelectorProduct( PRODUCT_JETPACK_BACKUP_ADDON_STORAGE_10GB_MONTHLY ),
