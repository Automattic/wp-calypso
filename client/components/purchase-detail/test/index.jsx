...
describe("PurchaseDetail", () => {
  ...
  test("should render given title and description", () => {
    wrapper = shallow(
      <PurchaseDetail title="test:title" description="test:description" />
    );
    expect(wrapper.find(".purchase-detail__title").props().children).to.equal(
      "test:title"
    );
    expect(
      wrapper.find(".purchase-detail__description").props().children
  ...
  test("should render a <PurchaseButton> with given info unless the body text is passed", () => {
    ...
    expect(purchaseButton.prop("href")).to.equal(
      "https://wordpress.com/test/url"
    );
