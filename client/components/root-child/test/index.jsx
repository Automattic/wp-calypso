...
describe("RootChild", () => {
  ...
  describe("rendering", () => {
    ...
    test("accepts props to be added to a wrapper element", () => {
      ...
      expect(tree.refs.rootChild.parentNode.parentNode.parentNode).to.eql(
        document.body
      );
    });

    test("should update the children if parent is re-rendered", () => {
      const tree = mount(React.createElement(Greeting), {
        attachTo: container
      });
  ...
  describe("unmounting", () => {
    test("should destroy the root child when the component is unmounted", () => {
      ...
      expect([].slice.call(document.body.querySelectorAll("*"))).to.eql([
        container
      ]);
    });
  });
