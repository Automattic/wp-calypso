...
class CancelPurchaseForm extends Component {
	...
	surveyContent() {
		const {
			...
			purchase,
			site,
			hasInstallPluginsFeature,
			hasInstallPurchasedPluginsFeature,
			hasInstallThemesFeature,
			hasSftpFeature,
			hasBackupsFeature,
		} = this.props;
		...
		if ( surveyStep === ATOMIC_REVERT_STEP ) {
			...
			let subHeaderText;
			if ( hasInstallPluginsFeature || hasInstallThemesFeature || hasSftpFeature ) {
				subHeaderText = translate(
			...
			} else if ( hasInstallPurchasedPluginsFeature ) {
				subHeaderText = translate(
...
export default connect(
	( state, { purchase } ) => ( {
		...
		hasInstallPluginsFeature: siteHasFeature(
			state,
			purchase.siteId,
			WPCOM_FEATURES_INSTALL_PLUGINS
		),
		hasInstallPurchasedPluginsFeature: siteHasFeature(
			state,
			purchase.siteId,
			WPCOM_FEATURES_INSTALL_PURCHASED_PLUGINS
		),
		hasInstallThemesFeature: siteHasFeature( state, purchase.siteId, FEATURE_INSTALL_THEMES ),
		hasSftpFeature: siteHasFeature( state, purchase.siteId, FEATURE_SFTP ),
