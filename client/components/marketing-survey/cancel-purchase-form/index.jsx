...
class CancelPurchaseForm extends Component {
	...
	surveyContent() {
		...
		if ( surveyStep === ATOMIC_REVERT_STEP ) {
			...
			let subHeaderText;
			if ( canInstallPlugins || canInstallThemes || canUseSftp ) {
				subHeaderText = translate(
			...
			} else if ( canInstallPremiumPlugins ) {
				subHeaderText = translate(
					'If you deactivate your product, we will set your site to private and revert it to the point when you installed your first plugin ' +
						'on {{strong}}%(atomicTransferDate)s{{/strong}}. All of your posts, pages, and media will be preserved, except for content generated by plugins. {{moreInfoTooltip/}}',
					{
						args: { atomicTransferDate },
						components: {
							moreInfoTooltip: (
								<InfoPopover className="cancel-purchase-form__atomic-revert-more-info">
									{ translate(
										'On %(atomicTransferDate)s, we automatically moved your site to a platform that supports the usage of plugins. ' +
...
export default connect(
	( state, { purchase } ) => ( {
		...
		canInstallPlugins: siteHasFeature( state, purchase.siteId, WPCOM_FEATURES_INSTALL_PLUGINS ),
		canInstallPremiumPlugins: siteHasFeature(
			state,
			purchase.siteId,
			WPCOM_FEATURES_INSTALL_PURCHASED_PLUGINS
		),
		canInstallThemes: siteHasFeature( state, purchase.siteId, FEATURE_INSTALL_THEMES ),
		canUseSftp: siteHasFeature( state, purchase.siteId, FEATURE_SFTP ),
		canDownloadBackups: siteHasFeature( state, purchase.siteId, WPCOM_FEATURES_BACKUPS ),
	} ),
	{
		fetchAtomicTransfer,
		recordTracksEvent,
