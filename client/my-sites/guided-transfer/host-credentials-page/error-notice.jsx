...
const getErrorText = ( { translate, errorCode } ) => {
	switch ( errorCode ) {
		case 'exceeded-max-validation-attempts':
			return translate( "You've entered invalid details one too many times. Please contact support." );
		case 'verify-details-failed':
			return translate( 'Those hosting details are incorrect. Please try again or contact support.' );
		case 'details-already-validated':
			return translate( "We've already confirmed your details. Please contact support if you need to change them." );
	}
...
const ErrorNotice = localize( ( { translate, errorCode } ) => {
	...
	return <Notice showDismiss={ false } status="is-error" text={ getErrorText( { translate, errorCode } ) }>
