...
export default React.createClass({
    ...
    renewLink(count) {
        const fullMessage = this.translate('Renew it now.', 'Renew them now.', {
            ...
            compactMessage = this.translate('Renew', {
                context: 'Call to action link for renewing an expiring/expired domain',
            });
        return (
    ...
    expiringDomainsCanManage() {
        ...
        if (expiringDomains.length === 1) {
            text = this.translate(
                '{{strong}}%(domainName)s{{/strong}} is expiring %(timeUntil)s.',
                {
                    components: { strong: <strong /> },
                    args: {
    ...
    unverifiedDomainsCanManage() {
        ...
        if (domains.length === 1) {
            ...
            return (
                ...
                >
                    <NoticeAction
                        href={paths.domainManagementEdit(this.props.selectedSite.slug, domain)}
                    >
                        {this.translate('Fix')}
                    </NoticeAction>
                </Notice>
    ...
    pendingGappsTosAcceptanceDomains() {
        ...
        return pendingDomains.length !== 0 &&
    ...
    pendingTransfer() {
        ...
        const compactNotice = this.translate(
