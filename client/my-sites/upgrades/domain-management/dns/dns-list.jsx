...
const DnsList = React.createClass({
    ...
    deleteDns: function(record, confirmed = false) {
        ...
        deleteDnsAction(this.props.selectedDomainName, record, error => {
            if (error) {
                notices.error(
                    error.message || this.translate('The DNS record has not been deleted.')
                );
            } else {
    ...
    addDns: function(record) {
        addDnsAction(this.props.selectedDomainName, record, error => {
            if (error) {
                notices.error(
                    error.message || this.translate('The DNS record could not be restored.')
                );
            } else {
    ...
    render: function() {
        const { dialog } = this.state,
            ...
            dnsRecordsList = dns.records.map(
                function(dnsRecord, index) {
                    return (
                        <DnsRecord
                            key={index}
                            dnsRecord={dnsRecord}
