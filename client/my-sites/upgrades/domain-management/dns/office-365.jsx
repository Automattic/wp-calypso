...
class Office365 extends Component {
    ...
    onAddDnsRecords = event => {
        ...
        upgradesActions.addDnsOffice(this.props.selectedDomainName, this.state.token, error => {
            if (error) {
                notices.error(
                    error.message || this.props.translate('The DNS record has not been added.')
                );
                this.setState({ submitting: false });
            } else {
                notices.success(
                    this.props.translate('All DNS records that Office 365 needs have been added.'),
                    {
                        duration: 5000,
                    }
                );
                    ...
                    <FormFieldset>
                        <FormLabel>
                            {this.props.translate(
                                'Office 365 Verification Token - from the TXT record verification'
                            )}
                        </FormLabel>
                        <FormTextInput
                            name="token"
                        ...
                        {this.state.token &&
                            !isDataValid &&
                            <FormInputValidation
                                text={this.props.translate('Invalid Token')}
                                isError={true}
                            />}
                    </FormFieldset>
