...
class JetpackThankYouCard extends Component {
    ...
    warnIfNotFinished(event) {
        ...
        const beforeUnloadText = this.props.translate(
            "We haven't finished installing your plugins."
        );
    ...
    renderPlugins() {
        ...
        let mappedPlugins;
        if (
            !this.props.hasRequested || this.props.isRequesting || PluginsStore.isFetchingSite(site)
        ) {
    ...
    renderAction() {
        ...
        const countCompletion = plugins.reduce(
            (total, plugin) => {
                switch (plugin.status) {
                    case 'done':
                        total += 3;
        ...
        return <ProgressBar value={countCompletion} total={countSteps} isPulsing />;
    }
