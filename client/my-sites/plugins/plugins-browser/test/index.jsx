...
describe('PluginsBrowser basic tests', () => {
	...
	test('should show upsell nudge when appropriate', () => {
		...
		expect(comp.find('UpsellNudge[event="calypso_plugins_browser_upgrade_nudge"]').length).toBe(1);
	});
	test('should not show upsell nudge if no site is selected', () => {
		...
		expect(comp.find('UpsellNudge[event="calypso_plugins_browser_upgrade_nudge"]').length).toBe(0);
	});
