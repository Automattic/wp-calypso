...
describe('PluginMeta basic tests', () => {
	...
	test('should show upgrade nudge when appropriate', () => {
		...
		expect(comp.find('UpsellNudge[event="calypso_plugin_detail_page_upgrade_nudge"]')).toHaveLength(
			1
		);
	});
	test('should not show upgrade nudge if no site is selected', () => {
		const comp = shallow(
			<PluginMeta {...props} selectedSite={null} isJetpackSite={false} hasBusinessPlan={false} />
		);
		expect(comp.find('UpsellNudge[event="calypso_plugin_detail_page_upgrade_nudge"]')).toHaveLength(
			0
		);
