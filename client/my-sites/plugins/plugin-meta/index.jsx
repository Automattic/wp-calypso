...
const PluginMeta = React.createClass({
    ...
    hasBusinessPlan() {
        ...
        return isBusiness(this.props.selectedSite.plan) ||
    ...
    isWpcomPreinstalled: function() {
        ...
        return !this.props.selectedSite.jetpack &&
            includes(installedPlugins, this.props.plugin.name);
    },
    ...
    getInstallButton() {
        ...
        if (selectedSite && selectedSite.jetpack && this.hasOrgInstallButton()) {
            return (
                <PluginInstallButton disabled={this.isJetpackInstallDisabled()} {...this.props} />
            );
        }
    ...
    maybeDisplayUnsupportedNotice() {
        ...
        if (
            selectedSite &&
            this.isUnsupportedPlugin() &&
            (!selectedSite.jetpack || automatedTransferSite)
        ) {
            return (
                <Notice
                    text={this.translate(
                        'Incompatible plugin: WordPress.com already provides this feature.'
                    )}
                    status="is-warning"
                    showDismiss={false}
                >
                ...
                <Card>
                    ...
                    {!this.props.isMock &&
                        ...
                        <PluginInformation
                            ...
                            siteVersion={
                                this.props.selectedSite &&
                                    this.props.selectedSite.options.software_version
                            }
                            hasUpdate={this.getAvailableNewVersions().length > 0}
                        />}
                </Card>

                {isATEnabledForCurrentSite() && this.maybeDisplayUnsupportedNotice()}
