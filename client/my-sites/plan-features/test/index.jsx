...
describe('isPrimaryUpgradeByPlanDelta', () => {
	test('Should return true when called with blogger and personal plan', () => {
		...
		expect(isPrimaryUpgradeByPlanDelta(PLAN_BLOGGER_2_YEARS, PLAN_PERSONAL_2_YEARS)).toBe(true);
	});
	...
	test('Should return true when called with personal and premium plan', () => {
		...
		expect(isPrimaryUpgradeByPlanDelta(PLAN_PERSONAL_2_YEARS, PLAN_PREMIUM_2_YEARS)).toBe(true);
	});
	...
	test('Should return true when called with premium and business plan', () => {
		...
		expect(isPrimaryUpgradeByPlanDelta(PLAN_PREMIUM_2_YEARS, PLAN_BUSINESS_2_YEARS)).toBe(true);
	});
	...
	test('Should return false when called with other plan combinations', () => {
		...
		expect(isPrimaryUpgradeByPlanDelta(PLAN_JETPACK_PERSONAL, PLAN_JETPACK_PREMIUM)).toBe(false);
		expect(isPrimaryUpgradeByPlanDelta(PLAN_JETPACK_PERSONAL, PLAN_JETPACK_PREMIUM_MONTHLY)).toBe(
			false
		);
		expect(
...
describe('calculatePlanCredits', () => {
	...
	test('Should return 0 when no plan is available', () => {
		...
		const credits = calculatePlanCredits({}, 1, [{ ...baseProps, availableForPurchase: false }]);
...
describe('PlanFeatures.renderCreditNotice', () => {
	const baseProps = {
		...
		hasPlaceholders: false,
		planCredits: 5,
		planProperties: [{ currencyCode: 'USD', planName: 'test-bundle', availableForPurchase: true }],
