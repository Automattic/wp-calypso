...
const PostSelectorPosts = React.createClass({
    ...
    componentWillReceiveProps(nextProps) {
        if (!isEqual(this.props.query, nextProps.query) || this.props.siteId !== nextProps.siteId) {
            this.setState({
                requestedPages: [1],
            });
        }
    ...
    componentDidUpdate(prevProps) {
        const forceUpdate = prevProps.selected !== this.props.selected ||
    ...
    getCompactContainerHeight() {
        return range(0, this.getRowCount()).reduce(
            (memo, index) => {
                            ...
                            >
                                {decodeEntities(
                                    get(
                                        ...
                                        [item.type, 'labels', 'singular_name'],
                                        ''
                                    )
                                )}
                            </span>}
        ...
        return (
            <div className={classes}>
                {requestedPages.map(page => (
                    <QueryPosts key={`page-${page}`} siteId={siteId} query={{ ...query, page }} />
                ))}
                {isTypeLabelsVisible && siteId && <QueryPostTypes siteId={siteId} />}
                {showSearch && <Search searchTerm={searchTerm} onSearch={this.onSearch} />}
                <div className="post-selector__results">
                    <AutoSizer key={JSON.stringify(query)} disableHeight={isCompact}>
                        {({ height, width }) => (
                            <List
                                ref={this.setListRef}
                                width={width}
