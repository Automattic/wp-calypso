...
var MenuEditableItem = React.createClass({
    ...
    setItemContent: function(content) {
        ...
        if (
            ...
            if (isInjectedNewPageItem(content)) {
                ...
                this.setState({
                    placeholderText: this.translate('Enter page name', { textOnly: true }),
                });
            } else {
                this.setState({
                    placeholderText: this.translate('Enter menu label', { textOnly: true }),
        ...
        return (
            <div className="menu-item-options menu-item-url">
                ...
                <label className="menu-item-form-label">
                    {this.translate('Link address (URL)')}
                </label>
                <input
                ...
                />
                <label htmlFor="menu-flag-open-in-new-window">
                    {this.translate('Open link in new window/tab')}
                </label>
            </div>
        );
    },
            ...
            <ul className="menus__menu-item-form-types">
                {this.props.itemTypes.map(
                    function(itemType) {
                        ...
                        isSelected = itemType.name === this.state.itemType.name
                            ? 'is-selected'
                            : '';
    ...
    renderUnsupportedNotice: function() {
        ...
        body = this.translate(
            'You can manage this item as usual, but you cannot edit it here. {{br/}} Check the {{wpAdminLink}}WP Admin{{/wpAdminLink}} and see if you can edit it from there.',
            {
                components: {
                    br: <br />,
                    wpAdminLink: (
                        <a
                            href={MenuUtils.getNavMenusUrl()}
                            target="_blank"
    ...
    renderButtons: function() {
        return [
            ...
            function(button) {
                return (this.isNew() && button.showIfNew) || !this.isNew()
                    ? <Button {...button} />
                    : null;
            },
            this
