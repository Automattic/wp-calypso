...
export const siteComments = ( context, next ) => {
	...
	if ( ! siteFragment ) {
		return page.redirect( '/promote/all' );
	}

	const status = mapPendingStatusToUnapproved( params.status );
	const analyticsPath = `/promote/${ status }/:site`;
...
export const postComments = ( context, next ) => {
	...
	const analyticsPath = `/promote/${ status }/:site/:post`;

	if ( ! postId ) {
		return page.redirect( `/promote/${ params.status }/${ siteFragment }` );
	}
...
export const comment = ( context, next ) => {
	...
	if ( ! commentId ) {
		return siteFragment
			? page.redirect( `/promote/all/${ siteFragment }` )
			: page.redirect( '/promote/all' );
	}

	const action = sanitizeQueryAction( query.action );
	const redirectToPostView = ( postId ) => () =>
		page.redirect( `/promote/all/${ siteFragment }/${ postId }` );
...
export const redirect = ( { path } ) => {
	const siteFragment = getSiteFragment( path );
	if ( siteFragment ) {
		return page.redirect( `/promote/all/${ siteFragment }` );
	}
	return page.redirect( '/promote/all' );
};
...
export const clearCommentNotices = ( { store }, next ) => {
	...
	if ( ! startsWith( nextPath, '/promote' ) ) {
