...
import FeatureFooter from './footer';
...
export default function( context ) {
	...
	let header = i18n.translate( 'Get Custom Design' );
	let title;
	let description;
	let upgradedPlan;
	let featuresToShow;
	...
	function checkoutPlan() {
		page( '/checkout/' + context.params.domain + '/' + getPlanCheckoutSlug( upgradedPlan ) );
	}

	if ( planSlug === 'free_plan' ) {
		upgradedPlan = 'value_bundle';
		title = i18n.translate( 'Get Custom Design with Premium' );
		description = i18n.translate( 'Make your site unique with custom Fonts, Colors, CSS, domain name and variety of other features by upgrading to Premium plan!' );
		featuresToShow = [
			'more-storage',
			'custom-domain',
			'no-ads',
		...
		successCard = <Card>{ i18n.translate( 'This is a Jetpack site hosted on your own serwer. You may have a variety of customization options depending on your theme.' ) }</Card>
	} else {
		successCard = <Card href={ `customize/${ context.params.domain }?nomuse=1` }>{ i18n.translate( 'You already own an upgrade on this site and you can use custom design. Good Job! Click here to customize your site.' ) }</Card>
	}

	renderWithReduxStore(
		<Main>
			<HeaderCake onClick={ goBack }>{ header }</HeaderCake>
			<Card compact>
				{ /*eslint-disable react/no-danger*/ }
				<div className="plan-feature__logo" dangerouslySetInnerHTML={ { __html: svgLogo } } />
				<h1 className="plan-feature__title">{ title }</h1>
				{ /*eslint-disable react/no-danger*/ }
				<span className="plan-feature__description">{ description }</span>
