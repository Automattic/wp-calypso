...
export default function( context ) {
	...
	let header = i18n.translate( 'Get More Space' );
	let title;
	let description;
	let upgradedFeature;
	...
	if ( planSlug === 'free_plan' ) {
		currentFeatureTitle = i18n.translate( '3GB Space' );
		upgradedFeature = 'more-storage';
		upgradedPlan = 'value_bundle';
		title = i18n.translate( 'Get more space with Premium' );
		description = i18n.translate( 'Supercharge your site with more space, video support, a custom domain and much more.' );
		featuresToShow = [
			'custom-domain',
			'no-ads',
			'custom-design',
	...
	} else if ( planSlug === 'value_bundle' ) {
		upgradedFeature = 'unlimited-storage';
		currentFeatureTitle = getFeatureTitle( 'more-storage' );
		upgradedPlan = 'business-bundle';
		title = i18n.translate( 'Get unlimited space with Business' );
		description = i18n.translate( 'Supercharge your site with unlimited space for your data, google analytics and more' );
		featuresToShow = [
			'unlimited-premium-themes',
			'live-chat-support',
		];
	} else if ( planSlug === 'business-bundle' ) {
		successCard = <Card>{ i18n.translate( 'You already own business upgrade on this site and you have unlimited storage space. Good Job!' ) }</Card>
	} else if ( site.jetpack ) {
		successCard = <Card>{ i18n.translate( 'This is a Jetpack site hosted on your own serwer. You should contact your hosting provider if you want to know more.' ) }</Card>
	}
