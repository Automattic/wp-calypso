...
class DnsList extends React.Component {
	...
	deleteDns = (record, confirmed = false) => {
		...
		this.props.deleteDns(selectedDomainName, record).then(
			...
			(error) => {
				this.props.errorNotice(error.message || translate('The DNS record has not been deleted.'));
			}
		);
	};
	...
	addDns(record) {
		...
		this.props.addDns(this.props.selectedDomainName, record).then(
			...
			(error) => {
				this.props.errorNotice(error.message || translate('The DNS record could not be restored.'));
			}
		);
	}
