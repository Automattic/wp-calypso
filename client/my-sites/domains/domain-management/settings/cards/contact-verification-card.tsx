...
const ContactVerificationCard: FunctionComponent< Props > = ( props ) => {
	...
	const tldType = getVerificationTldType( props.selectedDomainName );
	...
	const getDocumentList = (): string[] => {
		const acceptedDocuments = getAcceptedDocuments( translate );
		if ( tldType === 'uk' ) {
	...
	const renderNationalitySelector = () => {
		...
		return (
			<RadioControl
				label={ translate( 'Are you an Indian national?' ) }
				selected={ nationalityType }
				options={ [
					{
						label: translate( 'Indian national' ),
						value: 'indian_national',
					},
					{
						label: translate( 'Foreign national' ),
	...
	const handleFilesSelected = ( files: FileList ) => {
		...
		const oversizedFiles = [ ...files ].filter( ( file: File ) => file.size > maxFileSize );
		if ( oversizedFiles.length > 0 ) {
			dispatch( errorNotice( translate( 'Files must be under 5MB each.' ) ) );
			return;
		}
	...
	const submitFiles = () => {
		...
		setSubmitting( true );
		const formData: ( [ string, File, string ] | [ string, string ] )[] = [];
		[ ...selectedFiles ].forEach( ( file: File ) => {
			formData.push( [ 'files[]', file, file.name ] );
		} );

		if ( tldType === 'in' ) {
			formData.push( [ 'nationality_type', nationalityType ] );
		}
