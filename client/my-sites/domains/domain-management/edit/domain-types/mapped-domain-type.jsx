...
class MappedDomainType extends React.Component {
	resolveStatus() {
		...
		if (!this.props.isJetpackSite && !this.props.isAutomatedTransferSite && !domain.pointsToWpcom) {
			return {
	...
	(state, ownProps) => {
		...
		return {
			purchase: purchaseSubscriptionId
				...
				: null,
			mappingPurchase: subscriptionId ? getByPurchaseId(state, parseInt(subscriptionId, 10)) : null,
