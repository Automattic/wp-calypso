			...
			.then((response) => {
				...
				if (migrationStatus) {
					if (startTime && isEmpty(this.state.startTime)) {
						...
						const localizedStartTime = startMoment.local().locale(getLocaleSlug()).format('lll');
	...
	renderProgressItem(progressState) {
		...
		return (
			<li key={`progress-${progressState}`} className="migrate__progress-item">
				<div className="migrate__progress-item-icon">{this.renderProgressIcon(progressState)}</div>
