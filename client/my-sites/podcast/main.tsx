...
const PodcastMain = ( { section }: PodcastMainProps ) => {
	...
	useEffect( () => {
		...
		const isSectionUrl = /^\/settings\/podcast\/(episodes|distribution|settings|stats)(\/|$)/.test(
			path
		);
		const isContentSectionUrl = /^\/settings\/podcast\/(episodes|distribution|stats)(\/|$)/.test(
			path
		);
	...
	const tabs = [
		...
		},
		{
			...
			path: '/settings/podcast/stats' + pathSuffix,
		},
	];
