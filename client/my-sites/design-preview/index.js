...
@keyframes bounce-appear {
	...
	to {
		...
		transform: scale3d(1, 1, 1);
			...
			const updateElement = ( element, changes ) => Object.assign( {}, element, changes );
			const findElement = element => updateElement( element, { target: previewDocument.querySelector( element.selector ) } );
			...
			const addIcon = element => {
				const icon = previewDocument.createElement( 'div' );
				icon.className = 'cdm-icon cdm-icon--text';
				icon.innerHTML = editIcon;
				previewDocument.body.appendChild( icon );
				return updateElement( element, { icon } );
			};
