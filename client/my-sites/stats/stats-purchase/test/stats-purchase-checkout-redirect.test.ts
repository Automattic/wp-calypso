...
// `calypso-products` reads the named export at import time, so both shapes have to resolve to the
// same mock or the module graph fails to load.
...
describe( 'gotoCheckoutPage — site with no WordPress.com connection', () => {
	...
	it( 'declares the buyer unlinked, so checkout offers to link an account', () => {
		expect( checkoutUrl( { siteId: null } ).searchParams.get( 'unlinked' ) ).toBe( '1' );
	} );

	it( 'returns the buyer to the site’s own Stats page', () => {
		expect( checkoutUrl( { siteId: null } ).searchParams.get( 'redirect_to' ) ).toBe(
			`${ ADMIN_URL }admin.php?page=stats&statsPurchaseSuccess=paid`
		);
	} );
	...
	it( 'does not scope checkout to a site it cannot name', () => {
		expect( checkoutUrl( { siteId: null } ).searchParams.get( 'site' ) ).toBeNull();
	} );
} );
...
describe( 'gotoCheckoutPage — connected site', () => {
	...
	it( 'falls back to siteless checkout while the user connection is missing', () => {
		...
		expect( url.pathname ).toBe( '/checkout/jetpack/jetpack_stats_yearly' );
		expect( url.searchParams.get( 'connect_after_checkout' ) ).toBe( 'true' );
		// A known site may still have a linked user, so this path must not claim otherwise.
		expect( url.searchParams.get( 'unlinked' ) ).toBeNull();
	} );
} );
...
describe( 'gotoCheckoutPage — Calypso', () => {
	...
	it( 'uses the site slug when the site is known', () => {
		expect( checkoutUrl().pathname ).toBe( `/checkout/${ SITE_SLUG }/jetpack_stats_yearly` );
	} );
} );
