...
type SwitchOnDialogProps = {
	siteId: number | null;
	/** The new Stats, where the reader lands once the write succeeds. */
	dashboardUrl: string;
	onClose: () => void;
};
...
export default function SwitchOnDialog( { siteId, dashboardUrl, onClose }: SwitchOnDialogProps ) {
	...
	return (
		<Modal
			...
			title={ translate( 'Switch on the new Stats?' ) }
		...
		>
			...
			{ hasFailed && (
				<p className="stats-premium-analytics-preview-dialog__error" role="alert">
					{ translate(
						'We couldn’t switch on the new Stats. Please try again — if it keeps happening, {{link}}get in touch with support{{/link}}.',
						{
							components: {
								link: (
									<a
