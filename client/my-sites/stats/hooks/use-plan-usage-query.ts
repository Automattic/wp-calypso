...
export default function usePlanUsageQuery(
	...
	return useQuery( {
		...
		// A site with no WordPress.com connection has no id and no recorded usage, so the request
		// could only ever 404. Callers already read missing usage as "no tier in use".
		enabled: !! siteId,
	} );
}
