...
module.exports = React.createClass({
    ...
    getSettingsFromStore: function(siteInstance) {
        var site = siteInstance || this.props.site, store = EarningsStore.getById(site.ID) || {};
    ...
    payoutNotice: function() {
        var owed = this.state.earnings && this.state.earnings.total_amount_owed
    ...
    earningsBreakdown: function() {
        var earnings = this.state.earnings && this.state.earnings.total_earnings
            ? Number(this.state.earnings.total_earnings)
            : 0,
            owed = this.state.earnings && this.state.earnings.total_amount_owed
                ? Number(this.state.earnings.total_amount_owed)
                : 0,
            paid = this.state.earnings &&
            ...
            <ul className="earnings_breakdown__list">
                <li className="earnings_breakdown__item">
                    <span className="earnings_breakdown__label">
                        {this.translate('Total earnings', { context: 'Sum of earnings' })}
                    </span>
                    <span className="earnings_breakdown__value">
                        ${this.numberFormat(earnings, 2)}
                    </span>
                </li>
                <li className="earnings_breakdown__item">
                    <span className="earnings_breakdown__label">
                        {this.translate('Total paid', {
                            context: 'Sum of earnings that have been distributed',
                        })}
                    </span>
                ...
                <li className="earnings_breakdown__item">
                    <span className="earnings_breakdown__label">
                        {this.translate('Outstanding amount', {
                            context: 'Sum earnings left unpaid',
                        })}
                    </span>
            ...
            if (earnings.hasOwnProperty(period)) {
                rows.push(
                    <tr key={type + '-' + period}>
                        ...
                        <td className="earnings-history__value">
                            ${this.numberFormat(earnings[period].amount, 2)}
                        </td>
            ...
            <Card className={classes}>
                ...
                <div className="module-content">
                    ...
                    <table>
                        <thead>
                            <tr>
                                <th className="earnings-history__header">
            ...
            <div>
                ...
                {this.state.earnings && this.checkSize(this.state.earnings.wordads)
                    ? this.earningsTable(
                          ...
                          this.translate('Earnings History'),
                          'wordads'
                      )
                    : null}
                {this.state.earnings && this.checkSize(this.state.earnings.sponsored)
                    ? this.earningsTable(
                          ...
                          this.translate('Sponsored Content History'),
                          'sponsored'
                      )
                    : null}
                {this.state.earnings && this.checkSize(this.state.earnings.adjustment)
                    ? this.earningsTable(
                          ...
                          this.translate('Adjustments History'),
                          'adjustment'
                      )
                    : null}
            </div>
