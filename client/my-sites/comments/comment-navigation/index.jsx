...
export class CommentNavigation extends Component {
	...
	setBulkStatus = (newStatus) => () => {
		...
		recordBulkAction(newStatus, selectedComments.length, queryStatus, isPostView ? 'post' : 'site');
					...
					<CommentNavigationTab className="comment-navigation__actions">
						<ButtonGroup>
							{this.statusHasAction('approve') && (
								<Button compact disabled={!selectedCount} onClick={this.setBulkStatus('approved')}>
									{translate('Approve')}
								</Button>
							)}
