...
const ShadowNotice = localize(({ shadowStatus, onUndoClick, translate }) => (
	<div className="page__shadow-notice-cover">
		...
		>
			{shadowStatus.undo && <NoticeAction onClick={onUndoClick}>{translate('Undo')}</NoticeAction>}
		</Notice>
	</div>
));
...
class Page extends Component {
	...
	getEditItem() {
		...
		return (
			<PopoverMenuItem onClick={this.editPage} onMouseOver={preloadEditor} onFocus={preloadEditor}>
				<Gridicon icon="pencil" size={18} />
				{this.props.translate('Edit')}
			</PopoverMenuItem>
		);
