...
export default function usePrepareProductsForCart({
	...
	const planSlug = useSelector((state) => getUpgradePlanSlugFromPath(state, siteId, productAlias));
...
function useAddRenewalItems({ originalPurchaseId, productAlias, setState }) {
	...
	}, [isFetchingProducts, products, originalPurchaseId, productAlias, setState, selectedSiteSlug]);
}
...
function createItemToAddToCart({
	...
	if (
		(productAlias.startsWith('jetpack_backup') || productAlias.startsWith('jetpack_search')) &&
		isJetpackNotAtomic
	) {
