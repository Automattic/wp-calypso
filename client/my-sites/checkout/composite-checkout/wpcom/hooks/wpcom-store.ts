...
type WpcomStoreAction =
	...
	| { type: 'SET_RECAPTCHA_CLIENT_ID'; payload: string }
...
export function useWpcomStore(
	...
	function recaptchaClientIdReducer( state: string, action: WpcomStoreAction ): string {
		switch ( action.type ) {
	...
	registerStore( 'wpcom', {
		...
		actions: {
			...
			setRecaptchaClientId( payload: string ): WpcomStoreAction {
