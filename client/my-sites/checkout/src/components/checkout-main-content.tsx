...
import { SubmitButtonSlotContext } from '../lib/submit-button-slot';
...
export default function CheckoutMainContent( {
	...
	// Slot for portaling the real payment-method submit button into the sidebar.
	// `CheckoutFormSubmit` below renders payment-method-specific buttons (Apple Pay/Google Pay
	// need genuine user clicks on their native buttons), so instead of a DOM-click proxy we
	// render them directly where the user sees the Pay button.
...
function CheckoutTermsAndCheckboxes( {
	...
	// The "less legalese, more trust" experiment moves the legal text block into a modal
	// opened from the sidebar "Read more" link. We keep it visible inline only when a
	// consent checkbox references it ("I have read and agree to all of the above.").
	const requiresInlineTerms = hasMarketplaceProduct || has100YearPlan;

	return (
		<CheckoutTermsAndCheckboxesWrapper className="checkout-terms-and-checkboxes">
			{ requiresInlineTerms && <BeforeSubmitCheckoutHeader /> }
...
/**
 * Renders `CheckoutFormSubmit` into the sidebar Pay-button slot via a React portal.
 *
 * The component stays in its original position inside `<CheckoutStepGroup>` so it keeps
 * full step-state awareness (only active on the last step, disabled until all prior
 * steps complete, payment-method-specific submit button element, etc.). The portal
 * just relocates its DOM output into the sidebar so the user clicks the real button —
 * no DOM-click proxy.
 */
function PortaledCheckoutFormSubmit( {
	...
	const { slotEl } = useContext( SubmitButtonSlotContext );
	const submit = <CheckoutFormSubmit validateForm={ validateForm } />;
	if ( ! slotEl ) {
		return null;
	}
	return createPortal( submit, slotEl );
}
