...
const Collection = ({ children, component, examplesToMount = 10, filter, section = 'design' }) => {
	let showCounter = 0;
	const summary = [];
	...
	const examples = React.Children.map(children, (example) => {
		...
		const exampleLink = `/devdocs/${section}/${encodeURIComponent(camelCaseToSlug(exampleName))}`;
