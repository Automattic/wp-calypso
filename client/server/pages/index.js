...
function getAssetsPath(target) {
	const result = path.join(ASSETS_PATH, target ? `assets-${target}.json` : 'assets-fallback.json');
	return result;
}
...
function handleLocaleSubdomains(req, res, next) {
	const langSlug = endsWith(req.hostname, config('hostname')) ? split(req.hostname, '.')[0] : null;
...
module.exports = function () {
	...
	app.get('/domain-services/:action', setupDefaultContext('entry-domains-landing'), (req, res) => {
