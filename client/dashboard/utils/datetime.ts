...
/**
 * Get ISO 8601 week date components for a given date.
 *
 * ISO 8601 week dates follow these rules:
 * - Week 1 is the week containing the first Thursday of the year
 * - Weeks run Monday (day 1) through Sunday (day 7)
 * - The year in the result may differ from the calendar year for dates
 *   at the beginning/end of the calendar year
 */
export function getISOWeekDate( date: Date ): { year: number; week: string; day: number } {
	...
	// This determines which ISO year the date belongs to
	const thursdayDate = new Date( utcDate );
	thursdayDate.setUTCDate( utcDate.getUTCDate() + 4 - dayNum );
	...
	return {
		year: thursdayDate.getUTCFullYear(),
