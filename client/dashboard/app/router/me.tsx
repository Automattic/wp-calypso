...
/**
 * Resolver route for post-checkout plan-change redirects.
 *
 * The plan-upgrade stepper sends users here after checkout because the new
 * purchase ID isn't known at checkout-build time. This route finds the site's
 * active plan and redirects to its purchase settings page before any component
 * renders — so the user sees only the Dashboard's pulsing WP logo during the
 * transition (no intermediate loading flash).
 */
