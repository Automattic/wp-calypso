...
describe( 'getDeleteSiteRedirectIntent', () => {
	it( 'returns cancel when the purchase is cancelable', () => {
		const purchase = {
			is_cancelable: true,
			is_removable: false,
		} as Purchase;

		expect( getDeleteSiteRedirectIntent( purchase ) ).toBe( 'cancel' );
	} );
	...
	it( 'prefers cancel when both cancel and remove are available', () => {
		const purchase = {
			is_cancelable: true,
			is_removable: true,
		} as Purchase;
