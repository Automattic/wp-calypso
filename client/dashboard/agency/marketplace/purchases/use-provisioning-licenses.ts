import { invalidateAgencyLicenses, pendingAgencySitesQuery } from '@automattic/api-queries';
...
function toProvisioningKeys( data: unknown ): string[] {
	...
	return ( data as PendingAgencySite[] )
		.filter( ( { features } ) => features?.wpcom_atomic?.state === 'provisioning' )
		.map( ( { features } ) => features.wpcom_atomic.license_key );
}
...
export function useProvisioningLicenses( agencyId: number ): {
	...
	const { data } = useQuery( {
		...pendingAgencySitesQuery( agencyId ),
		enabled: agencyId > 0,
