...
import woopaymentsLogo from '../marketplace/exclusive-offers/images/woopayments.svg';
 ...
 */
export default function WooPaymentsIllustration( {
	title,
	brandMark = false,
}: {
	title?: string;
	/**
	 * A4AD-190: put the WooPayments lockup on the storefront screen instead of a
	 * generic cart glyph, so the illustration names the product too. Opt-in, so
	 * the Overview keeps the art it has.
	 */
	brandMark?: boolean;
} ) {
	return (
		<>
			...
			<SVG
				viewBox={ brandMark ? '75 40 281 152' : '-2 30 294 152' }
				fill="none"
			...
			>
				...
				{ brandMark ? (
					<>
						<Rect x="185" y="76" width="141" height="100" rx="4.62" fill="white" stroke="#C3C4C7" />
						<Rect x="145" y="66" width="141" height="100" rx="4.62" fill="white" stroke="#C3C4C7" />
					</>
				) : (
					<>
