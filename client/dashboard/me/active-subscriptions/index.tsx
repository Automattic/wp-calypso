...
// FIXME
const sites: Array< { ID: string; name: string; domain: string; URL: string } > = [];

const fields: Fields< ActiveSubscription > = [
	{
		id: 'site',
		label: __( 'Site' ),
		type: 'text',
	...
	},
	{
		...
		getValue: ( { item }: { item: ActiveSubscription } ) => {
			...
			return getDisplayName( item ) + ' ' + item.blogname + ' ' + ( item.site_slug || item.domain );
		},
		render: ( { item }: { item: ActiveSubscription } ) => {
