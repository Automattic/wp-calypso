...
const SitesSidebar = ( { path }: { path: string } ) => {
	...
	const menuItems = [
		createItem( {
			...
			trackEventProps: {
				menu_item: 'Jetpack Cloud / Dashboard / Needs Attention',
			},
		} ),
		createItem( {
			...
			icon: starEmpty,
			path: JETPACK_MANAGE_DASHBOARD_LINK_NEEDS_ATTENTION,
			link: JETPACK_MANAGE_DASHBOARD_LINK_FAVORITES,
			title: translate( 'Favorites' ),
			trackEventProps: {
				menu_item: 'Jetpack Cloud / Dashboard / Favorites',
			},
		} ),
		createItem( {
			...
			title: translate( 'All' ),
			trackEventProps: {
				menu_item: 'Jetpack Cloud / Dashboard / All',
			},
		} ),
	];
	...
	return (
		<NewSidebar
			...
			backButtonProps={ {
				...
				onClick: () => {
					dispatch(
						recordTracksEvent( 'calypso_jetpack_sidebar_sites_dashboard_back_button_click' )
					);
