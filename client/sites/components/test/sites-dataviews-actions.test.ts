...
describe( 'isActionEligible', () => {
	...
	it.each( [ 'prepare-for-launch', 'domains', 'jetpack-cloud' ] )(
		'%s action is hidden for P2 site',
		( action ) => {
	...
	it.each( [ 'prepare-for-launch', 'settings', 'domains', 'restore', 'jetpack-cloud' ] )(
		'%s action is hidden for non-admin users',
		( action ) => {
	...
	it.each( [ 'jetpack-cloud' ] )( '%s action is shown for non-Atomic Jetpack sites', ( action ) => {
		const result = isActionEligible( action, capabilitiesMock );
