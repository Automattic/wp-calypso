...
export const errors = (state = {}, action) => {
	switch (action.type) {
		...
		case MEDIA_REQUEST_FAILURE: {
			...
			const mediaErrors = Array.isArray(action.error.errors) ? action.error.errors : [action.error];
		...
		case MEDIA_ERRORS_CLEAR:
			...
			return {
				...
				[action.siteId]: pickBy(
					mapValues(state[action.siteId], (mediaErrors) => without(mediaErrors, action.errorType)),
