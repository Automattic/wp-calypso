 ...
 * @deprecated Use requestSingleTheme() instead.
...
export function requestSingleTheme( themeId, siteId, sourceType, locale ) {
	return ( dispatch ) => {
		dispatch( {
			type: THEME_REQUEST,
			siteId,
...
function makeWpcomRequest( dispatch, path, themeId, siteId, locale ) {
	return wpcom.req
		.get( path, Object.assign( { apiVersion: '1.2' }, locale ? { locale } : null ) )
		.then( ( theme ) => {
			dispatch( receiveTheme( normalizeWpcomTheme( theme ), siteId ) );
			dispatch( {
				type: THEME_REQUEST_SUCCESS,
