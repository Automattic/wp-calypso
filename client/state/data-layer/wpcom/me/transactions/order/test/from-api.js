...
describe('wpcom-api', () => {
	...
	describe('convertProcessingStatus()', () => {
		...
		test('should convert to processing.', () => {
			expect(convertProcessingStatus('processing')).toEqual(ORDER_TRANSACTION_STATUS.PROCESSING);
		});

		test('should convert to failure.', () => {
			expect(convertProcessingStatus('payment-failure')).toEqual(ORDER_TRANSACTION_STATUS.FAILURE);
		});
