	...
	describe('me/transactions/order', () => {
		...
		describe('onSuccess()', () => {
			test('should return the expected setting action for populating state.', () => {
				...
				expect(onSuccess(action, detail)).toEqual(setOrderTransaction(action.orderId, detail));
			});
		});
		...
		describe('onError()', () => {
			test('should return the expected error notice action.', () => {
				...
				expect(onError({ orderId }, error)).toEqual(setOrderTransactionError(orderId, error));
			});
		});
	});
});
