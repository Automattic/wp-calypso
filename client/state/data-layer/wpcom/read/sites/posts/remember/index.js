 ...
 */
import { READER_REMEMBERED_POSTS_REMEMBER } from 'state/action-types';
...
export function requestRememberPost( { dispatch, getState }, action ) {
		...
		http(
			{
				...
				path: `/read/sites/${ action.payload.siteId }/posts/${ action.payload.postId }/remember`,
...
export function receiveRememberPost( store, action, response ) {
	// validate that it worked
	const isFollowing = !! ( response && response.success );
	if ( ! isFollowing ) {
		receiveRememberPostError( store, action );
		return;
	}
...
export function receiveRememberPostError(
	{ dispatch },
...
export default {
	[ READER_REMEMBERED_POSTS_REMEMBER ]: [
		dispatchRequest( requestRememberPost, receiveRememberPost, receiveRememberPostError ),
	],
};
