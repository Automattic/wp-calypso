...
import { getCommentParentKey, updateExistingIn } from './utils';
...
function getLikeSetter({ like = true, likeCount }) {
    return comment => comment.set('i_like', like).update('like_count', count => {
        ...
        case COMMENTS_RECEIVE:
            ...
            return updateSpecificState(state, action, function(comments = Immutable.List()) {
                let newComments = comments
                    .filter(comment => !newIds.has(comment.get('ID')))
        ...
        case COMMENTS_ERROR:
            return updateSpecificState(state, action, (comments = Immutable.List()) =>
                updateExistingIn(
                    ...
                    comment =>
                        comment
                            .set('placeholderState', PLACEHOLDER_STATE.ERROR)
...
export function requests(state = Immutable.Map(), action) {
    switch (action.type) {
        ...
        case COMMENTS_REQUEST_FAILURE:
            return updateSpecificState(state, action, (requestStatuses = Immutable.Map()) =>
                requestStatuses.set(action.requestId, action.type));
        case SERIALIZE:
            return {};
        case DESERIALIZE:
            return Immutable.Map();
