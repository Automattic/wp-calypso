...
import { getCommentParentKey, createRequestId } from '../utils';
...
describe('actions', () => {
    ...
    describe('#receivePost()', () => {
        ...
        it('should not dispatch a thing if the request is already in flight', () => {
            const requestId = createRequestId(
                MANY_COMMENTS_POST.siteId,
                MANY_COMMENTS_POST.postId,
                { order: 'DESC', number: NUMBER_OF_COMMENTS_PER_FETCH }
            );
            ...
            requestPostComments(MANY_COMMENTS_POST.siteId, MANY_COMMENTS_POST.postId)(
                dispatchSpy,
                getStateStub
            );
        ...
        it('should dispatch correct first request actions', function() {
            ...
            nock(API_DOMAIN)
                .get(
                    `/rest/v1.1/sites/${MANY_COMMENTS_POST.siteId}/posts/${MANY_COMMENTS_POST.postId}/replies/`
                )
                .query({ order: 'DESC', number: NUMBER_OF_COMMENTS_PER_FETCH })
                .reply(200, { found: 123, comments: [] });

            const reqPromise = requestPostComments(
                MANY_COMMENTS_POST.siteId,
                MANY_COMMENTS_POST.postId
            )(dispatchSpy, getStateStub);

            expect(dispatchSpy).to.have.been.calledWith({
                type: COMMENTS_REQUEST,
                requestId: createRequestId(MANY_COMMENTS_POST.siteId, MANY_COMMENTS_POST.postId, {
                    order: 'DESC',
        ...
        it('should dispatch correct consecutive request actions', function() {
            ...
            expect(dispatchSpy).to.have.been.calledWith({
                ...
                requestId: createRequestId(MANY_COMMENTS_POST.siteId, MANY_COMMENTS_POST.postId, {
                    order: 'DESC',
                    number: NUMBER_OF_COMMENTS_PER_FETCH,
                    before: new Date(beforeDateString).toISOString(),
                }),
            });
    ...
    describe('#writeComment()', () => {
            ...
            nock(API_DOMAIN)
                ...
                .reply(200, {
                    ...
                    post: {
                        ...
                        type: 'post',
                        link: 'https:\/\/public-api.wordpress.com\/rest\/v1.1\/sites\/' +
                            MANY_COMMENTS_POST.siteId +
                            '\/posts\/' +
                            MANY_COMMENTS_POST.postId,
                    },
        ...
        it('should dispatch correct actions', function() {
            const dispatchSpy = sinon.spy();
            const writeCommentThunk = writeComment(
    ...
    describe('#removeComment()', () => {
        it('should dispatch remove for a placeholder when provided', () => {
            const removeCommentAction = removeComment(
                MANY_COMMENTS_POST.siteId,
                MANY_COMMENTS_POST.postId,
                'placeholder-123'
            );
