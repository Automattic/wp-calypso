 ...
 */
import { items, requests, totalCommentsCount, default as comments } from '../reducer';
import {
    COMMENTS_LIKE,
...
describe('reducer', () => {
    describe('#items()', () => {
        it('should build an ordered by date list', () => {
            ...
            [...commentsNestedTree]
                ...
                .forEach((comment, idx) =>
                    expect(comment.ID).to.equal(specificRes.getIn([idx, 'ID'])));
        });
        ...
        it('should build correct items list on consecutive calls', () => {
            ...
            const specificCommentItemList = getPostCommentItems(
                { comments: { items: res2 } },
                1,
                1
            );
        ...
        it('should set error state on a placeholder', () => {
            ...
            expect(
                specificRes.find(c => c.get('ID') === 'placeholder-123').get('placeholderState')
            ).to.equal(PLACEHOLDER_STATE.ERROR);
        });
    }); // end of items
    ...
    describe('#requests', () => {
        it('should set state of query according to the action', () => {
            ...
            let specificRes = getPostCommentRequests(
                { comments: { requests: res } },
                siteId,
                postId
            );
    ...
    describe('#totalCommentsCount()', () => {
        it('should update post comments count', () => {
            ...
            const specificRes = getPostTotalCommentsCount(
                { comments: { totalCommentsCount: newState } },
                1,
                1
            );
