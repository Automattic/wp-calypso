 ...
 */
import { createRequestId, buildCommentsTree, updateExistingIn } from '../utils';
...
describe('utils', () => {
    ...
    describe('#buildCommentsTree()', () => {
        it('should build valid tree', () => {
            const commentsTreeForPost = buildCommentsTree(
                Immutable.fromJS([
                    {
                        ID: 11,
                        parent: { ID: 9 },
        ...
        it('should be ordered by date in root and children lists', () => {
            const finalRes = buildCommentsTree(
                Immutable.fromJS([
                    {
                        ID: 11,
                        parent: { ID: 9 },
            ...
            const validateDates = childNodesList => {
                const actualDatesList = childNodesList.map(
                    commentId => new Date(finalRes.getIn([commentId, 'data', 'date']))
                );
    ...
    describe('#updateExistingIn()', () => {
        it('should modify only first item', () => {
            ...
            const newList = updateExistingIn(
                list,
