	...
	(state = null, action) => {
		switch (action.type) {
			...
			case 'FLUX_TRANSACTION_NEW_CREDIT_CARD_DETAILS_SET':
				return has(action, 'rawDetails.country') ? get(action, 'rawDetails.country', null) : state;
 ...
 * @returns {object} - The updated global state.
 */
export const postalCode = withSchemaValidation(paymentPostalCodeSchema, (state = null, action) => {
	switch (action.type) {
