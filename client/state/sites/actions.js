...
export function receiveSitesPlans( sites ) {
	return {
		type: SITES_PLANS_RECEIVE,
		sites,
	};
}
	...
	return dispatch => {
		...
		return wpcom
			...
			.sites( {
				...
				fields:
					'ID,URL,name,capabilities,jetpack,visible,is_private,is_vip,icon,jetpack_modules,single_user_site,is_multisite,options', //eslint-disable-line max-len
				options:
...
export function requestSitesPlan() {
	return dispatch => {
		dispatch( {
			type: SITES_PLANS_REQUEST,
		} );
		...
		return wpcom
			...
			.then( response => {
				dispatch( receiveSitesPlans( response.sites ) );
				// dispatch( {
				// 	type: SITES_REQUEST_SUCCESS,
				// } );
			} )
