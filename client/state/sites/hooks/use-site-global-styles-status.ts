...
const getGlobalStylesInfoForSite = ( siteId: number | null ): Promise< GlobalStylesStatus > => {
	// When the site hasn't been created yet, limit global styles by default so we don't
	// surface premium-style notices before we can fetch the real status from the server.
	if ( siteId === null ) {
