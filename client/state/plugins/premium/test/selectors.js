...
describe( 'Premium Plugin Selectors', () => {
	...
	describe( 'isInstalling', () => {
		...
		test( 'Should get `true` if there is a plugin installing on the requested site, filtering for a specied plugin (akismet)', () => {
			expect( isInstalling( state, 'installing.site', 'akismet' ) ).toBe( true );
		} );

		test( 'Should get `true` if there is a plugin configuring on the requested site, filtering for a specied plugin (akismet)', () => {
