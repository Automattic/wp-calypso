...
describe('canUpgradeToPlan', () => {
	...
	test('should return true from lower-tier plans to higher-tier plans', () => {
		[
			...
			expect(canUpgradeToPlan(makeState(siteId, planOwned), siteId, planToPurchase)).toBe(true)
		);
	});
	...
	test('should return false from yearly plans to monthly plans', () => {
		[
			...
			expect(canUpgradeToPlan(makeState(siteId, planOwned), siteId, planToPurchase)).toBe(false)
		);
	});
