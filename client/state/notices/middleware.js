...
export const onGravatarUploadRequestFailure = () =>
	errorNotice(translate('Hmm, your new profile photo was not saved. Please try uploading again.'));
...
export const onBillingTransactionRequestFailure = ({ transactionId, error }) => {
	...
	if ('invalid_receipt' === error.error) {
		return errorNotice(translate("Sorry, we couldn't find receipt #%s.", { args: transactionId }), {
			id,
			displayOnNextPage,
			duration: 5000,
		});
