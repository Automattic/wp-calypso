...
describe('#httpHandler', () => {
	...
	test('should call `onSuccess` when a response returns with data', async () => {
		...
		expect(dispatch).toHaveBeenCalledWith(extendAction(succeeder, successMeta({ body: data })));
	});
	...
	test('should set appropriate headers', () => {
		...
		headers.forEach(([key, value]) =>
			expect(fetch.mock.calls[fetch.mock.calls.length - 1][1].headers).toHaveProperty(key, value)
		);
	});
