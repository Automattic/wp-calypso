...
function canEditPaymentDetails(purchase) {
    return config.isEnabled('upgrades/credit-cards') &&
        !isExpired(purchase) &&
...
const ManagePurchase = React.createClass({
    ...
    renderCreditCardExpiringNotice() {
        ...
        if (creditCardExpiresBeforeSubscription(purchase)) {
            return (
                ...
                >
                    {this.translate(
                        'Your %(cardType)s ending in %(cardNumber)d expires %(cardExpiry)s – before the next renewal. ' +
    ...
    renderPrice() {
        const purchase = getPurchase(this.props),
            ...
            period = productSlug && isMonthly(productSlug)
